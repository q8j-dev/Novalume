...
public:
	bool success;
	RakNet::RakPeerInterface* const networkPeer;

	RakNet::RakPeerInterface* operator->() { return networkPeer; }
	Peer()
		:success(false)
		,networkPeer(RakNet::RakPeerInterface::GetInstance())
	{
		networkPeer->AttachPlugin(this);
	}
	~Peer()
	{
		networkPeer->Shutdown(500);
		networkPeer->DetachPlugin(this);
		RakNet::RakPeerInterface::DestroyInstance(networkPeer);
	}
...
BOOST_AUTO_TEST_CASE(SimpleConnect)
{
	...
	{
		Packet* packet = client.networkPeer->Receive();
		if (packet!=0)
			client.networkPeer->DeallocatePacket(packet);
		packet = server.networkPeer->Receive();
		if (packet!=0)
			server.networkPeer->DeallocatePacket(packet);
