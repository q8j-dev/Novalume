...
namespace RBX {
	...
	// A free angular joint that constrains only the two attachment pivots.
	// Current BallSocketConstraint instances use this instead of the legacy
	// RotateJoint, whose axis-alignment row would incorrectly turn them into
	// hinges.
	class BallSocketJoint : public MultiJoint
	{
	private:
		ConstraintBallInSocket* ballInSocket;

	protected:
		void putInKernel(Kernel* kernel) override;
		void removeFromKernel() override;
		JointType getJointType() const override { return Joint::ROTATE_JOINT; }

	public:
		BallSocketJoint();
		~BallSocketJoint() override;
	};
