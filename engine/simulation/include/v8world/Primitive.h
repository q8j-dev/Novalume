...
namespace RBX {
	...
	public:
		...
		unsigned int getCollisionGroupId() const { return collisionGroupId; }
		unsigned int getCollisionGroupMask() const { return collisionGroupMask; }
		void setCollisionGroup(unsigned int id, unsigned int mask)
		{
			collisionGroupId = id;
			collisionGroupMask = mask;
		}
		bool canCollideWith(const Primitive& other) const
		{
			return noCollisionPartners.find(const_cast<Primitive*>(&other)) ==
					noCollisionPartners.end() &&
				(collisionGroupMask & (1u << other.collisionGroupId)) != 0 &&
				(other.collisionGroupMask & (1u << collisionGroupId)) != 0;
		}
		void setCanCollideWith(Primitive& other, bool value);
	...
	private:
		...
		unsigned int collisionGroupId;
		unsigned int collisionGroupMask;
		std::map<Primitive*, unsigned int> noCollisionPartners;
