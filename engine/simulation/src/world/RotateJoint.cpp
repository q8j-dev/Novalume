...
void BallSocketJoint::putInKernel(Kernel* kernel)
{
	MultiJoint::putInKernel(kernel);
	Body* first = getPrimitive(0)->getBody();
	Body* second = getPrimitive(1)->getBody();
	if (kernel->getUsingPGSSolver())
	{
		if (ballInSocket && (ballInSocket->getBodyA()->getUID() != first->getUID() ||
			ballInSocket->getBodyB()->getUID() != second->getUID()))
		{
			delete ballInSocket;
			ballInSocket = NULL;
		}
		if (!ballInSocket)
			ballInSocket = new ConstraintBallInSocket(first, second);
		ballInSocket->setPivotA(jointCoord0.translation);
		ballInSocket->setPivotB(jointCoord1.translation);
		kernel->pgsSolver.addConstraint(ballInSocket);
	}
	else
	{
		Point* point0 = kernel->newPointLocal(first, jointCoord0.translation);
		Point* point1 = kernel->newPointLocal(second, jointCoord1.translation);
		addToMultiJoint(point0, point1, new PointToPointBreakConnector(
			point0, point1, getJointK(), Math::inf()));
	}
}

void BallSocketJoint::removeFromKernel()
{
	RBXASSERT(getKernel());
	if (getKernel()->getUsingPGSSolver() && ballInSocket)
