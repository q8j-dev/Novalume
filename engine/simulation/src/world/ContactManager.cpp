...
Primitive* ContactManager::getHitAllSlow(
	const RbxRay& worldRay,
	const std::vector<const Primitive*>* ignorePrim,
	const HitTestFilter* filter,
...
{
	ReadOnlyValidator readOnlyValidator(concurrencyValidator);
	if (!worldRay.origin().isFinite() || !worldRay.direction().isFinite())
		return NULL;
	const float maxDistance = worldRay.direction().magnitude();
	if (maxDistance <= 0.0f || maxDistance >= 5000.0f)
		return NULL;
	G3D::Array<const Primitive*> ignored;
	if (ignorePrim)
		ignored = *ignorePrim;
	bool stopped = false;
	Primitive* answer = getSlowHit(world->getPrimitives(), worldRay.unit(), ignored,
		filter, hitPointWorld, surfaceNormal, surfaceMaterial, maxDistance, stopped);
	return stopped ? NULL : answer;
}
