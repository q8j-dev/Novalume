...
, collisionGroupId(0)
, collisionGroupMask(0xffffffffu)
, specificGravity(0.0)
, body(new Body())
...
Primitive::~Primitive() 
{
	...
	for (std::map<Primitive*, unsigned int>::iterator it = noCollisionPartners.begin();
		it != noCollisionPartners.end(); ++it)
		it->first->noCollisionPartners.erase(this);
	noCollisionPartners.clear();
...
void Primitive::setCanCollideWith(Primitive& other, bool value)
{
	if (&other == this)
		return;
	if (value)
	{
		std::map<Primitive*, unsigned int>::iterator found =
			noCollisionPartners.find(&other);
		if (found == noCollisionPartners.end())
			return;
		if (--found->second == 0)
			noCollisionPartners.erase(found);
		found = other.noCollisionPartners.find(this);
		RBXASSERT(found != other.noCollisionPartners.end());
		if (found != other.noCollisionPartners.end() && --found->second == 0)
			other.noCollisionPartners.erase(found);
	}
	else
	{
		++noCollisionPartners[&other];
		++other.noCollisionPartners[this];
	}
	if (world)
		world->onPrimitivePreventCollideChanged(this);
	if (other.world)
		other.world->onPrimitivePreventCollideChanged(&other);
}
