...
static const Reflection::RefPropDescriptor<Humanoid, PartInstance> propRootPart(
	"RootPart", category_Data, &Humanoid::getRootPartDangerous, NULL,
...
static const Reflection::PropDescriptor<Humanoid, bool> propAutomaticScalingEnabled(
    "AutomaticScalingEnabled", "Game", &Humanoid::getAutomaticScalingEnabled,
    &Humanoid::setAutomaticScalingEnabled);
...
static Reflection::BoundFuncDesc<Humanoid, Humanoid::BodyPartR15(shared_ptr<Instance>)>
	...
	Security::None);
static Reflection::BoundFuncDesc<Humanoid, bool(Humanoid::BodyPartR15, shared_ptr<Instance>)>
	func_ReplaceBodyPartR15(&Humanoid::replaceBodyPartR15, "ReplaceBodyPartR15",
	"bodyPart", "part", Security::None);
static Reflection::BoundFuncDesc<Humanoid, Vector3(shared_ptr<Instance>, Humanoid::BodyPartR15)>
	func_GetAccessoryHandleScale(&Humanoid::getAccessoryHandleScale,
	"GetAccessoryHandleScale", "instance", "partType", Security::RobloxScript);
...
static Reflection::BoundYieldFuncDesc<Humanoid, bool(std::string)> desc_PlayEmoteAsync(
	&Humanoid::playEmoteAsync, "PlayEmoteAsync", "emoteName", Security::None);
...
static Reflection::EventDesc<Humanoid, void(bool, shared_ptr<Instance>)> event_EmoteTriggered(
	&Humanoid::emoteTriggeredSignal, "EmoteTriggered", "emoteSuccess",
	"emoteTrack", Security::RobloxScript);
REFLECTION_END();
...
void Humanoid::onDescendantAdded(Instance* instance)
{
	...
	{
		if (DoubleValue* scale = Instance::fastDynamicCast<DoubleValue>(instance))
		{
			// Replication may materialize RigType before the scale-value
			// children.  Observe each value as it arrives so the final authored
			// R15/Rthro proportions replace the temporary classic fallback.
			connectAvatarScaleValue(scale);
			refreshAvatarScale();
		}
	}
...
{ 
	...
	if (type != rigType)
	{
		...
		if (getParent())
		{
			...
			for (std::size_t index = 0; index < numChildren(); ++index)
				if (DoubleValue* scale = Instance::fastDynamicCast<DoubleValue>(getChild(index)))
					connectAvatarScaleValue(scale);
			refreshAvatarScale();
		}
...
void Humanoid::setAutomaticScalingEnabled(bool value)
{
    if (automaticScalingEnabled != value)
    {
        automaticScalingEnabled = value;
        raisePropertyChanged(propAutomaticScalingEnabled);
		if (value)
			refreshAvatarScale();
    }
}
...
PartInstance* Humanoid::getRootPartDangerous() const
{
	const Instance* character = getParent();
	if (character)
	{
		if (const PartInstance* root = Instance::fastDynamicCast<PartInstance>(
				character->findConstFirstChildByName("HumanoidRootPart")))
			return const_cast<PartInstance*>(root);
	}
...
{
	if (DoubleValue* scale = Instance::fastDynamicCast<DoubleValue>(child.get()))
	{
		connectAvatarScaleValue(scale);
		refreshAvatarScale();
	}
...
void Humanoid::connectAvatarScaleValue(DoubleValue* value)
{
	if (!value)
		return;
	rbx::signals::scoped_connection* connection = NULL;
	if (value->getName() == "BodyHeightScale") connection = &bodyHeightScaleChanged;
	else if (value->getName() == "BodyWidthScale") connection = &bodyWidthScaleChanged;
	else if (value->getName() == "BodyDepthScale") connection = &bodyDepthScaleChanged;
	else if (value->getName() == "HeadScale") connection = &headScaleChanged;
	else if (value->getName() == "BodyTypeScale") connection = &bodyTypeScaleChanged;
	else if (value->getName() == "BodyProportionScale") connection = &bodyProportionScaleChanged;
	if (connection)
	{
...
{
	const DoubleValue* value = Instance::fastDynamicCast<const DoubleValue>(
		humanoid.findConstFirstChildByName(name));
	const double result = value ? value->getValue() : fallback;
	return std::isfinite(result) && result > 0.0 ? result : fallback;
}

struct AvatarProportionTargets
{
	const char* partName;
	Vector3 classic;
	Vector3 normal;
...
// These are the authored neutral sizes in the exact current Studio
// characterR15.rbxm and RigBuilder/AnthroRigs.rbxm models. Roblox's scale-type
// contract maps a body part from the proportions it was authored for to the
// requested Classic/Normal/Slender target, then applies the four user-facing
// dimension scales. Keeping the target table here also lets custom body parts
// with AvatarPartScaleType use the same interpolation as the canonical rigs.
static const AvatarProportionTargets avatarProportionTargets[] = {
	{"Head", Vector3(2.0f, 1.0f, 1.0f), Vector3(0.774738f, 1.03269f, 0.843667f), Vector3(0.618412f, 1.03005f, 0.789122f)},
	{"LeftHand", Vector3(1.0f, 0.300031f, 1.0f), Vector3(0.585897f, 0.969546f, 0.712358f), Vector3(0.394135f, 0.942286f, 0.753491f)},
	{"RightHand", Vector3(1.0f, 0.300031f, 1.0f), Vector3(0.585897f, 0.969546f, 0.712358f), Vector3(0.394135f, 0.942286f, 0.753491f)},
	{"LeftLowerArm", Vector3(1.0f, 1.05192f, 1.0f), Vector3(0.537162f, 1.00432f, 0.703369f), Vector3(0.809134f, 0.84988f, 0.90461f)},
	{"RightLowerArm", Vector3(1.0f, 1.05192f, 1.0f), Vector3(0.537162f, 1.00432f, 0.703369f), Vector3(0.809134f, 0.84988f, 0.90461f)},
	{"LeftUpperArm", Vector3(1.0f, 1.16867f, 1.0f), Vector3(1.09553f, 1.50064f, 1.16344f), Vector3(0.74377f, 1.03277f, 0.706485f)},
	{"RightUpperArm", Vector3(1.0f, 1.16867f, 1.0f), Vector3(1.09553f, 1.50064f, 1.16344f), Vector3(0.74377f, 1.03277f, 0.706485f)},
	{"UpperTorso", Vector3(2.0f, 1.60003f, 1.0f), Vector3(1.35398f, 2.08418f, 1.20334f), Vector3(1.61288f, 2.75854f, 1.17362f)},
	{"LeftFoot", Vector3(1.0f, 0.3f, 1.0f), Vector3(0.646259f, 0.544544f, 1.42333f), Vector3(0.486346f, 0.401856f, 1.16493f)},
	{"RightFoot", Vector3(1.0f, 0.3f, 1.0f), Vector3(0.646259f, 0.5446f, 1.42333f), Vector3(0.486346f, 0.401856f, 1.16493f)},
	{"LeftLowerLeg", Vector3(1.0f, 1.19309f, 1.0f), Vector3(0.791186f, 1.89912f, 1.01706f), Vector3(0.691f, 1.61121f, 0.880905f)},
	{"RightLowerLeg", Vector3(1.0f, 1.19309f, 1.0f), Vector3(0.791186f, 1.89932f, 1.01706f), Vector3(0.691f, 1.61121f, 0.880905f)},
	{"LeftUpperLeg", Vector3(1.0f, 1.21657f, 1.0f), Vector3(0.673538f, 1.37216f, 0.739193f), Vector3(0.544801f, 1.60631f, 0.733537f)},
	{"RightUpperLeg", Vector3(1.0f, 1.21657f, 0.999967f), Vector3(0.673538f, 1.37231f, 0.739193f), Vector3(0.544801f, 1.60631f, 0.733537f)},
	{"LowerTorso", Vector3(2.0f, 0.400055f, 1.0f), Vector3(1.57657f, 1.73609f, 1.21574f), Vector3(1.06745f, 0.596527f, 0.926699f)},
};

static const AvatarProportionTargets* findAvatarProportionTargets(const std::string& partName)
{
	for (const AvatarProportionTargets& targets : avatarProportionTargets)
		if (partName == targets.partName)
			return &targets;
	return NULL;
}

static Vector3 interpolateAvatarTarget(const Vector3& first,
	const Vector3& second, float alpha)
{
	return first + (second - first) * alpha;
}

static Vector3 divideAvatarTarget(const Vector3& target, const Vector3& authored)
{
	return Vector3(target.x / authored.x, target.y / authored.y,
		target.z / authored.z);
}

static Vector3 avatarScaleForPart(const PartInstance& part,
	float width, float height, float depth, float head,
	float bodyType, float bodyProportion)
{
	if (part.getName() == "HumanoidRootPart")
		return Vector3::one();

	Vector3 result = part.getName() == "Head"
		? Vector3(head, head, head) : Vector3(width, height, depth);
	const AvatarProportionTargets* targets =
		findAvatarProportionTargets(part.getName());
	if (!targets)
		return result;

	const Vector3 rthroTarget = interpolateAvatarTarget(
		targets->normal, targets->slender, bodyProportion);
	const Vector3 requestedTarget = interpolateAvatarTarget(
		targets->classic, rthroTarget, bodyType);
	const StringValue* scaleType = Instance::fastDynamicCast<const StringValue>(
		part.findConstFirstChildByName("AvatarPartScaleType"));
	const Vector3* authoredTarget = &targets->classic;
	if (scaleType && scaleType->getValue() == "ProportionsNormal")
		authoredTarget = &targets->normal;
	else if (scaleType && scaleType->getValue() == "ProportionsSlender")
		authoredTarget = &targets->slender;
	return result * divideAvatarTarget(requestedTarget, *authoredTarget);
}

static const char* bodyPartR15Name(Humanoid::BodyPartR15 bodyPart)
{
	switch (bodyPart)
	{
	case Humanoid::BODY_PART_R15_HEAD: return "Head";
	case Humanoid::BODY_PART_R15_UPPER_TORSO: return "UpperTorso";
	case Humanoid::BODY_PART_R15_LOWER_TORSO: return "LowerTorso";
	case Humanoid::BODY_PART_R15_LEFT_FOOT: return "LeftFoot";
	case Humanoid::BODY_PART_R15_LEFT_LOWER_LEG: return "LeftLowerLeg";
	case Humanoid::BODY_PART_R15_LEFT_UPPER_LEG: return "LeftUpperLeg";
	case Humanoid::BODY_PART_R15_RIGHT_FOOT: return "RightFoot";
	case Humanoid::BODY_PART_R15_RIGHT_LOWER_LEG: return "RightLowerLeg";
	case Humanoid::BODY_PART_R15_RIGHT_UPPER_LEG: return "RightUpperLeg";
	case Humanoid::BODY_PART_R15_LEFT_HAND: return "LeftHand";
	case Humanoid::BODY_PART_R15_LEFT_LOWER_ARM: return "LeftLowerArm";
	case Humanoid::BODY_PART_R15_LEFT_UPPER_ARM: return "LeftUpperArm";
	case Humanoid::BODY_PART_R15_RIGHT_HAND: return "RightHand";
	case Humanoid::BODY_PART_R15_RIGHT_LOWER_ARM: return "RightLowerArm";
	case Humanoid::BODY_PART_R15_RIGHT_UPPER_ARM: return "RightUpperArm";
	case Humanoid::BODY_PART_R15_ROOT_PART: return "HumanoidRootPart";
	default: return NULL;
	}
}

static Humanoid::BodyPartR15 bodyPartR15FromName(const std::string& name)
{
	for (int value = Humanoid::BODY_PART_R15_HEAD;
		value <= Humanoid::BODY_PART_R15_ROOT_PART; ++value)
	{
		const Humanoid::BodyPartR15 bodyPart =
			static_cast<Humanoid::BodyPartR15>(value);
		const char* partName = bodyPartR15Name(bodyPart);
		if (partName && name == partName)
			return bodyPart;
	}
	return Humanoid::BODY_PART_R15_UNKNOWN;
}

Humanoid::BodyPartR15 Humanoid::getBodyPartR15(shared_ptr<Instance> instance)
{
	const PartInstance* part = Instance::fastDynamicCast<const PartInstance>(instance.get());
	if (!part || part->getParent() != getParent())
		return BODY_PART_R15_UNKNOWN;
	return bodyPartR15FromName(part->getName());
}

Vector3 Humanoid::getAccessoryHandleScale(shared_ptr<Instance> instance,
	BodyPartR15 bodyPart)
{
	const PartInstance* handle =
		Instance::fastDynamicCast<const PartInstance>(instance.get());
	const char* partName = bodyPartR15Name(bodyPart);
	const AvatarProportionTargets* targets = partName
		? findAvatarProportionTargets(partName) : NULL;
	if (!handle || !targets)
		return Vector3::one();

	const float bodyType = std::max(0.0f, std::min(1.0f,
		static_cast<float>(getAvatarScaleValue(*this, "BodyTypeScale", 0.0))));
	const float bodyProportion = std::max(0.0f, std::min(1.0f,
		static_cast<float>(getAvatarScaleValue(*this, "BodyProportionScale", 0.0))));
	const Vector3 rthroTarget = interpolateAvatarTarget(
		targets->normal, targets->slender, bodyProportion);
	const Vector3 requestedTarget = interpolateAvatarTarget(
		targets->classic, rthroTarget, bodyType);
	const StringValue* scaleType = Instance::fastDynamicCast<const StringValue>(
		handle->findConstFirstChildByName("AvatarPartScaleType"));
	const Vector3* authoredTarget = &targets->classic;
	if (scaleType && scaleType->getValue() == "ProportionsNormal")
		authoredTarget = &targets->normal;
	else if (scaleType && scaleType->getValue() == "ProportionsSlender")
	...
	const float width = static_cast<float>(getAvatarScaleValue(*this,
		"BodyWidthScale", 1.0));
	const float height = static_cast<float>(getAvatarScaleValue(*this,
		"BodyHeightScale", 1.0));
	const float depth = static_cast<float>(getAvatarScaleValue(*this,
		"BodyDepthScale", 1.0));
	const float head = static_cast<float>(getAvatarScaleValue(*this,
		"HeadScale", 1.0));
	const Vector3 dimensions = bodyPart == BODY_PART_R15_HEAD
		? Vector3(head, head, head) : Vector3(width, height, depth);
	return dimensions * divideAvatarTarget(requestedTarget, *authoredTarget);
}

static void collectCharacterMotors(Instance& instance,
	std::vector<shared_ptr<Motor6D> >& motors)
{
	if (Motor6D* motor = Instance::fastDynamicCast<Motor6D>(&instance))
		motors.push_back(Instance::fastSharedDynamicCast<Motor6D>(shared_from(motor)));
	for (std::size_t index = 0; index < instance.numChildren(); ++index)
		collectCharacterMotors(*instance.getChild(index), motors);
}

void Humanoid::buildRigFromAttachments()
{
	Instance* character = getParent();
	if (!getUseR15() || !character)
		return;
	PartInstance* root = getRootPartDangerous();
	if (!root)
		return;

	// A public rebuild must be idempotent and must not leave Motor6Ds pointing at
	// parts which have been swapped since the last assembly. Remove only joints
	// that are backed by the matching R15 RigAttachment pair; custom motors are
	// preserved.
	std::vector<shared_ptr<Motor6D> > motors;
	collectCharacterMotors(*character, motors);
	for (const shared_ptr<Motor6D>& motor : motors)
	{
		PartInstance* part0 = motor->getPart0Dangerous();
		PartInstance* part1 = motor->getPart1Dangerous();
		const std::string attachmentName = motor->getName() + "RigAttachment";
		if (part0 && part1 &&
			Instance::fastDynamicCast<Attachment>(
				part0->findFirstChildByName(attachmentName)) &&
			Instance::fastDynamicCast<Attachment>(
				part1->findFirstChildByName(attachmentName)))
			motor->setParent(NULL);
	}

	refreshAvatarScale();
	std::vector<PartInstance*> characterParts;
	getParts(characterParts);
	buildJointsFromAttachments(root, characterParts);
}

bool Humanoid::replaceBodyPartR15(BodyPartR15 bodyPart,
	shared_ptr<Instance> instance)
{
	const char* partName = bodyPartR15Name(bodyPart);
	PartInstance* replacement = Instance::fastDynamicCast<PartInstance>(instance.get());
	Instance* character = getParent();
	if (!getUseR15() || !partName || !replacement || !character ||
		replacement->getName() != partName)
		return false;

	PartInstance* previous = Instance::fastDynamicCast<PartInstance>(
		character->findFirstChildByName(partName));
	if (!previous)
		return false;
	...
	std::vector<shared_ptr<Motor6D> > motors;
	collectCharacterMotors(*character, motors);
	const CoordinateFrame previousFrame = previous->getCoordinateFrame();
	replacement->setCoordinateFrame(previousFrame);
	replacement->setParent(character);

	for (const shared_ptr<Motor6D>& motor : motors)
	{
		if (motor->getParent() == previous)
			motor->setParent(replacement);
		if (motor->getPart0() == previous)
		{
			motor->setPart0(replacement);
			if (Attachment* attachment = Instance::fastDynamicCast<Attachment>(
				replacement->findFirstChildByName(motor->getName() + "RigAttachment")))
				motor->setC0(attachment->getFrameInPart());
		}
		if (motor->getPart1() == previous)
		{
			motor->setPart1(replacement);
			if (Attachment* attachment = Instance::fastDynamicCast<Attachment>(
				replacement->findFirstChildByName(motor->getName() + "RigAttachment")))
				motor->setC1(attachment->getFrameInPart());
		}
	}
...
static void scaleAvatarFrames(Instance& instance, const Vector3& scale)
{
	if (Attachment* attachment = Instance::fastDynamicCast<Attachment>(&instance))
	{
		Vector3 original = attachment->getFrameInPart().translation;
		if (const Vector3Value* saved = Instance::fastDynamicCast<const Vector3Value>(
			attachment->findFirstChildByName("OriginalPosition")))
			original = saved->getValue();
		CoordinateFrame frame = attachment->getFrameInPart();
		frame.translation = original * scale;
		attachment->setFrameInPart(frame);
	}
	for (std::size_t index = 0; index < instance.numChildren(); ++index)
		scaleAvatarFrames(*instance.getChild(index), scale);
}

static void refreshAvatarConstraints(Instance& instance)
{
	if (AnimationConstraint* constraint =
		Instance::fastDynamicCast<AnimationConstraint>(&instance))
		constraint->refreshJoint();
	else if (BallSocketConstraint* constraint =
		Instance::fastDynamicCast<BallSocketConstraint>(&instance))
		constraint->refreshJoint();
	for (std::size_t index = 0; index < instance.numChildren(); ++index)
		refreshAvatarConstraints(*instance.getChild(index));
}

void Humanoid::refreshAvatarScale()
{
	if (!automaticScalingEnabled || !getUseR15() || !getParent())
		return;
	const float height = static_cast<float>(getAvatarScaleValue(*this,
		"BodyHeightScale", 1.0));
	const float width = static_cast<float>(getAvatarScaleValue(*this,
	...
	for (std::size_t index = 0; index < getParent()->numChildren(); ++index)
		if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(
			getParent()->getChild(index)))
		{
			const Vector3 scale = avatarScaleForPart(*part,
				width, height, depth, head, bodyType, bodyProportion);
			if (const Vector3Value* original =
				Instance::fastDynamicCast<const Vector3Value>(
					part->findFirstChildByName("OriginalSize")))
				part->setPartSizeXml(original->getValue() * scale);
			if (part->getName() == "HumanoidRootPart")
			{
				// HumanoidRootPart keeps a fixed 2x2x1 body, but its
				// RootRigAttachment moves with the selected body proportions.
				// Its OriginalPosition is the classic (-0.35) location in the
				// supplied rigs, so treating the root's part scale as identity
				// collapses Rthro legs back into the classic skeleton.
				if (Attachment* rootAttachment = Instance::fastDynamicCast<Attachment>(
						part->findFirstChildByName("RootRigAttachment")))
				{
					const float classicOffset = 0.35f;
					const float normalOffset = 0.987f;
					const float slenderOffset = 0.917431f;
					const float rthroOffset = normalOffset +
						(slenderOffset - normalOffset) * bodyProportion;
					CoordinateFrame frame = rootAttachment->getFrameInPart();
					frame.translation.y = -height * (classicOffset +
						(rthroOffset - classicOffset) * bodyType);
					rootAttachment->setFrameInPart(frame);
				}
			}
			else
				scaleAvatarFrames(*part, scale);
		}

	// Rig attachments are the authoritative neutral joint frames in current
	// Studio rigs. Reapply them after scaling so both Motor6D and
	// AnimationConstraint skeletons keep exact endpoint alignment.
	for (std::size_t index = 0; index < getParent()->numChildren(); ++index)
		if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(
			getParent()->getChild(index)))
			for (std::size_t childIndex = 0; childIndex < part->numChildren(); ++childIndex)
				if (Motor6D* motor = Instance::fastDynamicCast<Motor6D>(
					part->getChild(childIndex)))
				{
					const std::string attachmentName = motor->getName() + "RigAttachment";
					Attachment* first = motor->getPart0()
						? Instance::fastDynamicCast<Attachment>(motor->getPart0()->
							findFirstChildByName2(attachmentName, true).get()) : NULL;
					Attachment* second = motor->getPart1()
						? Instance::fastDynamicCast<Attachment>(motor->getPart1()->
							findFirstChildByName2(attachmentName, true).get()) : NULL;
					if (first && second)
					{
						motor->setC0(first->getFrameInPart());
						motor->setC1(second->getFrameInPart());
					}
				}
	refreshAvatarConstraints(*getParent());
}
...
template<>
EnumDesc<RBX::Humanoid::BodyPartR15>::EnumDesc()
	:EnumDescriptor("BodyPartR15")
{
	addPair(RBX::Humanoid::BODY_PART_R15_HEAD, "Head");
	addPair(RBX::Humanoid::BODY_PART_R15_UPPER_TORSO, "UpperTorso");
	addPair(RBX::Humanoid::BODY_PART_R15_LOWER_TORSO, "LowerTorso");
	addPair(RBX::Humanoid::BODY_PART_R15_LEFT_FOOT, "LeftFoot");
	addPair(RBX::Humanoid::BODY_PART_R15_LEFT_LOWER_LEG, "LeftLowerLeg");
	addPair(RBX::Humanoid::BODY_PART_R15_LEFT_UPPER_LEG, "LeftUpperLeg");
	addPair(RBX::Humanoid::BODY_PART_R15_RIGHT_FOOT, "RightFoot");
	addPair(RBX::Humanoid::BODY_PART_R15_RIGHT_LOWER_LEG, "RightLowerLeg");
	addPair(RBX::Humanoid::BODY_PART_R15_RIGHT_UPPER_LEG, "RightUpperLeg");
	addPair(RBX::Humanoid::BODY_PART_R15_LEFT_HAND, "LeftHand");
	addPair(RBX::Humanoid::BODY_PART_R15_LEFT_LOWER_ARM, "LeftLowerArm");
	addPair(RBX::Humanoid::BODY_PART_R15_LEFT_UPPER_ARM, "LeftUpperArm");
	addPair(RBX::Humanoid::BODY_PART_R15_RIGHT_HAND, "RightHand");
	addPair(RBX::Humanoid::BODY_PART_R15_RIGHT_LOWER_ARM, "RightLowerArm");
	addPair(RBX::Humanoid::BODY_PART_R15_RIGHT_UPPER_ARM, "RightUpperArm");
	addPair(RBX::Humanoid::BODY_PART_R15_ROOT_PART, "RootPart");
	addPair(RBX::Humanoid::BODY_PART_R15_UNKNOWN, "Unknown");
}

template<>
RBX::Humanoid::BodyPartR15& Variant::convert<RBX::Humanoid::BodyPartR15>(void)
{
	return genericConvert<RBX::Humanoid::BodyPartR15>();
}
...
template<>
bool RBX::StringConverter<RBX::Humanoid::BodyPartR15>::convertToValue(
	const std::string& text, RBX::Humanoid::BodyPartR15& value)
{
	return RBX::Reflection::EnumDesc<RBX::Humanoid::BodyPartR15>::singleton()
...
void Humanoid::playEmoteAsync(std::string emoteName,
	boost::function<void(bool)> resumeFunction,
	boost::function<void(std::string)> errorFunction)
{
	Instance* character = getParent();
	Instance* animate = character
		? character->findFirstChildByName("Animate") : NULL;
	BindableFunction* playEmote = animate
		? Instance::fastDynamicCast<BindableFunction>(
			animate->findFirstChildByName("PlayEmote")) : NULL;
	if (!playEmote)
	{
		errorFunction("Humanoid:PlayEmoteAsync requires the character Animate.PlayEmote function");
		return;
	}

	shared_ptr<Reflection::Tuple> arguments(new Reflection::Tuple(1));
	arguments->values[0] = emoteName;
	weak_ptr<Humanoid> weakHumanoid = weak_from(this);
	playEmote->invoke(arguments,
		[weakHumanoid, resumeFunction](shared_ptr<const Reflection::Tuple> result)
		{
			bool played = result && !result->values.empty() &&
				result->values[0].isType<bool>() &&
				result->values[0].cast<bool>();
			shared_ptr<Instance> track;
			if (result && result->values.size() > 1 &&
				result->values[1].isType<shared_ptr<Instance> >())
				track = result->values[1].cast<shared_ptr<Instance> >();
			if (shared_ptr<Humanoid> humanoid = weakHumanoid.lock())
				humanoid->emoteTriggeredSignal(played, track);
			resumeFunction(played);
		},
		errorFunction);
}
