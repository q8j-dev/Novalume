...
DYNAMIC_FASTFLAGVARIABLE(EnableHipHeight, true)
...
{
	// Current R15 HipHeight is measured from the bottom of HumanoidRootPart
	// to the floor.  The controller works with the root center, so add only
	// half of that root part here.  The pre-R15 calculation below derives the
	// height from torso and leg geometry and remains the R6 compatibility path.
	if (DFFlag::EnableHipHeight && humanoid->getUseR15())
	{
		if (PartInstance* rootPart = humanoid->getRootPartDangerous())
			return 0.5f * rootPart->getPartSizeXml().y + humanoid->getHipHeight();
	}
