...
template<>
bool StringConverter<DateTime>::convertToValue(
    const std::string& text, DateTime& value)
{
    long long milliseconds;
    if (!StringConverter<long long>::convertToValue(text, milliseconds))
        return false;
...
template<>
void RBX::Reflection::TypedPropertyDescriptor<long long>::readValue(DescribedBase* instance, const XmlElement* element, IReferenceBinder& binder) const
{
	if (!element->isXsiNil())
	{
		std::string text;
		long long value;
		if (element->getValue(text) && StringConverter<long long>::convertToValue(text, value))
			setValue(instance, value);
	}
}
template<>
void RBX::Reflection::TypedPropertyDescriptor<long long>::writeValue(const DescribedBase* instance, XmlElement* element) const
{
	element->setValue(StringConverter<long long>::convertToString(getValue(instance)));
}

template<>
void RBX::Reflection::TypedPropertyDescriptor<DateTime>::readValue(
    DescribedBase* instance, const XmlElement* element, IReferenceBinder&) const
{
    if (!element->isXsiNil())
    {
        ...
        if (element->getValue(text) &&
            StringConverter<long long>::convertToValue(text, milliseconds))
            setValue(instance, DateTime(milliseconds));
    }
}

template<>
void RBX::Reflection::TypedPropertyDescriptor<DateTime>::writeValue(
    const DescribedBase* instance, XmlElement* element) const
{
    element->setValue(StringConverter<long long>::convertToString(
        getValue(instance).getUnixTimestampMillis()));
}

template<>
int TypedPropertyDescriptor<DateTime>::getDataSize(const DescribedBase*) const
{
    return sizeof(DateTime);
}

template<>
bool TypedPropertyDescriptor<DateTime>::hasStringValue() const
{
    return true;
}

template<>
std::string TypedPropertyDescriptor<DateTime>::getStringValue(
    const DescribedBase* instance) const
{
    return StringConverter<long long>::convertToString(
        getValue(instance).getUnixTimestampMillis());
}

template<>
bool TypedPropertyDescriptor<DateTime>::setStringValue(
...
template<>
const Type& Type::getSingleton<DateTime>()
{
    static TType<DateTime> type("DateTime");
    return type;
}

template<>
DateTime& Variant::convert<DateTime>()
{
    return genericConvert<DateTime>();
}

template<>
long long& RBX::Reflection::Variant::convert<long long>(void)
{
	// Luau numbers arrive through reflection as doubles. Preserve the full
	// integral API surface used by current services while retaining the
	// existing Variant conversion behavior for already-integral values.
	if (_type->isType<double>())
	{
		value = static_cast<long long>(cast<double>());
		_type = &Type::singleton<long long>();
	}
	else if (_type->isType<float>())
	{
		value = static_cast<long long>(cast<float>());
		_type = &Type::singleton<long long>();
	}
	else if (_type->isType<int>())
	{
		value = static_cast<long long>(cast<int>());
		_type = &Type::singleton<long long>();
	}
	else if (_type->isType<long>())
	{
		value = static_cast<long long>(cast<long>());
		_type = &Type::singleton<long long>();
	}
	else if (_type->isType<bool>())
	{
		value = cast<bool>() ? 1LL : 0LL;
		_type = &Type::singleton<long long>();
	}
...
{
	// Roblox XML serializes ContentId-backed properties with the historical
	// <Content> tag.  This reflected name is part of the RBXM/RBXL wire format;
	// changing it silently leaves textures, meshes, sounds, and other content
	// properties at their defaults when genuine models are loaded.
...
template<>
Content& RBX::Reflection::Variant::convert<Content>(void)
{
	if (_type->isType<std::string>())
	{
		value = Content::fromUri(cast<std::string>());
		_type = &Type::singleton<Content>();
	}
	return genericConvert<Content>();
}

template<>
Font& RBX::Reflection::Variant::convert<Font>(void)
{
	return genericConvert<Font>();
}
...
template<>
void RBX::Reflection::TypedPropertyDescriptor<RBX::Content>::readValue(
    DescribedBase* instance, const XmlElement* element, IReferenceBinder&) const
{
    if (!element->isXsiNil())
    {
        ContentId value;
        if (element->getValue(value))
            setValue(instance, Content::fromUri(value.toString()));
    }
}

template<>
void RBX::Reflection::TypedPropertyDescriptor<RBX::Content>::writeValue(
    const DescribedBase* instance, XmlElement* element) const
{
    const Content& value = getValue(instance);
    if (value.getSourceType() == CONTENT_SOURCE_URI)
        element->setValue(ContentId(value.getUri()));
    else
        element->addAttribute(name_xsinil, true);
}

template<>
int RBX::Reflection::TypedPropertyDescriptor<RBX::Content>::getDataSize(
    const DescribedBase* instance) const
{
    const Content& value = getValue(instance);
    return static_cast<int>(sizeof(Content) + value.getUri().size());
}

template<>
bool RBX::Reflection::TypedPropertyDescriptor<RBX::Content>::hasStringValue() const
{
    return true;
}

template<>
std::string RBX::Reflection::TypedPropertyDescriptor<RBX::Content>::getStringValue(
    const DescribedBase* instance) const
{
    const Content& value = getValue(instance);
    return value.getSourceType() == CONTENT_SOURCE_URI ? value.getUri() : std::string();
}

template<>
bool RBX::Reflection::TypedPropertyDescriptor<RBX::Content>::setStringValue(
    DescribedBase* instance, const std::string& text) const
{
    setValue(instance, Content::fromUri(text));
    return true;
}
    ...
    DescribedBase* instance, const XmlElement* element, IReferenceBinder&) const
{
    ...
    if (familyElement && familyElement->getValue(family) && !family.toString().empty())
    {
        ...
        if (styleElement)
            styleElement->getValue(styleName);
        FontStyle style = FONT_STYLE_NORMAL;
        Reflection::EnumDesc<FontStyle>::singleton().convertToValue(styleName.c_str(), style);
        setValue(instance, Font(family.toString(), static_cast<FontWeight>(weight), style));
    }
}
...
template<>
int RBX::Reflection::TypedPropertyDescriptor<RBX::Font>::getDataSize(
    const DescribedBase* instance) const
{
    return static_cast<int>(sizeof(Font) + getValue(instance).getFamily().size());
}

template<>
bool RBX::Reflection::TypedPropertyDescriptor<RBX::Font>::hasStringValue() const
{
    return false;
}

template<>
std::string RBX::Reflection::TypedPropertyDescriptor<RBX::Font>::getStringValue(
    const DescribedBase* instance) const
{
    return Super::getStringValue(instance);
}
...
template<>
bool RBX::Reflection::TypedPropertyDescriptor<RBX::Font>::setStringValue(
    DescribedBase* instance, const std::string& text) const
{
    return Super::setStringValue(instance, text);
}
