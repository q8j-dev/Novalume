...
template<>
bool StringConverter<RBX::Content>::convertToValue(const std::string& text, RBX::Content& value)
{
	value = Content::fromUri(text);
	return true;
}

template<>
std::string StringConverter<RBX::Content>::convertToString(const RBX::Content& value)
{
	return value.getSourceType() == CONTENT_SOURCE_URI ? value.getUri() : std::string();
}

template<>
bool StringConverter<RBX::Font>::convertToValue(const std::string& text, RBX::Font& value)
{
	if (text.empty())
		return false;
	value = Font(text);
...
template<>
std::string StringConverter<RBX::Font>::convertToString(const RBX::Font& value)
{
