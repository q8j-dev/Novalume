...
public:
	...
	rbx::signal<void()>* getOrCreateDestroyingSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->destroyingSignal : NULL; };
	...
	static const Reflection::EventDescriptor* getAttributeChangedEventDescriptor();
	int getPropertyChangedSignal(lua_State* state);
	static const Reflection::EventDescriptor* getPropertyChangedEventDescriptor();
	int addTagLua(lua_State* state);
	int removeTagLua(lua_State* state);
	int hasTagLua(lua_State* state);
	int getTagsLua(lua_State* state);
	bool addTagInternal(const std::string& tag);
	bool removeTagInternal(const std::string& tag);
	bool hasTagInternal(const std::string& tag) const;
	const std::set<std::string>& getTagsInternal() const { return tags; }
	bool isPropertyStyleManaged(const std::string& propertyName) const;
	void markPropertyStyleManaged(const std::string& propertyName);
	void clearPropertyStyleManaged(const std::string& propertyName);
	rbx::signal<void(std::string)>* getOrCreateAttributeChangedSignal(bool create = true)
	{
		return (onDemandRead() || create) ? &onDemandWrite()->attributeChangedSignal : NULL;
	}
...
public:
	...
	shared_ptr<Instance> findFirstAncestor(std::string name);
	shared_ptr<Instance> findFirstAncestorOfClass(std::string className);
	shared_ptr<Instance> findFirstAncestorWhichIsA(std::string className);
