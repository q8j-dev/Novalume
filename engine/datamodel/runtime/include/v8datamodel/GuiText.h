...
public:
		GuiTextMixin(const std::string& text, const Color3& textColor)
			...
			, textDirection(Enums::TEXT_DIRECTION_AUTO)
			, textTruncate(Enums::TEXT_TRUNCATE_NONE)
			...
			, fontFace(Font::fromEnum(TextService::FONT_LEGACY))
	...
	int getMaxVisibleGraphemes() const { return maxVisibleGraphemes; }
	Enums::TextDirection getTextDirection() const { return textDirection; }
	std::string getContentText() const;

	TextService::Font getFont() const { return font; }
	const Font& getFontFace() const { return fontFace; }
	...
	void applyFontFace(const Font& value)
	{
		...
		if (family.find("BuilderIcons-Filled") != std::string::npos)
			...
			font = TextService::FONT_BUILDER_ICONS_REGULAR;
		else if (family.find("BuilderSans") != std::string::npos)
		{
			if (value.getWeight() >= FONT_WEIGHT_EXTRA_BOLD)
				font = TextService::FONT_BUILDERSANS_EXTRABOLD;
			else if (value.getWeight() >= FONT_WEIGHT_BOLD)
				font = TextService::FONT_BUILDERSANS_BOLD;
			else if (value.getWeight() >= FONT_WEIGHT_MEDIUM)
				font = TextService::FONT_BUILDERSANS_MEDIUM;
			else
			...
			font = TextService::FONT_SOURCESANSLIGHT;
		else
			font = TextService::FONT_SOURCESANS;
	}
...
protected:
		...
		Enums::TextDirection textDirection;
		Enums::TextTruncate textTruncate;
...
void setMaxVisibleGraphemes(int value);\
void setTextDirection(Enums::TextDirection value);\
void setFont(TextService::Font value);\
void setFontFace(Font value) { if (getFontFace() != value) { applyFontFace(value); checkForResize(); } }\
...
void setTextTruncate(Enums::TextTruncate value);\
...
static const Reflection::PropDescriptor<Class, float> prop_TextSize("TextSize", category_Text, &Class::getTextSize, &Class::setTextSize);\
static const Reflection::PropDescriptor<Class, bool> prop_RichText("RichText", category_Text, &Class::getRichText, &Class::setRichText);\
static const Reflection::PropDescriptor<Class, std::string> prop_ContentText("ContentText", category_Text, &Class::getContentText, NULL, Reflection::PropertyDescriptor::UI);\
static const Reflection::PropDescriptor<Class, float> prop_LineHeight("LineHeight", category_Text, &Class::getLineHeight, &Class::setLineHeight);\
static const Reflection::PropDescriptor<Class, int> prop_MaxVisibleGraphemes("MaxVisibleGraphemes", category_Text, &Class::getMaxVisibleGraphemes, &Class::setMaxVisibleGraphemes);\
static const Reflection::EnumPropDescriptor<Class, Enums::TextDirection> prop_TextDirection("TextDirection", category_Text, &Class::getTextDirection, &Class::setTextDirection);\
...
static const Reflection::PropDescriptor<Class, Font> prop_FontFace("FontFace", category_Text, &Class::getFontFace, &Class::setFontFace);\
...
static const Reflection::EnumPropDescriptor<Class, Enums::TextTruncate> prop_TextTruncate("TextTruncate", category_Text, &Class::getTextTruncate, &Class::setTextTruncate);\
...
{\
	...
	if(!ProfanityFilter::ContainsProfanity(value) || getRobloxLocked()){\
		if(GuiTextMixin::text != value){\
			...
			raisePropertyChanged(prop_ContentText);\
			raisePropertyChanged(prop_TextBounds);\
...
{\
	if (GuiTextMixin::fontSize != value) {\
		...
		GuiTextMixin::textSize = GuiObject::convertFontSize(value);\
...
{\
	value = G3D::clamp(value, 1.0f, 100.0f);\
...
\
void Class::setRichText(bool value)\
{\
	if (GuiTextMixin::richText != value) { GuiTextMixin::richText = value; raisePropertyChanged(prop_RichText); raisePropertyChanged(prop_ContentText); raisePropertyChanged(prop_TextBounds); }\
}\
void Class::setLineHeight(float value)\
{\
	value = G3D::clamp(value, 0.5f, 3.0f); if (GuiTextMixin::lineHeight != value) { GuiTextMixin::lineHeight = value; raisePropertyChanged(prop_LineHeight); raisePropertyChanged(prop_TextBounds); }\
}\
void Class::setMaxVisibleGraphemes(int value)\
{\
	if (GuiTextMixin::maxVisibleGraphemes != value) { GuiTextMixin::maxVisibleGraphemes = value; raisePropertyChanged(prop_MaxVisibleGraphemes); }\
}\
void Class::setTextDirection(Enums::TextDirection value)\
{\
	if (GuiTextMixin::textDirection != value) { GuiTextMixin::textDirection = value; raisePropertyChanged(prop_TextDirection); }\
}\
\
...
\
void Class::setTextTruncate(Enums::TextTruncate value)\
{\
	if (GuiTextMixin::textTruncate != value) {\
		GuiTextMixin::textTruncate = value;\
		raisePropertyChanged(prop_TextTruncate);\
		raisePropertyChanged(prop_TextBounds);\
		raisePropertyChanged(prop_TextFits);\
	}\
}\
	...
	if(Network::Players::frontendProcessing(this, false))\
		...
		{\
			if(Typesetter* typesetter = textService->getTypesetter(GuiTextMixin::font))\
				return typesetter->measure(	getContentText(),\
											GuiTextMixin::textSize,\
...
{\
	if(Network::Players::frontendProcessing(this, false))\
		...
		{\
			if(Typesetter* typesetter = textService->getTypesetter(GuiTextMixin::font)){\
				bool result = false;\
				Vector2 bounds = typesetter->measure(	getContentText(),\
												GuiTextMixin::textSize,\
														GuiTextMixin::textWrap ? getRect2D().wh() : Vector2::zero(),\
														&result);\
