...
namespace RBX
{
	...
	public:
		...
		void setAutomaticSize(AutomaticSize value);
		AutomaticSize getAutomaticSize() const { return automaticSize; }
		...
		void setSelectionOrder(int value);
		int getSelectionOrder() const { return selectionOrder; }
		...
		GuiState getReflectedGuiState() const;
		...
		Color4 applyCanvasGroup(const Color4& color) const;
		...
		void invalidateAutomaticSizeParent();
		Vector2 getAutomaticContentSize(const Vector2& availableSize) const;
	...
	protected:
		...
		float getFontSizeScale(bool _textScale, bool _textWrap, float fontSize, const Rect2D& rect);
		...
		void render2dImpl(	Adorn* adorn, 
							const Color4& _backgroundColor,
							Rect2D& rect);
		void render2dGradientTexture(Adorn* adorn, const Rect2D& rect,
			const Vector2& texul, const Vector2& texbr, const Color4& color,
			const Rect2D* clipRect = NULL);
	...
	private:
		...
		bool hasActivated;
		int activationClickCount;
		InputObject::UserInputType lastActivationInputType;
		Vector2 lastActivationPosition;
	...
	public:
		...
		rbx::signal<void(shared_ptr<InputObject>, int)> activatedSignal;
