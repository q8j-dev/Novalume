...
namespace RBX {
	...
	public:
		...
		static Reflection::EnumPropDescriptor<GuiService, Enums::PreferredTextSize> prop_preferredTextSize;
	...
	private:
		...
		Vector4 hardwareSafeAreaInsets;
		Vector4 safeZoneOffsets;
		Rect2D topbarInset;
        
        rbx::signals::scoped_connection screenGuiConnection;
		rbx::signals::scoped_connection preferredTextSizeConnection;
		rbx::signals::scoped_connection preferredTransparencyConnection;
	...
	public:
		...
		int getBrickCount();
		int getResolutionScale() { return resolutionScale; }
		void setResolutionScale(int value);
		shared_ptr<const Reflection::ValueTable> getNotificationTypeList();
		...
		Enums::PreferredTextSize getPreferredTextSize() const;
		float getPreferredTransparency() const;
		Enums::DisplaySize getViewportDisplaySize() const;
		...
		rbx::signal<void()> nativeCloseSignal;
		void fireNativeCloseSignal() { nativeCloseSignal(); }
		...
		bool getTouchControlsEnabled() const { return touchControlsEnabled; }
		void setTouchControlsEnabled(bool value);
		bool getInspectMenuEnabled() { return inspectMenuEnabled; }
		void setInspectMenuEnabled(bool enabled);
		rbx::signal<void(bool)> inspectMenuEnabledChangedSignal;
		rbx::signal<void(shared_ptr<Instance>, std::string)>
			inspectPlayerFromHumanoidDescriptionRequestSignal;
		rbx::signal<void(long long, std::string)> inspectPlayerFromUserIdWithCtxRequestSignal;
		rbx::signal<void()> closeInspectMenuRequestSignal;
		rbx::signal<void(bool)> networkPausedEnabledChangedSignal;
		void inspectPlayerFromHumanoidDescription(
			shared_ptr<Instance> humanoidDescription, std::string name)
		{
			inspectPlayerFromHumanoidDescriptionRequestSignal(humanoidDescription, name);
		}
		void inspectPlayerFromUserId(long long userId, std::string context)
		{
			inspectPlayerFromUserIdWithCtxRequestSignal(userId, context);
		}
		void inspectPlayerFromUserId(long long userId)
		{
			inspectPlayerFromUserIdWithCtxRequestSignal(userId, std::string());
		}
		void closeInspectMenu() { closeInspectMenuRequestSignal(); }
		...
		shared_ptr<const Reflection::ValueTable> getSafeZoneOffsets();
		void setSafeZoneOffsets(float top, float bottom, float left, float right);
		rbx::signal<void()> safeZoneOffsetsChangedSignal;
