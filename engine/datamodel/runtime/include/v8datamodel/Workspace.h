...
class Workspace :  
	public DescribedNonCreatable<Workspace, WorldRoot, sWorkspace>,
	public Service,
...
private:
	...
	typedef DescribedNonCreatable<Workspace, WorldRoot, sWorkspace> Super;
public:
...
private:
	...
	mutable std::mutex serverTimeMutex;
	std::chrono::steady_clock::time_point serverTimeReferenceMonotonic;
	double serverTimeReferenceUnix;
...
public:
	...
	shared_ptr<const Reflection::Tuple> getRayHitWhitelist(RbxRay ray,
		shared_ptr<const Instances> whitelist, bool terrainCellsAreCubes = false,
		bool ignoreWaterCells = false);

	Reflection::Variant raycast(G3D::Vector3 origin, G3D::Vector3 direction,
		RaycastParams params = RaycastParams());
	shared_ptr<const Instances> getPartBoundsInBox(G3D::CoordinateFrame frame,
...
public:
	...
	void synchronizeServerTime(double serverUnixSeconds);
	...
	void setCollisionGroupData(const BinaryString& value);
