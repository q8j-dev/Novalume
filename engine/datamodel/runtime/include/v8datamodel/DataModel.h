...
public:
    ...
    enum AvatarRigVariant
    {
        AVATAR_RIG_R6 = 0,
        AVATAR_RIG_R15 = 1,
        AVATAR_RIG_RTHRO_NORMAL = 2,
        AVATAR_RIG_RTHRO_SLENDER = 3,
		AVATAR_RIG_R15_PLUS = 4,
    };
	...
	rbx::signal<void(std::string)> screenshotReadySignal;
...
private:			
    ...
    bool                                forceR15;
    AvatarRigVariant                    avatarRigVariant;
...
public:
	...
	long long getGameId() const { return static_cast<long long>(universeId); }
    ...
    AvatarRigVariant getAvatarRigVariant() const { return avatarRigVariant; }
    void setAvatarRigVariant(AvatarRigVariant value);
	...
	bool defineFastFlag(std::string name, bool defaultValue);
	bool getFastFlag(std::string name);
	bool getEngineFeature(std::string name);
	void setFlagVersion(std::string name, int version);
	int defineFastInt(std::string name, int defaultValue);
	int getFastInt(std::string name);
	std::string defineFastString(std::string name, std::string defaultValue);
	std::string getFastString(std::string name);
