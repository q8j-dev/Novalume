...
namespace RBX {
	...
	public:
		...
		static void addAssetOverlay(const char* path, const char* name, int priority);
		...
		static std::string findAsset(RBX::ContentId contentId, float pixelDensity);
		// Resolves local content emitted by trusted engine services.  File URLs
		// remain sandboxed to the player's capture gallery; arbitrary filesystem
		// paths supplied by experience scripts are never exposed to renderers.
		static std::string resolveLocalContent(RBX::ContentId contentId);
