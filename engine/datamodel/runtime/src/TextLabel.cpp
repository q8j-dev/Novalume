...
void TextLabel::render2dContext(Adorn* adorn, const Instance* context)
{
	...
	if(filterState == ContentFilter::Succeeded)
	{
        if (getRenderTextAlpha(getTextTransparency()) > 0.0f) {
		    render2dTextImpl(adorn, getRenderBackgroundColor4(), localizeText(getContentText()), 
			    getFont(), getTextSize(), applyCanvasGroup(getRenderTextColor4()), applyCanvasGroup(getRenderTextStrokeColor4()), getTextWrap(), getTextScale(),
