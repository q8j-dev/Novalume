...
namespace RBX {
	...
	static const Reflection::PropDescriptor<GuiLayerCollector, bool> propEnabled(
		"Enabled", category_Data, &GuiLayerCollector::getEnabled,
		&GuiLayerCollector::setEnabled);
	static const Reflection::PropDescriptor<GuiLayerCollector, bool> propResetOnSpawn(
		"ResetOnSpawn", category_Data, &GuiLayerCollector::getResetOnSpawn,
		&GuiLayerCollector::setResetOnSpawn);
	static const Reflection::EnumPropDescriptor<GuiLayerCollector, Enums::ZIndexBehavior> propZIndexBehavior(
		"ZIndexBehavior", category_Data, &GuiLayerCollector::getZIndexBehavior,
		&GuiLayerCollector::setZIndexBehavior);

	namespace Reflection {
	template<>
	EnumDesc<Enums::ZIndexBehavior>::EnumDesc()
		: EnumDescriptor("ZIndexBehavior")
	{
		addPair(Enums::ZINDEX_BEHAVIOR_GLOBAL, "Global");
		addPair(Enums::ZINDEX_BEHAVIOR_SIBLING, "Sibling");
	}
	}
	...
	GuiLayerCollector::GuiLayerCollector(const char* name)
		...
		, enabled(true)
		, resetOnSpawn(true)
		, zIndexBehavior(Enums::ZINDEX_BEHAVIOR_SIBLING)
	{
	...
	void GuiLayerCollector::setEnabled(bool value)
	{
		if (enabled == value)
			return;
		enabled = value;
		raisePropertyChanged(propEnabled);
	...
	void GuiLayerCollector::setResetOnSpawn(bool value)
	{
		if (resetOnSpawn == value)
			return;
		resetOnSpawn = value;
		raisePropertyChanged(propResetOnSpawn);
	}

	void GuiLayerCollector::setZIndexBehavior(Enums::ZIndexBehavior value)
	{
		if (zIndexBehavior == value)
			return;
		zIndexBehavior = value;
		rebuildGuiVector = true;
		raisePropertyChanged(propZIndexBehavior);
	}
	...
	void GuiLayerCollector::LoadSiblingZ(const shared_ptr<RBX::Instance>& instance, GuiVector& guiVector)
	{
		if (!instance->getChildren())
			return;

		std::vector<shared_ptr<GuiBase> > siblings;
		const Instances& children = *instance->getChildren();
		for (size_t i = 0; i < children.size(); ++i)
		{
			if (GuiBase* gui = children[i]->fastDynamicCast<GuiBase>())
			{
				if (gui->canProcessMeAndDescendants())
					siblings.push_back(shared_static_cast<GuiBase>(children[i]));
			}
			else if (children[i]->fastDynamicCast<Folder>())
			{
				LoadSiblingZ(children[i], guiVector);
			}
		}

		std::stable_sort(siblings.begin(), siblings.end(), [](const shared_ptr<GuiBase>& left, const shared_ptr<GuiBase>& right) {
			return left->getZIndex() < right->getZIndex();
		});
		for (size_t i = 0; i < siblings.size(); ++i)
		{
			guiVector.push_back(siblings[i]);
			LoadSiblingZ(siblings[i], guiVector);
		}
	}
	...
	void GuiLayerCollector::loadZVectors()
	{
		...
		if (zIndexBehavior == Enums::ZINDEX_BEHAVIOR_SIBLING)
		{
			LoadSiblingZ(shared_from(this), mSiblingGuiVector);
		}
		else if (getChildren())
        {
	...
	void GuiLayerCollector::render2dContext(Adorn* adorn, const Instance* context)
	{
		...
		if (zIndexBehavior == Enums::ZINDEX_BEHAVIOR_SIBLING)
		{
			for (size_t i = 0; i < mSiblingGuiVector.size(); ++i)
			{
				const shared_ptr<GuiBase>& item = mSiblingGuiVector[i];
				if (!item || !item->isVisible(viewport))
					continue;
				if (item->getGuiQueue() == GUIQUEUE_GENERAL)
					item->renderBackground2dContext(adorn, context);
				item->render2dContext(adorn, context);
			}
			return;
		}
    ...
    GuiResponse GuiLayerCollector::processGesture(const UserInputService::Gesture& gesture, const shared_ptr<const RBX::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args)
    {
		...
		if (zIndexBehavior == Enums::ZINDEX_BEHAVIOR_SIBLING)
		{
			for (GuiVector::reverse_iterator itr = mSiblingGuiVector.rbegin(); itr != mSiblingGuiVector.rend(); ++itr)
			{
	...
	void GuiLayerCollector::getGuiObjectsForSelection(std::vector<GuiObject*>& guiObjects)
	{
		...
		if (zIndexBehavior == Enums::ZINDEX_BEHAVIOR_SIBLING)
		{
			for (GuiVector::reverse_iterator itr = mSiblingGuiVector.rbegin(); itr != mSiblingGuiVector.rend(); ++itr)
			{
				...
				{
					ScrollingFrame* ancestor = guiObject->findFirstAncestorOfType<ScrollingFrame>();
					if (guiObject->getSelectable() && (guiObject->isCurrentlyVisible() || (ancestor && ancestor->isCurrentlyVisible())))
	...
	GuiResponse GuiLayerCollector::processDescendants(const shared_ptr<InputObject>& event)
	{
		...
		if (zIndexBehavior == Enums::ZINDEX_BEHAVIOR_SIBLING)
		{
			for (GuiVector::reverse_iterator itr = mSiblingGuiVector.rbegin(); itr != mSiblingGuiVector.rend(); ++itr)
			{
				...
				if (answer.getMouseWasOver() && !mouseWasOver)
				{
					mouseWasOver = true;
					mouseOverGuiBase = answer.getTarget() ? answer.getTarget().get() : (*itr).get();
				}
			}
			tryReleaseLastButtonDown(event);
