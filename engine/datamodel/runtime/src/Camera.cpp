...
static Reflection::PropDescriptor<Camera, float> desc_NearPlaneZ("NearPlaneZ", category_Data, &Camera::nearPlaneZ, NULL, Reflection::PropertyDescriptor::SCRIPTING);
...
static Reflection::PropDescriptor<Camera, float> desc_HeadScale("HeadScale", category_Data, &Camera::getHeadScale, &Camera::setHeadScale);
	...
	:EnumDescriptor("CameraType")
{
	...
	addPair(Camera::ORBITAL_CAMERA, "Orbital");
}
...
void Camera::setHeadScale(float value)
{
	if (!std::isfinite(value) || value <= 0.0f)
		throw std::runtime_error("Camera.HeadScale must be a positive finite number");
	if (headScale != value)
	{
		headScale = value;
		raisePropertyChanged(desc_HeadScale);
	}
}
...
{
	if (newSubject != cameraSubject.get() &&
		(newSubject == NULL || dynamic_cast<CameraSubject*>(newSubject)))
	{
		if(ICharacterSubject* charSubject = dynamic_cast<ICharacterSubject*>(cameraSubject.get()))
			charSubject->tellCameraSubjectDidChange(cameraSubject,
				newSubject ? shared_from(newSubject) : shared_ptr<Instance>());

		cameraSubject = newSubject ? shared_from(newSubject) : shared_ptr<Instance>();
		...
		shared_ptr<RBX::Instance> potentialHuman = cameraSubject
			? shared_from(cameraSubject->findFirstChildByName("Humanoid"))
			: shared_ptr<RBX::Instance>();
