...
static Reflection::BoundFuncDesc<ContextActionService, void(std::string, Lua::WeakFunctionRef, bool, int, shared_ptr<const Reflection::Tuple>)> func_bindCoreAtPriority(&ContextActionService::bindCoreActionAtPriority, "BindCoreActionAtPriority", "actionName", "functionToBind", "createTouchButton", "priorityLevel", "inputTypes", Security::RobloxScript);
static Reflection::BoundFuncDesc<ContextActionService, void(std::string, Lua::WeakFunctionRef, bool, int, shared_ptr<const Reflection::Tuple>)> func_bindAtPriority(&ContextActionService::bindActionAtPriority, "BindActionAtPriority", "actionName", "functionToBind", "createTouchButton", "priorityLevel", "inputTypes", Security::None);
...
template<> EnumDesc<ContextActionService::Priority>::EnumDesc()
	: EnumDescriptor("ContextActionPriority")
{
	addPair(ContextActionService::PRIORITY_LOW, "Low");
	addPair(ContextActionService::PRIORITY_MEDIUM, "Medium");
	addLegacyName("Default", ContextActionService::PRIORITY_MEDIUM);
	addPair(ContextActionService::PRIORITY_HIGH, "High");
}
}
...
Enums::ContextActionResult ContextActionService::callFunction(
	boost::function<Enums::ContextActionResult(shared_ptr<Reflection::Tuple>)> luaFunction,
	const std::string actionName, const InputObject::UserInputState state,
	const shared_ptr<Instance> inputObject)
{
	...
	return Enums::CONTEXT_ACTION_PASS;
}
			...
			if ( tupleContainsInputObject(iter->second.inputTypes, inputObject, sink) )
			{
				...
				if (!iter->second.luaFunction.empty())
				{
					if ( shared_ptr<Instance> instance = shared_from(Instance::fastDynamicCast<Instance>(inputObject.get())) )
					{
						...
						const Enums::ContextActionResult result = callFunction(
							iter->second.luaFunction, iter->first,
							menuIsOpen ? InputObject::INPUT_STATE_CANCEL : inputObject->getUserInputState(),
							instance);
						if (result == Enums::CONTEXT_ACTION_PASS)
							continue;
					}
...
static Enums::ContextActionResult InvokeCallback(weak_ptr<ContextActionService> weakActionService, Lua::WeakFunctionRef callback, shared_ptr<Reflection::Tuple> args)
{
    if(shared_ptr<ContextActionService> strongThis = weakActionService.lock())
    {
		if (Lua::ThreadRef threadRef = callback.lock())
		{
			if (ScriptContext* sc = ServiceProvider::create<ScriptContext>(strongThis.get()))
			{
				try
				{
					const Reflection::Tuple results = sc->callInNewThread(callback, *(args.get()));
					if (!results.values.empty() &&
						results.values.front().isType<Enums::ContextActionResult>())
						return results.values.front().cast<Enums::ContextActionResult>();
					return Enums::CONTEXT_ACTION_SINK;
				}
...
void ContextActionService::bindActionInternal(const std::string actionName, Lua::WeakFunctionRef functionToBind, bool createTouchButton, shared_ptr<const Reflection::Tuple> hotkeys, FunctionMap& funcMap, FunctionVector& funcVector, int priorityLevel)
{
	...
	boost::function<Enums::ContextActionResult(shared_ptr<Reflection::Tuple>)> luaFunction =
		boost::bind(&InvokeCallback, shared_from(this), functionToBind, _1);
	...
	FunctionVector::iterator insertion = funcVector.begin();
	while (insertion != funcVector.end() && insertion->second.priorityLevel <= priorityLevel)
		++insertion;
	funcVector.insert(insertion, std::pair<std::string, BoundFunctionData>(actionName,data));
...
void ContextActionService::bindCoreActionAtPriority(const std::string actionName, Lua::WeakFunctionRef functionToBind, bool createTouchButton, int priorityLevel, shared_ptr<const Reflection::Tuple> hotkeys)
{
	bindActionInternal(actionName, functionToBind, createTouchButton, hotkeys, coreFunctionMap, coreFunctionVector, priorityLevel);
}

void ContextActionService::bindActionAtPriority(const std::string actionName, Lua::WeakFunctionRef functionToBind, bool createTouchButton, int priorityLevel, shared_ptr<const Reflection::Tuple> hotkeys)
{
	bindActionInternal(actionName, functionToBind, createTouchButton, hotkeys, functionMap, functionVector, priorityLevel);
}
