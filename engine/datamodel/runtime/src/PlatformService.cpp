...
static bool tryAcquire(std::atomic<bool>& flag)
{
	bool expected = false;
	return flag.compare_exchange_strong(expected, true, std::memory_order_acquire);
}

static void release(std::atomic<bool>& flag)
{
