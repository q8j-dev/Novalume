...
DYNAMIC_FASTFLAGVARIABLE(CloseStatesBeforeChildRemoval, true)
...
FASTINTVARIABLE(NewDevConsoleMaxLogCount, 500)
FASTINTVARIABLE(NewDevConsoleMaxHttpCount, 300)
FASTINTVARIABLE(NewDevConsoleMaxGraphCount, 100)
...
static Reflection::BoundFuncDesc<DataModel, bool(std::string, bool)> funcDefineFastFlag(&DataModel::defineFastFlag, "DefineFastFlag", "name", "defaultValue", Security::RobloxScript);
static Reflection::BoundFuncDesc<DataModel, bool(std::string)> funcGetFastFlag(&DataModel::getFastFlag, "GetFastFlag", "name", Security::RobloxScript);
static Reflection::BoundFuncDesc<DataModel, bool(std::string)> funcGetEngineFeature(&DataModel::getEngineFeature, "GetEngineFeature", "name", Security::LocalUser);
static Reflection::BoundFuncDesc<DataModel, void(std::string, int)> funcSetFlagVersion(&DataModel::setFlagVersion, "SetFlagVersion", "name", "version", Security::RobloxScript);
static Reflection::BoundFuncDesc<DataModel, int(std::string, int)> funcDefineFastInt(&DataModel::defineFastInt, "DefineFastInt", "name", "defaultValue", Security::RobloxScript);
static Reflection::BoundFuncDesc<DataModel, int(std::string)> funcGetFastInt(&DataModel::getFastInt, "GetFastInt", "name", Security::RobloxScript);
static Reflection::BoundFuncDesc<DataModel, std::string(std::string, std::string)> funcDefineFastString(&DataModel::defineFastString, "DefineFastString", "name", "defaultValue", Security::RobloxScript);
static Reflection::BoundFuncDesc<DataModel, std::string(std::string)> funcGetFastString(&DataModel::getFastString, "GetFastString", "name", Security::RobloxScript);
...
static Reflection::PropDescriptor<DataModel, long long> prop_gameId(
	"GameId", category_State, &DataModel::getGameId, NULL,
	Reflection::PropertyDescriptor::Attributes(
		static_cast<Reflection::PropertyDescriptor::Functionality>(17)));
...
static Reflection::EventDesc<DataModel, void(std::string)> event_screenshotReady(
	&DataModel::screenshotReadySignal, "ScreenshotReady", "path", Security::RobloxScript);
...
bool DataModel::defineFastFlag(std::string name, bool defaultValue)
{
	bool value = defaultValue;
	if (!FLog::DefineBoolean(name, defaultValue, value))
		throw runtime_error("FastFlag '%s' has a non-boolean definition", name.c_str());
	return value;
}

bool DataModel::getFastFlag(std::string name)
{
	std::string value;
	bool result = false;
	if (!FLog::GetValue(name.c_str(), value) || !FLog::parseBoolean(value, result))
		throw runtime_error("FastFlag '%s' is not defined", name.c_str());
	return result;
}

bool DataModel::getEngineFeature(std::string name)
{
	return EngineFeatures::isEnabled(name);
}

void DataModel::setFlagVersion(std::string name, int version)
{
	if (name.empty())
		throw runtime_error("Flag name must not be empty");
	if (version < 0)
		throw runtime_error("Flag version must not be negative");

	static RBX::mutex versionMutex;
	static std::map<std::string, int> versions;
	RBX::mutex::scoped_lock lock(versionMutex);
	versions[name] = version;
}

int DataModel::defineFastInt(std::string name, int defaultValue)
{
	int value = defaultValue;
	if (!FLog::DefineInteger(name, defaultValue, value))
		throw runtime_error("FastInt '%s' has a non-integer definition", name.c_str());
	return value;
}

int DataModel::getFastInt(std::string name)
{
	std::string value;
	if (!FLog::GetValue(name.c_str(), value))
		throw runtime_error("FastInt '%s' is not defined", name.c_str());
	int result = 0;
...
std::string DataModel::defineFastString(std::string name, std::string defaultValue)
{
	std::string value;
	if (!FLog::DefineString(name, defaultValue, value))
		throw runtime_error("FastString '%s' has a non-string definition", name.c_str());
	return value;
}

std::string DataModel::getFastString(std::string name)
{
	std::string value;
	if (!FLog::GetValue(name.c_str(), value))
		throw runtime_error("FastString '%s' is not defined", name.c_str());
	return value;
}
...
void DataModel::doCloseDataModel(shared_ptr<DataModel> dataModel)
{
	...
	if (!DataModel::BlockingDataModelShutdown)
	{
...
void DataModel::closeDataModel(shared_ptr<DataModel> dataModel)
{
	...
	// Physics and heartbeat jobs take the DataModel arbiter while stepping.
	// Join them before acquiring the shutdown write lock; joining under that
	// lock deadlocks, while clearing RunService first leaves a removed job free
	// to finish against already-destroyed Workspace synchronization state.
	if (DataModel::BlockingDataModelShutdown)
	{
		shared_ptr<RunService> runService;
		{
			LegacyLock lock(dataModel, DataModelJob::Write);
			if (RunService* service = ServiceProvider::find<RunService>(dataModel.get()))
				runService = shared_from(service);
		}
		if (runService)
			runService->stopTasks(true);
	}
	...
	// Generic jobs synchronize through the DataModel arbiter, so joining them
	// while holding LegacyLock deadlocks. Once content and services are closed,
	// release that lock, remove every marshalled job synchronously, and only then
	// clear the owning references and permit DataModel destruction.
	if (DataModel::BlockingDataModelShutdown)
	{
		for (GenericJobs::iterator iter = dataModel->genericJobs.begin();
			 iter != dataModel->genericJobs.end(); ++iter)
		{
...
#pragma warning(pop)
    ...
    forceR15(false),
    avatarRigVariant(AVATAR_RIG_R6)
{
	if (TaskScheduler::singleton().isCyclicExecutive())
	{
...
void DataModel::initializeContents(bool startHeartbeat)
{
	...
	const char* initializationStage = "workspace roots";
	try
	{
	...
	initializationStage = "core services";
	...
	ServiceProvider::create<AnimationClipProvider>();
	ServiceProvider::create<GuiService>();
	ServiceProvider::create<TextService>();
	ServiceProvider::create<ChatService>();
	ServiceProvider::create<MarketplaceService>();
	ServiceProvider::create<CreatorStoreService>();
	ServiceProvider::create<ExperienceAuthService>();
	...
	initializationStage = "starter services";
	...
	initializationStage = "run service";
	runService = shared_from(ServiceProvider::create<RunService>());

	initializationStage = "sound service";
	ServiceProvider::create<Soundscape::SoundService>();
	
	initializationStage = "workspace commands";
	...
	initializationStage = "additional services";
	...
	{
		throw RBX::runtime_error("DataModel initialization failed during %s: %s",
...
{ 
	setAvatarRigVariant(v ? AVATAR_RIG_R15 : AVATAR_RIG_R6);
}

void DataModel::setAvatarRigVariant(AvatarRigVariant value)
{
    if (value < AVATAR_RIG_R6 || value > AVATAR_RIG_R15_PLUS)
        throw std::invalid_argument("invalid avatar rig variant");

    const bool newForceR15 = value != AVATAR_RIG_R6;
    avatarRigVariant = value;
    if (forceR15 != newForceR15)
    {
        forceR15 = newForceR15;
