...
void BasePlayerGui::setSelectedObject(GuiObject* value)
{
	...
	{
		shared_ptr<GuiObject> newSelectedObject = value ? shared_from(value) : shared_ptr<GuiObject>();
		std::set<GuiBase2d*> changedSelectionGroups;
		for (Instance* node = currentSelectedObject.get(); node && node != this; node = node->getParent())
			if (GuiBase2d* group = Instance::fastDynamicCast<GuiBase2d>(node))
				if (group->getSelectionGroup())
					changedSelectionGroups.insert(group);
		for (Instance* node = newSelectedObject.get(); node && node != this; node = node->getParent())
			if (GuiBase2d* group = Instance::fastDynamicCast<GuiBase2d>(node))
				if (group->getSelectionGroup())
					changedSelectionGroups.insert(group);
		...
		for (GuiBase2d* group : changedSelectionGroups)
		{
			const bool containsNewSelection = newSelectedObject &&
				(group == newSelectedObject.get() || group->isAncestorOf(newSelectedObject.get()));
			group->selectionChangedSignal(containsNewSelection, currentSelectedObject, newSelectedObject);
		}
...
GuiObject* BasePlayerGui::selectNewGuiObject(const Vector2& direction)
{
	...
	if (!closestGuiObject)
	{
		shared_ptr<Instance> stoppedSelectionGroup;
		for (Instance* node = sharedSelectedGuiObject.get(); node && node != this; node = node->getParent())
		{
			if (GuiBase2d* group = Instance::fastDynamicCast<GuiBase2d>(node))
			{
				if (group->getSelectionGroup() &&
					group->getSelectionBehavior(direction) == Enums::SELECTION_BEHAVIOR_STOP)
				{
					stoppedSelectionGroup = shared_from(group);
					break;
				}
			}
		}
		...
		{
			shared_ptr<Instance> selectionBoundary = stoppedSelectionGroup;
			if (!selectionBoundary)
				selectionBoundary = selectionGroup.first.lock();
			closestGuiObject = checkForDefaultGuiSelection(direction, currentRect, selectionBoundary);
		}
	}
...
GuiResponse BasePlayerGui::process(const shared_ptr<InputObject>& event)
{
	...
	std::function<void(Instance*)> collectLayers = [&](Instance* parent)
	{
		boost::shared_ptr<const Instances> children(parent->getChildren().read());
		if (!children)
			return;
		for (const boost::shared_ptr<Instance>& child : *children)
		{
			if (GuiBase* gui = fastDynamicCast<GuiBase>(child.get()))
			{
				ScreenGui* screen = fastDynamicCast<ScreenGui>(gui);
				layers.push_back(InputLayer{
					gui, screen ? screen->getDisplayOrder() : 0, sequence++});
				// Layer collectors process their own descendants exactly once.
				continue;
			}
			// Current CoreGui groups independently mounted ScreenGuis beneath
			// Folder/model containers (for example TopBarApp). The historical
			// direct-child loop made those layers renderable but non-interactive.
			if (fastDynamicCast<Folder>(child.get()))
				collectLayers(child.get());
		}
	};
	collectLayers(this);
	std::stable_sort(layers.begin(), layers.end(),
		[](const InputLayer& left, const InputLayer& right)
		{
	...
	for (const InputLayer& layer : layers)
	{
		GuiBase* guiBase = layer.gui;
		GuiResponse answer = guiBase->process(event);
		if (answer.wasSunk())
		{
			if (event->isMouseDownEvent() || event->isMouseUpEvent())
				std::cerr << "CoreGui input sunk child=" << guiBase->getFullName()
					<< " target="
					<< (answer.getTarget() ? answer.getTarget()->getFullName() : "<none>")
					<< '\n';
...
static Reflection::EnumPropDescriptor<PlayerGui, Enums::ScreenOrientation> propScreenOrientation(
    "ScreenOrientation", category_Behavior, &PlayerGui::getScreenOrientation,
    &PlayerGui::setScreenOrientation);
static Reflection::EnumPropDescriptor<PlayerGui, Enums::ScreenOrientation> propCurrentScreenOrientation(
    "CurrentScreenOrientation", category_State, &PlayerGui::getCurrentScreenOrientation,
...
: Super(),
    ...
    screenOrientation(Enums::SCREEN_ORIENTATION_SENSOR),
    currentScreenOrientation(Enums::SCREEN_ORIENTATION_LANDSCAPE_LEFT)
{
	setName("PlayerGui");

}

void PlayerGui::setScreenOrientation(Enums::ScreenOrientation value)
{
    if (screenOrientation == value)
        return;
    screenOrientation = value;
    raisePropertyChanged(propScreenOrientation);
}

void PlayerGui::setCurrentScreenOrientation(Enums::ScreenOrientation value)
{
    if (currentScreenOrientation == value)
        return;
    currentScreenOrientation = value;
    raisePropertyChanged(propCurrentScreenOrientation);
}
...
namespace Reflection 
{
	...
	EnumDesc<StarterGuiService::CoreGuiType>::EnumDesc():EnumDescriptor("CoreGuiType")
	{
		...
		addPair(StarterGuiService::COREGUI_EMOTESMENU, "EmotesMenu");
		addPair(StarterGuiService::COREGUI_SELFVIEW, "SelfView");
		addPair(StarterGuiService::COREGUI_CAPTURES, "Captures");
		addPair(StarterGuiService::COREGUI_AVATARSWITCHER, "AvatarSwitcher");
		addPair(StarterGuiService::COREGUI_EXPERIENCESHOP, "ExperienceShop");
	}
    ...
    resetPlayerGui(true)
{
	...
	for(int i = COREGUI_PLAYERLISTGUI; i <= COREGUI_EXPERIENCESHOP; ++i)
...
void StarterGuiService::setCoreGuiEnabled(CoreGuiType type, bool enabled)
{
	...
	else
	{
		...
		for (int i = COREGUI_PLAYERLISTGUI; i <= COREGUI_EXPERIENCESHOP; ++i)
		{
			...
			if (iteratedType != COREGUI_ALL && !coreGuiEnabledState[iteratedType])
			{
				allEnabled = false;
				break;
			}
		}
		coreGuiEnabledState[COREGUI_ALL] = allEnabled;
	}
...
static Reflection::EventDesc<CoreGuiService, void(bool, shared_ptr<Instance>, NormalId, float)> event_UserGuiRenderingChanged(
	&CoreGuiService::userGuiRenderingChangedSignal, "UserGuiRenderingChanged", "enabled", "guiAdornee",
	"faceId", "horizontalCurvature", Security::RobloxScript);
REFLECTION_END();
...
void CoreGuiService::setUserGuiRendering(bool enabled, const shared_ptr<Instance>& guiAdornee,
	NormalId faceId, float horizontalCurvature)
{
	userGuiRenderingChangedSignal(enabled, guiAdornee, faceId, horizontalCurvature);
}
