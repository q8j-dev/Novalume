...
namespace RBX
{
    ...
    static Reflection::BoundFuncDesc<AdService, shared_ptr<const Reflection::Tuple>()> func_getAdTeleportInfo(&AdService::getAdTeleportInfo, "GetAdTeleportInfo", Security::RobloxScript);
    static Reflection::EventDesc<AdService, void(bool)> event_videoClosed(&AdService::videoAdClosedSignal, "VideoAdClosed", "adShown");
	static Reflection::EventDesc<AdService, void()> event_adTeleportInitiated(
		&AdService::adTeleportInitiatedSignal, "AdTeleportInitiated", Security::RobloxScript);
	static Reflection::EventDesc<AdService, void()> event_adTeleportEnded(
		&AdService::adTeleportEndedSignal, "AdTeleportEnded", Security::RobloxScript);
	static Reflection::EventDesc<AdService, void(std::string, std::string)> event_showDynamicEudsaDisclosure(
		&AdService::showDynamicEudsaDisclosureSignal, "ShowDynamicEudsaDisclosure", "advertiserName", "payerName", Security::RobloxScript);
    REFLECTION_END();
   
    AdService::AdService() :
        showingVideoAd(false),
        adTeleportPlaceId(0),
        adTeleportUniverseId(0)
    {
        setName(sAdService);
    }

	shared_ptr<const Reflection::Tuple> AdService::getAdTeleportInfo()
	{
		shared_ptr<Reflection::Tuple> result(new Reflection::Tuple());
		result->values.push_back(Reflection::Variant(adTeleportPlaceId));
		result->values.push_back(Reflection::Variant(adTeleportUniverseId));
		return result;
	}

	void AdService::setAdTeleportInfo(long long placeId, long long universeId)
	{
		adTeleportPlaceId = placeId;
		adTeleportUniverseId = universeId;
	}
