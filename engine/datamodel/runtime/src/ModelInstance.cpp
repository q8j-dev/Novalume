...
static void includePartInBoundingBox(shared_ptr<Instance> descendant,
                                    const CoordinateFrame& boundingBoxAxes,
                                    Extents& bounds,
                                    bool& foundPart)
{
	if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(descendant.get()))
	{
		...
		const Extents partBounds = primitive->getExtentsLocal().express(
			primitive->getCoordinateFrame(), boundingBoxAxes);
		if (foundPart)
			bounds.unionWith(partBounds);
		else
		{
			bounds = partBounds;
			foundPart = true;
...
shared_ptr<const Reflection::Tuple> ModelInstance::getBoundingBox()
{
	CoordinateFrame boundingBoxAxes;
	if (PartInstance* primaryPart = getPrimaryPartSetByUser())
	{
		if (primaryPart->isDescendantOf(this))
			boundingBoxAxes.rotation = primaryPart->getCoordinateFrame().rotation;
	}

	Extents bounds;
	bool foundPart = false;
	visitDescendants(boost::bind(&includePartInBoundingBox, _1,
		boost::cref(boundingBoxAxes), boost::ref(bounds), boost::ref(foundPart)));

	CoordinateFrame resultFrame = boundingBoxAxes;
	Vector3 resultSize = Vector3::zero();
	if (foundPart)
	{
		resultFrame.translation = boundingBoxAxes.pointToWorldSpace(bounds.center());
		resultSize = bounds.size();
	}

	shared_ptr<Reflection::Tuple> result(new Reflection::Tuple(2));
	result->values[0] = resultFrame;
	result->values[1] = resultSize;
	return result;
}
