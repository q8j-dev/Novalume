...
template<>
EnumDesc<Lighting::Technology>::EnumDesc()
	: EnumDescriptor("Technology")
{
	addPair(Lighting::TECHNOLOGY_COMPATIBILITY, "Compatibility");
...
template<>
Lighting::Technology& Variant::convert<Lighting::Technology>()
{
	return genericConvert<Lighting::Technology>();
}
} }

template<>
bool RBX::StringConverter<RBX::Lighting::Technology>::convertToValue(
	const std::string& text, RBX::Lighting::Technology& value)
{
	return RBX::Reflection::EnumDesc<RBX::Lighting::Technology>::singleton()
...
void Lighting::setTechnology(Technology value)
{
	if (value != technology)
	{
		technology = value;
		this->raisePropertyChanged(desc_Technology);
		fireLightingChanged(false);
	}
}

void Lighting::setShadowSoftness(float value)
{
	value = G3D::clamp(value, 0.0f, 1.0f);
	if (value != shadowSoftness)
	{
		shadowSoftness = value;
		this->raisePropertyChanged(desc_ShadowSoftness);
		fireLightingChanged(false);
	}
}
