...
namespace RBX {
    ...
    namespace Reflection {
        ...
        template<>
        EnumDesc<Enums::PreferredInput>::EnumDesc()
        :EnumDescriptor("PreferredInput")
        {
            addPair(Enums::PREFERRED_INPUT_KEYBOARD_AND_MOUSE, "KeyboardAndMouse");
            addPair(Enums::PREFERRED_INPUT_GAMEPAD, "Gamepad");
            addPair(Enums::PREFERRED_INPUT_TOUCH, "Touch");
        }
        template<>
        ...
        :EnumDescriptor("Platform")
		{
			...
			addPair(UserInputService::PLATFORM_PS5, "PS5");
			addPair(UserInputService::PLATFORM_METAOS, "MetaOS");
			addPair(UserInputService::PLATFORM_WEB, "Web");
	...
	static Reflection::BoundFuncDesc<UserInputService, Enums::DeviceLevel()> func_getDeviceLevel(&UserInputService::getDeviceLevel, "GetDeviceLevel", Security::RobloxScript);
	...
	static Reflection::BoundFuncDesc<UserInputService, ContentId(KeyCode)> func_getImageForKeyCode(&UserInputService::getImageForKeyCode, "GetImageForKeyCode", "keyCode", Security::None);
	static Reflection::BoundFuncDesc<UserInputService, std::string(KeyCode)> func_getStringForKeyCode(&UserInputService::getStringForKeyCode, "GetStringForKeyCode", "keyCode", Security::None);
	static Reflection::BoundFuncDesc<UserInputService, Vector2()> func_getMouseDelta(
		&UserInputService::getMouseDelta, "GetMouseDelta", Security::None);
	...
	static const Reflection::EnumPropDescriptor<UserInputService, Enums::PreferredInput> prop_PreferredInput("PreferredInput", category_Data, &UserInputService::getPreferredInput, NULL);
    ...
    Reflection::PropDescriptor<UserInputService, Vector2>
        UserInputService::prop_StatusBarSize("StatusBarSize", category_Data,
            &UserInputService::getStatusBarSize, NULL,
            Reflection::PropertyDescriptor::SCRIPTING, Security::RobloxScript);
    Reflection::PropDescriptor<UserInputService, Vector2>
        UserInputService::prop_NavBarSize("NavBarSize", category_Data,
            &UserInputService::getNavBarSize, NULL,
            Reflection::PropertyDescriptor::SCRIPTING, Security::RobloxScript);
    Reflection::PropDescriptor<UserInputService, Vector2>
        UserInputService::prop_BottomBarSize("BottomBarSize", category_Data,
            &UserInputService::getBottomBarSize, NULL,
            Reflection::PropertyDescriptor::SCRIPTING, Security::RobloxScript);
    Reflection::PropDescriptor<UserInputService, Vector2>
    ...
    static Reflection::BoundFuncDesc<UserInputService,
        void(Vector2, Vector2, Vector2, Vector2)> func_sendAppUISizes(
            ...
            "bottomBarSize", "navBarSize", "statusBarSize",
	...
	static Reflection::EventDesc<UserInputService, void(float, RBX::Vector2, float, bool)>
		event_pointerAction(&UserInputService::pointerActionSignal, "PointerAction",
			"wheel", "pan", "pinch", "gameProcessedEvent", Security::None);
    ...
    {
		preferredInput = Enums::PREFERRED_INPUT_KEYBOARD_AND_MOUSE;
		switch (getPlatform()) 
		{
            // desktop
			...
			case UserInputService::PLATFORM_ANDROID:
            {
				...
				preferredInput = Enums::PREFERRED_INPUT_TOUCH;
			...
			case UserInputService::PLATFORM_XBOXONE:
			{
				...
				preferredInput = Enums::PREFERRED_INPUT_GAMEPAD;
    ...
    void UserInputService::sendAppUISizes(Vector2 rightBar, Vector2 bottomBar,
        Vector2 navBar, Vector2 statusBar)
    {
        if (rightBarSize != rightBar)
        {
            rightBarSize = rightBar;
            raisePropertyChanged(prop_RightBarSize);
        }
        if (bottomBarSize != bottomBar)
        {
            bottomBarSize = bottomBar;
            raisePropertyChanged(prop_BottomBarSize);
        }
        if (navBarSize != navBar)
        {
            navBarSize = navBar;
            raisePropertyChanged(prop_NavBarSize);
        }
        if (statusBarSize != statusBar)
        {
    ...
    void UserInputService::doDataModelProcessInput(DataModel* dataModel, const shared_ptr<InputObject>& inputObject, void* nativeInputObject, const InputObject::UserInputState& inputState)
    {
		...
		if (inputObject && inputObject->getUserInputType() == InputObject::TYPE_MOUSEWHEEL)
			pointerActionSignal(inputObject->getPosition().z, Vector2::zero(), 0.0f,
				processedEvent);
	...
	void UserInputService::dataModelProcessInput(const shared_ptr<InputObject>& inputObject, void* nativeInputObject, const InputObject::UserInputState& inputState)
	{
		if (inputObject)
		{
			if (inputObject->getUserInputType() == InputObject::TYPE_MOUSEDELTA)
			{
				const Vector3 delta = inputObject->getDelta();
				currentMouseDelta += Vector2(delta.x, delta.y);
			}
			if (inputObject->isPublicEvent())
			{
	...
	void UserInputService::updateLastInputType(const shared_ptr<InputObject>& inputObject)
	{
		if (lastInputType != inputObject->getUserInputType())
		{
			...
			case InputObject::TYPE_TEXTINPUT:
				family = UI::INPUT_FAMILY_KEYBOARD_AND_MOUSE;
				break;
			case InputObject::TYPE_TOUCH:
				family = UI::INPUT_FAMILY_TOUCH;
				break;
			...
			case InputObject::TYPE_GAMEPAD8:
				family = UI::INPUT_FAMILY_GAMEPAD;
				break;
			default:
				break;
			}
			...
			{
				preferredInput = nextPreferredInput;
				raisePropertyChanged(prop_PreferredInput);
			}
		}
	}
    ...
    {
		// GetMouseDelta is the sum of relative desktop samples consumed by this
		// render step. Absolute MouseMovement events are intentionally excluded;
		// captured hosts emit a paired MouseDelta and counting both doubles camera
		// motion.
		currentMouseDelta = Vector2::zero();
...
#if defined(__APPLE__) && !RBX_PLATFORM_IOS
        clipBoardText = rbx::platform::readClipboardText();
#endif
	...
	// Mouse Manipulation
   ///////////////////////////////////////////////////////////////////////////
	ContentId UserInputService::getImageForKeyCode(KeyCode keyCode)
	{
		const char* image = NULL;
		switch (keyCode)
		{
		case SDLK_GAMEPAD_BUTTONA: image = "ButtonA"; break;
		case SDLK_GAMEPAD_BUTTONB: image = "ButtonB"; break;
		case SDLK_GAMEPAD_BUTTONX: image = "ButtonX"; break;
		case SDLK_GAMEPAD_BUTTONY: image = "ButtonY"; break;
		case SDLK_GAMEPAD_BUTTONL1: image = "ButtonL1"; break;
		case SDLK_GAMEPAD_BUTTONL2: image = "ButtonL2"; break;
		case SDLK_GAMEPAD_BUTTONL3: image = "ButtonL3"; break;
		case SDLK_GAMEPAD_BUTTONR1: image = "ButtonR1"; break;
		case SDLK_GAMEPAD_BUTTONR2: image = "ButtonR2"; break;
		case SDLK_GAMEPAD_BUTTONR3: image = "ButtonR3"; break;
		case SDLK_GAMEPAD_BUTTONSTART: image = "ButtonStart"; break;
		case SDLK_GAMEPAD_BUTTONSELECT: image = "ButtonSelect"; break;
		case SDLK_GAMEPAD_DPADLEFT: image = "DpadLeft"; break;
		case SDLK_GAMEPAD_DPADRIGHT: image = "DpadRight"; break;
		case SDLK_GAMEPAD_DPADUP: image = "DpadUp"; break;
		case SDLK_GAMEPAD_DPADDOWN: image = "DpadDown"; break;
		case SDLK_GAMEPAD_THUMBSTICK1: image = "Thumbstick1"; break;
		case SDLK_GAMEPAD_THUMBSTICK2: image = "Thumbstick2"; break;
		case SDLK_GAMEPAD_THUMBSTICK1UP:
		case SDLK_GAMEPAD_THUMBSTICK1DOWN: image = "Thumbstick1Vertical"; break;
		case SDLK_GAMEPAD_THUMBSTICK1LEFT:
		case SDLK_GAMEPAD_THUMBSTICK1RIGHT: image = "Thumbstick1Horizontal"; break;
		case SDLK_GAMEPAD_THUMBSTICK2UP:
		case SDLK_GAMEPAD_THUMBSTICK2DOWN: image = "Thumbstick2Vertical"; break;
		case SDLK_GAMEPAD_THUMBSTICK2LEFT:
		case SDLK_GAMEPAD_THUMBSTICK2RIGHT: image = "Thumbstick2Horizontal"; break;
		default: return ContentId();
		}
		return ContentId(std::string("rbxasset://textures/ui/Controls/DesignSystem/") + image + ".png");
	}

	std::string UserInputService::getStringForKeyCode(KeyCode keyCode)
	{
		// Roblox keyboard KeyCode values retain SDL's layout-independent ASCII
		// range. Printable keys therefore have a stable display glyph on every
		// host; named navigation/gamepad keys use their reflected engine name.
		...
		default:
			return Reflection::EnumDesc<KeyCode>::singleton().convertToString(keyCode);
		}
	}
	...
	ContentId UserInputService::getDefaultMouseCursor(bool hoverOverActive, bool relativePathNoExtension)
	{
		...
		if (hoverOverActive)
		{
			...
			{
				defaultKeyboardMouseCursorString = "Cursors/KeyboardMouse/ArrowCursor";
				defaultGamepadCursorString = "Cursors/Gamepad/PointerOver";
			}
			else
			{
				defaultKeyboardMouseCursorString = "textures/Cursors/KeyboardMouse/ArrowCursor.png";
		...
		else
		{
			if (relativePathNoExtension)
			{
				defaultKeyboardMouseCursorString = "Cursors/KeyboardMouse/ArrowFarCursor";
				defaultGamepadCursorString = "Cursors/Gamepad/Pointer";
			}
			else
			{
				defaultKeyboardMouseCursorString = "textures/Cursors/KeyboardMouse/ArrowFarCursor.png";
