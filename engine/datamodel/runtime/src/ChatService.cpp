...
static Reflection::BoundYieldFuncDesc<ChatService, bool(int)> func_canUserChatAsync(
	&ChatService::canUserChatAsync, "CanUserChatAsync", "userId", Security::None);

static Reflection::BoundFuncDesc<ChatService, void(Reflection::Variant)>
	funcSetBubbleChatSettings(&ChatService::setBubbleChatSettings,
		"SetBubbleChatSettings", "settings", Security::None);
...
static Reflection::RemoteEventDesc<ChatService, void(bool, long long)>
	eventTimeoutChatAttempt(&ChatService::timeoutChatAttemptSignal,
		"TimeoutChatAttempt", "isPermanentTimeout", "endTime",
		Security::RobloxScript, Reflection::RemoteEventCommon::REPLICATE_ONLY,
		Reflection::RemoteEventCommon::BROADCAST);
static Reflection::PropDescriptor<ChatService, bool> propIsAutoMigrated(
	"IsAutoMigrated", category_Data, &ChatService::getIsAutoMigrated,
	&ChatService::setIsAutoMigrated, Reflection::PropertyDescriptor::HIDDEN_SCRIPTING,
	Security::RobloxScript);
static Reflection::PropDescriptor<ChatService, bool> propBubbleChatEnabled(
	"BubbleChatEnabled", category_Data, &ChatService::getBubbleChatEnabled, NULL,
	Reflection::PropertyDescriptor::SCRIPTING, Security::None);
static Reflection::PropDescriptor<ChatService, bool> propLoadDefaultChat(
	"LoadDefaultChat", category_Data, &ChatService::getLoadDefaultChat, NULL,
	Reflection::PropertyDescriptor::HIDDEN_SCRIPTING, Security::RobloxScript);
static Reflection::EventDesc<ChatService, void(Reflection::Variant)>
	eventBubbleChatSettingsChanged(&ChatService::bubbleChatSettingsChangedSignal,
		"BubbleChatSettingsChanged", "settings", Security::RobloxScript);
REFLECTION_END();
...
void ChatService::setBubbleChatEnabled(bool value)
{
	if (bubbleChatEnabled == value)
		return;
	bubbleChatEnabled = value;
	raisePropertyChanged(propBubbleChatEnabled);
}

void ChatService::setBubbleChatSettings(Reflection::Variant settings)
{
	if (!settings.isType<shared_ptr<const Reflection::ValueTable> >())
		throw std::runtime_error("Chat:SetBubbleChatSettings expects a settings table");
	bubbleChatSettingsChangedSignal(settings);
}

void ChatService::setIsAutoMigrated(bool value)
{
	if (isAutoMigrated == value)
		return;
	isAutoMigrated = value;
	raisePropertyChanged(propIsAutoMigrated);
}

void ChatService::canUserChatAsync(int userId,
	boost::function<void(bool)> resumeFunction,
	boost::function<void(std::string)> errorFunction)
{
	TextChatService* textChatService =
		ServiceProvider::create<TextChatService>(this);
	textChatService->canUserChatAsync(userId, resumeFunction, errorFunction);
}
...
void ChatService::timeoutChatAttempt(bool isPermanentTimeout, long long endTime)
{
	eventTimeoutChatAttempt.fireAndReplicateEvent(
		this, isPermanentTimeout, endTime);
}
