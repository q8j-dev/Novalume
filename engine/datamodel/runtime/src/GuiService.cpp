...
FASTFLAGVARIABLE(LocalizeVersionLabels, true)
...
namespace RBX
{
	...
	static Reflection::PropDescriptor<GuiService, bool> prop_TouchControlsEnabled("TouchControlsEnabled", category_Data, &GuiService::getTouchControlsEnabled, &GuiService::setTouchControlsEnabled);
	...
	static Reflection::BoundFuncDesc<GuiService, shared_ptr<const Reflection::Tuple>()> func_getGuiInset(&GuiService::getGuiInset, "GetGuiInset", Security::None);
	static Reflection::BoundFuncDesc<GuiService, Rect2D(ScreenInsetsType)> func_getInsetArea(&GuiService::getInsetArea, "GetInsetArea", "screenInsets", Security::None);
	static Reflection::BoundFuncDesc<GuiService, Vector2()> func_getHardwareSafeViewport(
		&GuiService::getHardwareSafeViewport, "GetHardwareSafeViewport", Security::RobloxScript);
	static Reflection::PropDescriptor<GuiService, Rect2D> prop_topbarInset("TopbarInset", category_Data, &GuiService::getTopbarInset, NULL);
	static Reflection::BoundFuncDesc<GuiService, void(Rect2D)> func_setTopbarInset(&GuiService::setTopbarInset, "SetTopbarInset", "topbarInset", Security::RobloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(float, float, float, float)> func_setHardwareSafeAreaInsets(&GuiService::setHardwareSafeAreaInsets, "SetHardwareSafeAreaInsets", "left", "top", "right", "bottom", Security::RobloxScript);
	static Reflection::BoundFuncDesc<GuiService, shared_ptr<const Reflection::ValueTable>()> func_getSafeZoneOffsets(&GuiService::getSafeZoneOffsets, "GetSafeZoneOffsets", Security::RobloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(float, float, float, float)> func_setSafeZoneOffsets(&GuiService::setSafeZoneOffsets, "SetSafeZoneOffsets", "top", "bottom", "left", "right", Security::RobloxScript);
	static Reflection::EventDesc<GuiService, void()> event_safeZoneOffsetsChanged(&GuiService::safeZoneOffsetsChangedSignal, "SafeZoneOffsetsChanged", Security::RobloxScript);
	...
	static Reflection::BoundFuncDesc<GuiService, int()> func_GetResolutionScale(
		&GuiService::getResolutionScale, "GetResolutionScale", Security::RobloxScript);
	static Reflection::BoundFuncDesc<GuiService, shared_ptr<const Reflection::ValueTable>()> func_GetNotificationTypeList(&GuiService::getNotificationTypeList, "GetNotificationTypeList", Security::RobloxScript);
	...
	static Reflection::EventDesc<GuiService, void()> event_nativeClose(
		&GuiService::nativeCloseSignal, "NativeClose", Security::RobloxScript);
	...
	Reflection::EnumPropDescriptor<GuiService, Enums::PreferredTextSize> GuiService::prop_preferredTextSize(
		"PreferredTextSize", category_Data, &GuiService::getPreferredTextSize, NULL);
	static Reflection::PropDescriptor<GuiService, float> prop_preferredTransparency(
		"PreferredTransparency", category_Data, &GuiService::getPreferredTransparency, NULL,
		Reflection::PropertyDescriptor::SCRIPTING);
	static Reflection::EnumPropDescriptor<GuiService, Enums::DisplaySize> prop_viewportDisplaySize(
		"ViewportDisplaySize", category_Data, &GuiService::getViewportDisplaySize, NULL);
	...
	static Reflection::BoundFuncDesc<GuiService, bool()> func_getInspectMenuEnabled(
		&GuiService::getInspectMenuEnabled, "GetInspectMenuEnabled", Security::None);
	static Reflection::BoundFuncDesc<GuiService, void(bool)> func_setInspectMenuEnabled(
		&GuiService::setInspectMenuEnabled, "SetInspectMenuEnabled", "enabled", Security::None);
	static Reflection::EventDesc<GuiService, void(bool)> event_inspectMenuEnabledChanged(
		&GuiService::inspectMenuEnabledChangedSignal, "InspectMenuEnabledChangedSignal", "enabled", Security::RobloxScript);
	static Reflection::EventDesc<GuiService, void(shared_ptr<Instance>, std::string)>
		event_inspectPlayerFromHumanoidDescriptionRequest(
			&GuiService::inspectPlayerFromHumanoidDescriptionRequestSignal,
			"InspectPlayerFromHumanoidDescriptionRequest", "humanoidDescription", "name",
			Security::RobloxScript);
	static Reflection::EventDesc<GuiService, void(long long, std::string)>
		event_inspectPlayerFromUserIdWithCtxRequest(
			&GuiService::inspectPlayerFromUserIdWithCtxRequestSignal,
			"InspectPlayerFromUserIdWithCtxRequest", "userId", "ctx",
			Security::RobloxScript);
	static Reflection::EventDesc<GuiService, void()> event_closeInspectMenuRequest(
		&GuiService::closeInspectMenuRequestSignal, "CloseInspectMenuRequest",
		Security::RobloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(shared_ptr<Instance>, std::string)>
		func_inspectPlayerFromHumanoidDescription(
			&GuiService::inspectPlayerFromHumanoidDescription,
			"InspectPlayerFromHumanoidDescription", "humanoidDescription", "name",
			Security::None);
	static Reflection::BoundFuncDesc<GuiService, void(long long)>
		func_inspectPlayerFromUserId(
			static_cast<void (GuiService::*)(long long)>(&GuiService::inspectPlayerFromUserId),
			"InspectPlayerFromUserId", "userId", Security::None);
	static Reflection::BoundFuncDesc<GuiService, void(long long, std::string)>
		func_inspectPlayerFromUserIdWithCtx(
			static_cast<void (GuiService::*)(long long, std::string)>(&GuiService::inspectPlayerFromUserId),
			"InspectPlayerFromUserIdWithCtx", "userId", "ctx", Security::RobloxScript);
	static Reflection::BoundFuncDesc<GuiService, void()> func_closeInspectMenu(
		&GuiService::closeInspectMenu, "CloseInspectMenu", Security::None);
	static Reflection::EventDesc<GuiService, void(bool)> event_networkPausedEnabledChanged(
		&GuiService::networkPausedEnabledChangedSignal, "NetworkPausedEnabledChanged",
		"enabled", Security::RobloxScript);
	...
	GuiService::GuiService()
		:Super("GuiService")
		,guiInset(Vector4::zero())
		,hardwareSafeAreaInsets(Vector4::zero())
		,safeZoneOffsets(Vector4::zero())
		,topbarInset(Rect2D::xyxy(0, 0, 0, 0))
		,currentDialog(NULL)
		,subscribedChars()
		,subscribedSpecials()
		,errorMessage("")
		...
		,resolutionScale(1)
	{
		preferredTextSizeConnection = GameBasicSettings::singleton().preferredTextSizeChangedSignal.connect(
			[this](Enums::PreferredTextSize) { raisePropertyChanged(prop_preferredTextSize); });
		preferredTransparencyConnection = GameBasicSettings::singleton().propertyChangedSignal.connect(
			[this](const Reflection::PropertyDescriptor* property) {
				if (property && property->name == "PreferredTransparency")
					raisePropertyChanged(prop_preferredTransparency);
			});
	}

	void GuiService::setTouchControlsEnabled(bool value)
	{
		if (touchControlsEnabled != value)
		{
			touchControlsEnabled = value;
			raisePropertyChanged(prop_TouchControlsEnabled);
		}
	}

	void GuiService::setResolutionScale(int value)
	{
		if (value < 1 || value > 3)
			throw std::invalid_argument("GuiService resolution scale must be between 1 and 3");
		resolutionScale = value;
	}

	Enums::PreferredTextSize GuiService::getPreferredTextSize() const
	{
		return GameBasicSettings::singleton().getPreferredTextSize();
	}

	float GuiService::getPreferredTransparency() const
	{
		return GameBasicSettings::singleton().getPreferredTransparency();
	}

	Enums::DisplaySize GuiService::getViewportDisplaySize() const
	{
		UserInputService* input = ServiceProvider::find<UserInputService>(this);
		if (!input || !input->getTouchEnabled() || UserInputService::isTenFootInterface())
			return Enums::DISPLAY_SIZE_LARGE;

		const Vector2 resolution = const_cast<GuiService*>(this)->getScreenResolution();
		const float shortestSide = std::min(resolution.x, resolution.y);
		if (shortestSide < 600.0f)
			return Enums::DISPLAY_SIZE_SMALL;
		if (shortestSide < 900.0f)
			return Enums::DISPLAY_SIZE_MEDIUM;
		return Enums::DISPLAY_SIZE_LARGE;
	}
	...
	void GuiService::setInspectMenuEnabled(bool enabled)
	{
		if (inspectMenuEnabled == enabled)
			return;
		inspectMenuEnabled = enabled;
		inspectMenuEnabledChangedSignal(enabled);
	}
	...
	shared_ptr<const Reflection::ValueTable> GuiService::getNotificationTypeList()
	{
		// This ordered native notification contract is present in both supplied
		// 2026 Player binaries. Values are stable ordinal protocol identifiers.
		static const char* const names[] = {
			"VIEW_PROFILE", "REPORT_ABUSE", "VIEW_GAME_DETAILS", "SHOW_TAB_BAR",
			"HIDE_TAB_BAR", "UNREAD_COUNT", "PRIVACY_SETTINGS",
			"BACK_BUTTON_NOT_CONSUMED", "PURCHASE_ROBUX", "VIEW_NOTIFICATIONS",
			"APP_READY", "CLOSE_MODAL", "VIEW_GAME_DETAILS_ANIMATED", "SEARCH_GAMES",
			"GAMES_SEE_ALL", "ACTION_LOG_OUT", "LAUNCH_CONVERSATION",
			"UNIVERSAL_FRIENDS", "OPEN_CUSTOM_WEBVIEW", "SET_STATUS_BAR_STYLE",
			"OPEN_YOUTUBE_VIDEO", "DID_LOG_OUT", "OPEN_BUILDERS_CLUB", "ACTION_LOG_IN",
			"OPEN_SETTINGS_WEBVIEW", "DID_LOG_IN", "OPEN_CAPTCHA_VIEW", "DID_SIGN_UP",
			"LUA_UNAUTHORIZED_LOG_OUT", "NATIVE_SHARE", "SDK_SHARE", "SDK_OPEN_URL",
			"NATIVE_EXIT", "RESTART_LUA_APP", "IDENTITY_VERIFICATION", "SCAN_QR_CODE",
			"SHOW_FRIEND_QR_CODE", "DID_SWITCH_ACCOUNT",
			"GAME_STREAMING_STATUS_CHANGED"
		};
		...
		for (std::size_t index = 0; index < sizeof(names) / sizeof(names[0]); ++index)
			(*result)[names[index]] = static_cast<int>(index);
		return result;
	}
	...
	shared_ptr<const Reflection::Tuple> GuiService::getGuiInset()
	{
		shared_ptr<Reflection::Tuple> result(new Reflection::Tuple());
		result->values.push_back(Reflection::Variant(Vector2(guiInset.x, guiInset.y)));
		result->values.push_back(Reflection::Variant(Vector2(guiInset.z, guiInset.w)));
		return result;
	}

	Rect2D GuiService::getInsetArea(ScreenInsetsType screenInsets)
	{
		const Vector2 resolution = getScreenResolution();
		float left = 0;
		float top = 0;
		float right = resolution.x;
		float bottom = resolution.y;
		switch (screenInsets)
		{
		case SCREEN_INSETS_DEVICE_SAFE:
			left += hardwareSafeAreaInsets.x;
			top += hardwareSafeAreaInsets.y;
			right -= hardwareSafeAreaInsets.z;
			bottom -= hardwareSafeAreaInsets.w;
			break;
		...
		case SCREEN_INSETS_TOPBAR_SAFE:
			top = std::max(top, topbarInset.y1());
			break;
		case SCREEN_INSETS_NONE:
		default:
			break;
		}
		return Rect2D::xyxy(left, top, std::max(left, right),
			std::max(top, bottom));
	}

	Vector2 GuiService::getHardwareSafeViewport()
	{
		const Rect2D safeArea = getInsetArea(SCREEN_INSETS_DEVICE_SAFE);
		return Vector2(safeArea.width(), safeArea.height());
	}

	void GuiService::setTopbarInset(Rect2D value)
	{
		if (topbarInset == value)
			return;
		topbarInset = value;
		raisePropertyChanged(prop_topbarInset);
	}

	void GuiService::setHardwareSafeAreaInsets(float left, float top, float right,
		float bottom)
	{
		hardwareSafeAreaInsets = Vector4(left, top, right, bottom);
	}

	shared_ptr<const Reflection::ValueTable> GuiService::getSafeZoneOffsets()
	{
		shared_ptr<Reflection::ValueTable> result(new Reflection::ValueTable());
		(*result)["top"] = Reflection::Variant(safeZoneOffsets.x);
		(*result)["bottom"] = Reflection::Variant(safeZoneOffsets.y);
		(*result)["left"] = Reflection::Variant(safeZoneOffsets.z);
		(*result)["right"] = Reflection::Variant(safeZoneOffsets.w);
		return result;
	}

	void GuiService::setSafeZoneOffsets(float top, float bottom, float left,
		float right)
	{
		const Vector4 value(top, bottom, left, right);
		if (safeZoneOffsets == value)
			return;
		safeZoneOffsets = value;
		safeZoneOffsetsChangedSignal();
	}
