...
DYNAMIC_FASTFLAGVARIABLE(FormFactorDeprecated,true)
...
const PropDescriptor<PartInstance, bool> PartInstance::prop_CastShadow("CastShadow", category_Appearance, &PartInstance::getCastShadow, &PartInstance::setCastShadow);
...
{
	if (World* world = WorldRoot::getWorldIfInWorldRoot(this)) {
...
void PartInstance::updatePrimitiveState()
{
	...
	World* world = WorldRoot::getWorldIfInWorldRoot(this);
	...
	if (world && !oldWorld)
	{
		...
		if (!service)
		{
			if (collisionGroupName != "Default")
				throw runtime_error("Collision group '%s' cannot enter Workspace before PhysicsService is available",
					collisionGroupName.c_str());
			getPartPrimitive()->setCollisionGroup(0, 0xffffffffu);
		}
		else
		{
			...
			getPartPrimitive()->setCollisionGroup(groupId,
				service->getCollisionGroupMask(groupId));
		}
	}
...
void PartInstance::onAncestorChanged(const AncestorChanged& event)
{
	...
	Primitive* prim = getPartPrimitive();
	if (prim->getNumJoints() > 0 && prim->findWorld() != WorldRoot::getWorldIfInWorldRoot(this))
	{
...
std::string PartInstance::getCollisionGroup() const
{
	...
	if (PhysicsService* service = ServiceProvider::find<PhysicsService>(this))
		return service->getCollisionGroupName(static_cast<int>(id));
	return collisionGroupName;
}

void PartInstance::setCollisionGroup(const std::string& name)
{
	PhysicsService* service = ServiceProvider::find<PhysicsService>(this);
	if (!service)
	{
		if (name.empty() || name.size() > 100)
			throw runtime_error("Collision group name must contain between 1 and 100 characters");
		collisionGroupName = name;
		raiseChanged(prop_CollisionGroup);
		return;
	}
	const unsigned int id = static_cast<unsigned int>(service->getCollisionGroupId(name));
	setCollisionGroupInternal(id, service->getCollisionGroupMask(id));
}

int PartInstance::getCollisionGroupId() const
{
	return static_cast<int>(getConstPartPrimitive()->getCollisionGroupId());
}
...
void PartInstance::setCollisionGroupId(int id)
{
	...
	setCollisionGroupInternal(static_cast<unsigned int>(id),
		service->getCollisionGroupMask(static_cast<unsigned int>(id)));
}

void PartInstance::setCollisionGroupInternal(unsigned int id, unsigned int mask)
{
	Primitive* primitive = getPartPrimitive();
	const std::string previousName = collisionGroupName;
	const unsigned int previousId = primitive->getCollisionGroupId();
	const unsigned int previousMask = primitive->getCollisionGroupMask();
	if (PhysicsService* service = ServiceProvider::find<PhysicsService>(this))
		collisionGroupName = service->getCollisionGroupName(static_cast<int>(id));
	else if (id == 0)
		collisionGroupName = "Default";
	const bool nameChanged = collisionGroupName != previousName;
	const bool idChanged = previousId != id;
	const bool maskChanged = previousMask != mask;
	if (!idChanged && !maskChanged && !nameChanged)
		return;
	if (idChanged || maskChanged)
		primitive->setCollisionGroup(id, mask);
	if (idChanged || nameChanged)
		raiseChanged(prop_CollisionGroup);
	if (idChanged)
		raiseChanged(prop_CollisionGroupId);
}
...
void PartInstance::setCastShadow(bool value)
{
	if (value != castShadow) {
		castShadow = value;
		raisePropertyChanged(prop_CastShadow);
		shouldRenderSetDirty();
	}
}
