...
namespace RBX {
	...
	Reflection::PropDescriptor<BillboardGui, Vector3> prop_studsOffsetWorldSpace("StudsOffsetWorldSpace", category_Data, &BillboardGui::getStudsOffsetWorldSpace, &BillboardGui::setStudsOffsetWorldSpace);
	...
	Reflection::PropDescriptor<BillboardGui, float>	  prop_LightInfluence("LightInfluence", category_Data, &BillboardGui::getLightInfluence, &BillboardGui::setLightInfluence);
    ...
    static bool getBillboardFrame(const Camera* camera, const CoordinateFrame& cameraCFrame, const Vector3& partSize, const CoordinateFrame& partCFrame, const Vector3& partExtentRelativeOffset, const Vector3& partStudsOffset, const Vector3& partStudsOffsetWorldSpace, const Vector2& billboardSizeRelativeOffset, const UDim2& billboardSize, bool vr, CoordinateFrame& result, Rect2D& viewport)
    {
        ...
        Vector3 billboardCenterInCameraSpace = relativeOffsetInCameraSpace + partStudsOffset +
			cameraCFrame.vectorToObjectSpace(partStudsOffsetWorldSpace);
	...
	BillboardGui::BillboardGui()
		...
		,partStudsOffsetWorldSpace(Vector3::zero())
        ,alwaysOnTop(false)
		,lightInfluence(1.0f)
		,enabled(true)
		,active(false)
            ...
            if (part && workspace)
            {
                ...
                if (FFlag::BillboardGuiVR)
                {
                    ...
                    if (uis && uis->getVREnabled())
                    {
                        ...
                        visibleAndValid = getBillboardFrame(camera, cframe, adornSize, adornCFrame, partExtentRelativeOffset, partStudsOffset, partStudsOffsetWorldSpace, billboardSizeRelativeOffset, billboardSize, /* vr= */ true, projectionFrame, viewport);
                    }
                    else
                    {
                        visibleAndValid = getBillboardFrame(camera, cameraCFrame, adornSize, adornCFrame, partExtentRelativeOffset, partStudsOffset, partStudsOffsetWorldSpace, billboardSizeRelativeOffset, billboardSize, /* vr= */ false, projectionFrame, viewport);
                    }
                ...
                else
                {
                    ...
                    viewportBillboarder->partStudsOffsetWorldSpace = partStudsOffsetWorldSpace;
	...
	const Vector3& BillboardGui::getStudsOffsetWorldSpace() const
	{
		return partStudsOffsetWorldSpace;
	}

	void BillboardGui::setStudsOffsetWorldSpace(const Vector3& value)
	{
		if (partStudsOffsetWorldSpace != value)
		{
			partStudsOffsetWorldSpace = value;
			raisePropertyChanged(prop_studsOffsetWorldSpace);
		}
	}
	...
	void BillboardGui::setLightInfluence(float value)
	{
		value = G3D::clamp(value, 0.0f, 1.0f);
		if (lightInfluence != value)
		{
			lightInfluence = value;
			raisePropertyChanged(prop_LightInfluence);
		}
	}
