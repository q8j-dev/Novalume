...
namespace RBX
{
    ...
    REFLECTION_BEGIN();
    Reflection::BoundProp<bool> GamepadService::prop_gamepadCursorEnabled(
        "GamepadCursorEnabled", category_Data,
        &GamepadService::gamepadCursorEnabled, Reflection::PropertyDescriptor::STANDARD,
        Security::RobloxScript);
    static Reflection::BoundFuncDesc<GamepadService, void()> funcAutoSelectGui(
        &GamepadService::autoSelectGuiLua, "AutoSelectGui", Security::RobloxScript);
    static Reflection::BoundFuncDesc<GamepadService, void(shared_ptr<Instance>)> funcEnableGamepadCursor(
        &GamepadService::enableGamepadCursor, "EnableGamepadCursor", "guiObject", Security::None);
    static Reflection::BoundFuncDesc<GamepadService, void()> funcDisableGamepadCursor(
        &GamepadService::disableGamepadCursor, "DisableGamepadCursor", Security::None);
    static Reflection::BoundFuncDesc<GamepadService, Vector2()> funcGetGamepadCursorPosition(
        &GamepadService::getGamepadCursorPosition, "GetGamepadCursorPosition", Security::RobloxScript);
    static Reflection::BoundFuncDesc<GamepadService, void(Vector2)> funcSetGamepadCursorPosition(
        &GamepadService::setGamepadCursorPosition, "SetGamepadCursorPosition", "position", Security::RobloxScript);
    static Reflection::EventDesc<GamepadService, void(Vector2)> eventGamepadThumbstick1Changed(
        &GamepadService::gamepadThumbstick1ChangedSignal, "GamepadThumbstick1Changed", "event", Security::RobloxScript);
    REFLECTION_END();
    ...
    GamepadService::GamepadService() :
		...
		gamepadCursorPosition(Vector2::zero())
    {
        setName(sGamepadService);
    }

	void GamepadService::setGamepadCursorEnabled(bool enabled)
	{
		if (gamepadCursorEnabled == enabled)
			return;
		gamepadCursorEnabled = enabled;
		raisePropertyChanged(prop_gamepadCursorEnabled);
	}

	void GamepadService::enableGamepadCursor(shared_ptr<Instance> instance)
	{
		GuiObject* guiObject = Instance::fastDynamicCast<GuiObject>(instance.get());
		if (!guiObject)
			throw std::runtime_error("EnableGamepadCursor requires a GuiObject");
		PlayerGui* playerGui = guiObject->findFirstAncestorOfType<PlayerGui>();
		if (!playerGui)
			throw std::runtime_error("EnableGamepadCursor requires a descendant of PlayerGui");
		if (!guiObject->isCurrentlyVisible())
			throw std::runtime_error("EnableGamepadCursor requires a visible GuiObject");
		gamepadCursorPosition = guiObject->getAbsolutePosition() + guiObject->getAbsoluteSize() * 0.5f;
	...
	void GamepadService::setGamepadCursorPosition(Vector2 position)
	{
		GuiService* guiService = ServiceProvider::find<GuiService>(this);
		const Vector2 viewport = guiService ? guiService->getScreenResolution() : Vector2::zero();
		if (viewport.x > 0.0f)
			position.x = std::max(0.0f, std::min(position.x, viewport.x));
		if (viewport.y > 0.0f)
			position.y = std::max(0.0f, std::min(position.y, viewport.y));
		gamepadCursorPosition = position;
	}
	...
	void GamepadService::onInputChanged(const shared_ptr<Instance>& event)
	{
		if (InputObject* inputObject = Instance::fastDynamicCast<InputObject>(event.get()))
		{
			...
			{
				gamepadThumbstick1ChangedSignal(event->get2DPosition());
