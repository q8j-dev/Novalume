...
static Reflection::PropDescriptor<Workspace, BinaryString> prop_collisionGroupData("CollisionGroupData", category_Data, &Workspace::getCollisionGroupData, &Workspace::setCollisionGroupData, Reflection::PropertyDescriptor::STANDARD_NO_SCRIPTING);
...
template<> RaycastParams& Variant::convert<RaycastParams>()
{
    if (!isType<RaycastParams>())
        throw runtime_error("Unable to convert value to RaycastParams");
    return cast<RaycastParams>();
}

template<> const Type& Type::getSingleton<RaycastParams>()
{
    static TType<RaycastParams> type("RaycastParams");
    return type;
}

template<> OverlapParams& Variant::convert<OverlapParams>()
{
    if (!isType<OverlapParams>())
        throw runtime_error("Unable to convert value to OverlapParams");
    return cast<OverlapParams>();
}

template<> const Type& Type::getSingleton<OverlapParams>()
{
    static TType<OverlapParams> type("OverlapParams");
    return type;
}

template<> const Type& Type::getSingleton<RaycastResult>()
{
    static TType<RaycastResult> type("RaycastResult");
    return type;
}
...
class RaycastQueryFilter : public HitTestFilter
{
public:
    RaycastQueryFilter(const RaycastParams& params, unsigned int collisionMask)
        : params(params)
        , collisionMask(collisionMask)
    {
    }

    Result filterResult(const Primitive* primitive) const override
    {
        const PartInstance* part = PartInstance::fromConstPrimitive(primitive);
        bool listed = false;
        if (params.filterDescendantsInstances)
        {
            for (Instances::const_iterator it = params.filterDescendantsInstances->begin();
                 it != params.filterDescendantsInstances->end(); ++it)
            {
                const Instance* root = it->get();
                if (root && (part == root || part->isDescendantOf(root)))
                {
                    listed = true;
                    break;
                }
...
bool overlapIncludes(const PartInstance& part, const OverlapParams& params,
    unsigned int collisionMask)
{
    bool listed = false;
    if (params.filterDescendantsInstances)
...
G3D::Box partBounds(const PartInstance& part)
{
    const G3D::Vector3 half = part.getPartSizeUi() * 0.5f;
    return part.getCoordinateFrame().toWorldSpace(G3D::AABox(-half, half));
}
...
static Reflection::BoundFuncDesc<Workspace, shared_ptr<const Reflection::Tuple>(RbxRay, shared_ptr<const Instances>, bool, bool)> workspace_FindPartOnRayWhitelist(&Workspace::getRayHitWhitelist, "FindPartOnRayWithWhitelist", "ray", "whitelistDescendentsTable", "terrainCellsAreCubes", false, "ignoreWater", false, Security::None);
static Reflection::BoundFuncDesc<Workspace,
    Reflection::Variant(G3D::Vector3, G3D::Vector3, RaycastParams)>
    workspace_Raycast(&Workspace::raycast, "Raycast", "origin", "direction",
        "raycastParams", RaycastParams(), Security::None);
static Reflection::BoundFuncDesc<Workspace,
    shared_ptr<const Instances>(G3D::CoordinateFrame, G3D::Vector3, OverlapParams)>
    workspace_GetPartBoundsInBox(&Workspace::getPartBoundsInBox,
        "GetPartBoundsInBox", "cframe", "size", "overlapParams", OverlapParams(), Security::None);
static Reflection::BoundFuncDesc<Workspace,
...
static Reflection::BoundFuncDesc<Workspace, double()> getServerTimeNowFunction(
	&Workspace::getServerTimeNow, "GetServerTimeNow", Security::None);
...
Workspace::Workspace(IDataState* dataState)	 :
	...
	serverTimeReferenceMonotonic(std::chrono::steady_clock::now()),
	serverTimeReferenceUnix(std::chrono::duration<double>(
	...
	, collisionGroupData(BinaryString(std::string("\x01\x01\x00\x04\xff\xff\xff\xff\x07" "Default", 16)))
{
...
{
	std::lock_guard<std::mutex> guard(serverTimeMutex);
	return serverTimeReferenceUnix + std::chrono::duration<double>(
		std::chrono::steady_clock::now() - serverTimeReferenceMonotonic).count();
}

void Workspace::synchronizeServerTime(double serverUnixSeconds)
{
	if (!std::isfinite(serverUnixSeconds) || serverUnixSeconds <= 0.0)
		throw std::runtime_error("server time must be a positive finite Unix timestamp");
	std::lock_guard<std::mutex> guard(serverTimeMutex);
	serverTimeReferenceMonotonic = std::chrono::steady_clock::now();
	serverTimeReferenceUnix = serverUnixSeconds;
}

BinaryString Workspace::getCollisionGroupData() const
{
	if (PhysicsService* service = ServiceProvider::find<PhysicsService>(this))
		return service->getCollisionGroupData();
	return collisionGroupData;
}

void Workspace::setCollisionGroupData(const BinaryString& value)
{
	if (PhysicsService* service = ServiceProvider::find<PhysicsService>(this))
	{
		service->setCollisionGroupData(value);
		collisionGroupData = service->getCollisionGroupData();
	}
	else
	{
		collisionGroupData = value;
	}
}
...
shared_ptr<const Reflection::Tuple> Workspace::getRayHitWhitelist(
    RbxRay ray, shared_ptr<const Instances> whitelist,
    bool terrainCellsAreCubes, bool ignoreWaterCells)
{
    (void)terrainCellsAreCubes;
    RaycastParams params;
    params.filterDescendantsInstances = whitelist;
    params.filterType = Enums::RAYCAST_FILTER_INCLUDE;
    params.ignoreWater = ignoreWaterCells;

    const Reflection::Variant hit = raycast(ray.origin(), ray.direction(), params);
    shared_ptr<Reflection::Tuple> result(new Reflection::Tuple());
    if (hit.isType<RaycastResult>())
    {
        const RaycastResult& raycastResult = hit.cast<RaycastResult>();
        result->values.push_back(raycastResult.instance);
        result->values.push_back(raycastResult.position);
        result->values.push_back(raycastResult.normal);
        result->values.push_back(raycastResult.material);
    }
    else
    {
        result->values.push_back(shared_ptr<Instance>());
        result->values.push_back(ray.origin() + ray.direction());
        result->values.push_back(Vector3::zero());
        result->values.push_back(AIR_MATERIAL);
    }
    return result;
}

Reflection::Variant Workspace::raycast(
    G3D::Vector3 origin, G3D::Vector3 direction, RaycastParams params)
{
    if (!origin.isFinite() || !direction.isFinite())
        throw runtime_error("Workspace:Raycast origin and direction must be finite");

    const float length = direction.magnitude();
    if (length > 15000.0f)
        throw runtime_error("Workspace:Raycast direction must not exceed 15000 studs");
    if (length <= 0.0f)
        return Reflection::Variant();

    PhysicsService* physicsService = ServiceProvider::find<PhysicsService>(this);
    if (!physicsService)
        throw runtime_error("Workspace:Raycast requires PhysicsService");
    const unsigned int queryGroupId = static_cast<unsigned int>(
        physicsService->getCollisionGroupId(params.collisionGroup));
    const unsigned int queryCollisionMask =
        physicsService->getCollisionGroupMask(queryGroupId);

    ContactManager* contactManager = getWorld()->getContactManager();
    if (!contactManager)
        return Reflection::Variant();

    RaycastQueryFilter filter(params, queryCollisionMask);
    G3D::Vector3 hitPoint;
    G3D::Vector3 surfaceNormal;
    PartMaterial surfaceMaterial = AIR_MATERIAL;
    Primitive* primitive = NULL;
    const G3D::Vector3 unitDirection = direction / length;
    float traveled = 0.0f;
    while (!primitive && traveled < length)
    {
        const float segmentLength = std::min(4999.0f, length - traveled);
        const RbxRay segment = RbxRay::fromOriginAndDirection(
            origin + unitDirection * traveled, unitDirection * segmentLength);
        primitive = params.bruteForceAllSlow
            ? contactManager->getHitAllSlow(segment, NULL, &filter,
                hitPoint, surfaceNormal, surfaceMaterial)
            : contactManager->getHit(segment, NULL, &filter, hitPoint, false,
                params.ignoreWater, surfaceNormal, surfaceMaterial);
    ...
    result.instance = shared_from<Instance>(part);
    result.position = hitPoint;
    result.normal = surfaceNormal;
    result.material = surfaceMaterial;
    result.distance = (hitPoint - origin).magnitude();
    return Reflection::Variant(result);
}

shared_ptr<const Instances> Workspace::getPartBoundsInBox(
    G3D::CoordinateFrame frame, G3D::Vector3 size, OverlapParams params)
{
    if (!frame.translation.isFinite() || !size.isFinite() ||
        size.x <= 0.0f || size.y <= 0.0f || size.z <= 0.0f)
        throw runtime_error("Workspace:GetPartBoundsInBox requires a finite CFrame and positive size");
    PhysicsService* physicsService = ServiceProvider::find<PhysicsService>(this);
    if (!physicsService)
        throw runtime_error("Workspace:GetPartBoundsInBox requires PhysicsService");
    ...
    const G3D::Vector3 half = size * 0.5f;
    const G3D::Box query = frame.toWorldSpace(G3D::AABox(-half, half));
    ...
    visitDescendants([&](shared_ptr<Instance> instance) {
        ...
        if (!part || !overlapIncludes(*part, params, collisionMask))
            return;
...
shared_ptr<const Instances> Workspace::getPartBoundsInRadius(
    G3D::Vector3 position, float radius, OverlapParams params)
{
    if (!position.isFinite() || !std::isfinite(radius) || radius <= 0.0f)
        throw runtime_error("Workspace:GetPartBoundsInRadius requires a finite position and positive radius");
    PhysicsService* physicsService = ServiceProvider::find<PhysicsService>(this);
    if (!physicsService)
        throw runtime_error("Workspace:GetPartBoundsInRadius requires PhysicsService");
    const unsigned int groupId = static_cast<unsigned int>(
        physicsService->getCollisionGroupId(params.collisionGroup));
    const unsigned int collisionMask = physicsService->getCollisionGroupMask(groupId);
    ...
    visitDescendants([&](shared_ptr<Instance> instance) {
        ...
        if (!part || !overlapIncludes(*part, params, collisionMask))
            return;
        const G3D::Vector3 local = part->getCoordinateFrame().pointToObjectSpace(position);
        const G3D::Vector3 half = part->getPartSizeUi() * 0.5f;
        const G3D::Vector3 closest(
            G3D::clamp(local.x, -half.x, half.x),
            G3D::clamp(local.y, -half.y, half.y),
