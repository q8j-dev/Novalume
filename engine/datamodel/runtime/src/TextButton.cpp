...
void GuiTextButton::render2dContext(Adorn* adorn, const Instance* context)
{
	...
	if(filterState == ContentFilter::Succeeded)
		render2dTextImpl(adorn, rect, localizeText(getContentText()), getFont(), getTextSize(), applyCanvasGroup(getRenderTextColor4()), applyCanvasGroup(getRenderTextStrokeColor4()), getTextWrap(), getTextScale(), getXAlignment(), getYAlignment(), getTextTruncate());
