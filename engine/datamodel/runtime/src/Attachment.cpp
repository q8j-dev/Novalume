...
CoordinateFrame Attachment::getParentFrame() const
{
    if( !locked )
    {
        ...
        if (const Attachment* attachment =
                Instance::fastDynamicCast<const Attachment>(getParent()))
            return attachment->getFrameInWorld();
...
void Attachment::verifySetParent(const Instance* instance) const
{
    ...
    // Current skinned rigs use nested attachment markers (for example the
    // JointRotation marker below a shoulder RigAttachment), as well as
    // attachments beneath animated Bones.  Their local frame composes through
    // the attachment hierarchy in getParentFrame().
    if( instance != NULL &&
        instance->fastDynamicCast< PartInstance >() == NULL &&
        instance->fastDynamicCast< Workspace >() == NULL &&
        instance->fastDynamicCast< Attachment >() == NULL )
    {
        const std::string message =
            "Attachments can only be parented to parts, attachments, or the workspace; got " +
            instance->getClassNameStr();
        throw RBX::runtime_error(message.c_str());
    }
}

void Attachment::verifyAddChild(const Instance* newChild) const
{
    // Current avatar rigs store scaling metadata such as the
    // OriginalPosition Vector3Value beneath rig attachments. Attachment
    // children still pass the normal Instance parenting validation.
