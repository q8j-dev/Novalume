...
static Reflection::BoundFuncDesc<InsertService, shared_ptr<Instance>(std::string)> func_LoadLocalAsset(
	&InsertService::loadLocalAsset, "LoadLocalAsset", "assetPath", Security::RobloxScript);
...
shared_ptr<Instance> InsertService::loadLocalAsset(std::string assetPath)
{
	const ContentId contentId(assetPath);
	if (!contentId.isAsset())
		throw RBX::runtime_error("LoadLocalAsset only accepts rbxasset:// paths");

	ContentProvider* contentProvider = ServiceProvider::create<ContentProvider>(this);
	if (!contentProvider)
		throw RBX::runtime_error("LoadLocalAsset cannot access ContentProvider");

	Instances roots;
	contentProvider->blockingLoadInstances(contentId, roots);
	if (roots.size() != 1 || !roots.front())
		throw RBX::runtime_error("LoadLocalAsset expected one root instance in %s", assetPath.c_str());
	roots.front()->setParent(NULL);
	return roots.front();
}
