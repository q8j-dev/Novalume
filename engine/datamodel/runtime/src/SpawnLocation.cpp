...
void SpawnerService::SpawnPlayer(Workspace* workspace, shared_ptr<ModelInstance> model, CoordinateFrame location, int forceFieldDuration)
{
	...
	// calculateSpawnLocation already returns the center of a clear character-sized
	// volume.  Applying the legacy seven-stud R6 lift to an R15 rig can put the
	// whole avatar inside a low ceiling; physics then resolves it onto the roof.
	// Preserve the old placement contract for R6 places, but use the computed
	// safe center directly for modern rigs.
	Humanoid* humanoid = Humanoid::modelIsCharacter(model.get());
	if (!humanoid || !humanoid->getUseR15())
		dropPoint.y += 7;
