...
namespace
{
int reportLuauAssertion(const char* expression, const char* file, int line,
    const char* function)
{
    std::fprintf(stderr, "Luau assertion failed: %s (%s:%d, %s)\n",
...
void installLuauAssertionDiagnostics()
{
    static std::once_flag installed;
    std::call_once(installed, [] {
        Luau::assertHandler() = &reportLuauAssertion;
    });
}
}
#endif
...
struct LuaProfiler
{
    static std::string getResumePosition(lua_State* L, int nargs)
    {
        ...
        {
            lua_Debug debug = {};
            if (!lua_getinfo(L, -(nargs + 1), "s", &debug))
                return "??";
            if (debug.what && strcmp(debug.what, "C") == 0)
                return "=[C](-1)";
            const char* file = debug.source ? debug.source : "?";
            const bool string = strchr(file, '\n') != NULL;
            return RBX::format("%s(%d)", string ? "<string>" : file, debug.linedefined);
        }
        return "(resume)";
#else
    ...
    {
#if defined(RBX_LUAU_VM)
        if (!lua_getinfo(L, 0, "ns", ar))
            return;
        const char* func = stringCache.getCall(ar->name, ar->what, ar->source, ar->linedefined);
#else
        lua_getinfo(L, "nS", ar);
...
#if defined(RBX_LUAU_VM)
            while (depth < lua_stackdepth(L) && lua_getinfo(L, depth, "sln", &entry))
            {
                stack.push_back(stringCache.getCall(
                    entry.name, entry.what, entry.source, entry.currentline));
                ++depth;
            }
#else
...
#if defined(RBX_LUAU_VM)
static int utf8NfcNormalize(lua_State* L)
{
    size_t length = 0;
    const char* input = luaL_checklstring(L, 1, &length);
    utf8proc_uint8_t* normalized = NULL;
    const utf8proc_ssize_t normalizedLength = utf8proc_map(
        reinterpret_cast<const utf8proc_uint8_t*>(input),
    ...
    {
        luaL_error(L, "invalid UTF-8 string: %s", utf8proc_errmsg(normalizedLength));
        return 0;
    }

    lua_pushlstring(L, reinterpret_cast<const char*>(normalized),
        static_cast<size_t>(normalizedLength));
    std::free(normalized);
    return 1;
}

static void installUtf8Extensions(lua_State* L)
{
    lua_getglobal(L, "utf8");
    RBXASSERT(lua_istable(L, -1));
    lua_pushcfunction(L, utf8NfcNormalize);
...
bool ScriptContext::openState(size_t idx)
{
...
#if defined(RBX_LUAU_VM)
		lua_stackdepth(NULL);
		lua_getinfo(NULL, 0, "", NULL);
		lua_getargument(NULL, 0, 0);
		lua_getlocal(NULL, 0, 0);
		lua_setlocal(NULL, 0, 0);
		lua_getupvalue(NULL, 0, 0);
		lua_setupvalue(NULL, 0, 0);
#else
		lua_getstack(0,0,0);
		lua_getinfo(0,0,0);
		lua_getlocal(0,0,0);
...
#if defined(RBX_LUAU_VM)
	RobloxExtraSpace::constructRoot(globalState);
	RobloxExtraSpace::get(globalState)->setContext(this);
	WeakThreadRef::Node::create(globalState);
	lua_callbacks(globalState)->userthread = [](lua_State* parent, lua_State* thread) {
		if (parent)
			RobloxExtraSpace::constructChild(thread, RobloxExtraSpace::get(parent));
		else
			RobloxExtraSpace::destroyChild(thread);
	};
#endif

#if defined(RBX_LUAU_VM)
	lua_callbacks(globalState)->userdata = this;
	lua_callbacks(globalState)->interrupt = [](lua_State* state, int gc) {
		if (gc < 0)
			ScriptContext::hook(state, NULL);
	};
	lua_callbacks(globalState)->panic = [](lua_State* state, int) {
		panic(state);
	};
	lua_gc(globalState, LUA_GCSETSTEPMUL, RBX::LuaSettings::singleton().gcStepMul);
	lua_gc(globalState, LUA_GCSETGOAL, RBX::LuaSettings::singleton().gcPause);
#else
...
#if defined(RBX_LUAU_VM)
        loadLibraryProtected(globalState, luaopen_coroutine);
        loadLibraryProtected(globalState, luaopen_bit32);
        loadLibraryProtected(globalState, luaopen_utf8);
        installUtf8Extensions(globalState);
        loadLibraryProtected(globalState, luaopen_buffer);
#endif
...

	{
		...
		ContentBridge::registerClass(globalState);
		FontBridge::registerClass(globalState);
		...
		Path2DControlPointBridge::registerClass(globalState);
		DateTimeBridge::registerClass(globalState);
		RandomBridge::registerClass(globalState);
		TweenInfoBridge::registerClass(globalState);
		RaycastParamsBridge::registerClass(globalState);
		OverlapParamsBridge::registerClass(globalState);
		RaycastResultBridge::registerClass(globalState);
    }

	{
		RBXASSERT_BALLANCED_LUA_STACK(globalState);
		LibraryBridge::registerClassLibrary(globalState);
		Lua::Enums::registerClassLibrary(globalState);
		...
		ContentBridge::registerClassLibrary(globalState);
		FontBridge::registerClassLibrary(globalState);
		...
		ValueObjectBridge::registerClassLibrary(globalState);
		...
		Path2DControlPointBridge::registerClassLibrary(globalState);
		DateTimeBridge::registerClassLibrary(globalState);
		RandomBridge::registerClassLibrary(globalState);
		TweenInfoBridge::registerClassLibrary(globalState);
		RaycastParamsBridge::registerClassLibrary(globalState);
		OverlapParamsBridge::registerClassLibrary(globalState);
    }
...
#if defined(RBX_LUAU_VM)
		// Current Luau's base library implements protected calls with a native
		// yield continuation. Preserve the historical ypcall alias without
		// wrapping the call in a second manually-resumed coroutine.
		lua_getglobal(globalState, "pcall");
		lua_setglobal(globalState, "ypcall");
#else
		// The legacy Lua VM needs Roblox's yieldable protected-call bridge.
...
#if defined(RBX_LUAU_VM)
		// Luau's task library is backed by the same scheduler queues as the
		// engine's yielding globals.  The table is frozen with the rest of the
		// runtime by luaL_sandbox after registration completes.
		lua_newtable(globalState);
		lua_pushcfunction(globalState, taskSpawn);
		lua_setfield(globalState, -2, "spawn");
		lua_pushcfunction(globalState, taskDefer);
		lua_setfield(globalState, -2, "defer");
		lua_pushcfunction(globalState, taskDelay);
		lua_setfield(globalState, -2, "delay");
		lua_pushcfunction(globalState, taskCancel);
		lua_setfield(globalState, -2, "cancel");
		lua_pushcfunction(globalState, wait);
		lua_setfield(globalState, -2, "wait");
		lua_setglobal(globalState, "task");
#endif
...
#if defined(RBX_LUAU_VM)
		loadLibraryProtected(globalState, luaopen_os);
#else
        loadLibraryProtected(globalState, luaopen_os_rbx);
#endif

		Lua::Enums::declareAllEnums(globalState);
	}
...
void ScriptContext::closeStates(bool resettingSimulation)
{
	...
	// Stop every scheduler entry that can resume or collect this VM before
	// releasing script-owned coroutine references.  Closing a Luau state while
	// WaitingScriptsJob or the incremental GC job is in flight can mutate the
	// gray list concurrently with lua_close.
	if (gcJob)
	{
	...
	// Render-step callbacks deliberately keep their originating script threads alive.
	// Release those pins before closing the Lua states that own them.
	clearBoundRenderStepCallbackPins(runService.get());
...
void ScriptContext::closeState(lua_State* globalState)
{
	...
	dumpThreadRefCounts();
#if defined(RBX_LUAU_VM)
	RobloxExtraSpace* rootSpace = RobloxExtraSpace::get(globalState);
	lua_close(globalState);
	RobloxExtraSpace::destroyDetached(rootSpace);
#else
	lua_close(globalState);
#endif
	dumpThreadRefCounts();
...
{
#if defined(RBX_LUAU_VM)
    installLuauAssertionDiagnostics();
#endif
...
{
#if defined(RBX_LUAU_VM)
    return thread ? lua_mainthread(thread) : NULL;
#else
...
int ScriptContext::taskSpawn(lua_State* thread)
{
	ThreadRef caller(thread);
	const int top = lua_gettop(thread);
	if (top < 1 || (!lua_isfunction(thread, 1) && !lua_isthread(thread, 1)))
	{
		luaL_error(thread, "task.spawn requires a function or thread");
		return 0;
	}

	lua_State* target = NULL;
	int argumentCount = top - 1;
	if (lua_isthread(thread, 1))
	{
		target = lua_tothread(thread, 1);
		if (target == thread)
		{
			luaL_error(thread, "task.spawn cannot resume the running thread");
			return 0;
		}
		for (int index = 2; index <= top; ++index)
			lua_pushvalue(thread, index);
		if (argumentCount)
			lua_xmove(thread, target, argumentCount);
	}
	else
	{
		target = lua_newthread(thread);
		for (int index = 1; index <= top; ++index)
			lua_pushvalue(thread, index);
		lua_xmove(thread, target, top);
	}

	ThreadRef targetLock(target);
	ScriptContext& context = ScriptContext::getContext(thread);
	if (context.resume(target, argumentCount) != ScriptContext::Yield)
		lua_resetstack(target, 0);

	if (lua_isthread(thread, 1))
		lua_pushvalue(thread, 1);
	// For a function, lua_newthread left the created thread at the top.
	return 1;
}

int ScriptContext::taskDefer(lua_State* thread)
{
	ThreadRef caller(thread);
	const int top = lua_gettop(thread);
	if (top < 1 || (!lua_isfunction(thread, 1) && !lua_isthread(thread, 1)))
	{
		luaL_error(thread, "task.defer requires a function or thread");
		return 0;
	}

	lua_State* target = NULL;
	const int argumentCount = top - 1;
	if (lua_isthread(thread, 1))
	{
		target = lua_tothread(thread, 1);
		if (target == thread)
		{
			luaL_error(thread, "task.defer cannot resume the running thread");
			return 0;
		}
		for (int index = 2; index <= top; ++index)
			lua_pushvalue(thread, index);
	...
	Lua::YieldingThreads* yielding = ScriptContext::getContext(thread).yieldEvent.get();
	if (yielding)
		yielding->queueWaiter(target, 0.0, argumentCount, false);
...
int ScriptContext::taskDelay(lua_State* thread)
{
	ThreadRef caller(thread);
	const int top = lua_gettop(thread);
	if (top < 2 || !lua_isnumber(thread, 1) || !lua_isfunction(thread, 2))
	{
		luaL_error(thread, "task.delay requires a duration and function");
		return 0;
	}

	LUA_NUMBER timeout = lua_tonumber(thread, 1);
	if (Math::isNanInf(timeout))
	{
		luaL_error(thread, "task.delay duration must be finite");
		return 0;
	}
	timeout = std::max<LUA_NUMBER>(0.0, timeout);

	lua_State* target = lua_newthread(thread);
	const int argumentCount = top - 2;
	for (int index = 2; index <= top; ++index)
		lua_pushvalue(thread, index);
	lua_xmove(thread, target, top - 1);

	Lua::YieldingThreads* yielding = ScriptContext::getContext(thread).yieldEvent.get();
	if (yielding)
		yielding->queueWaiter(target, timeout, argumentCount, false);
	return 1;
}

int ScriptContext::taskCancel(lua_State* thread)
{
	if (!lua_isthread(thread, 1))
	{
		luaL_error(thread, "task.cancel requires a thread");
		return 0;
	}
	...
	if (target == thread)
	{
		luaL_error(thread, "task.cancel cannot cancel the running thread");
		return 0;
	}

	RobloxExtraSpace* targetSpace = RobloxExtraSpace::get(target);
	if (targetSpace->taskCancelled || (lua_status(target) == 0 && lua_gettop(target) == 0))
	{
		luaL_error(thread, "task.cancel cannot cancel a completed thread");
		return 0;
	}

	targetSpace->taskCancelled = true;
	Lua::YieldingThreads* yielding = ScriptContext::getContext(thread).yieldEvent.get();
	if (yielding)
		yielding->cancelWaiter(target);
#if defined(RBX_LUAU_VM)
	lua_resetthread(target);
#else
	// Lua 5.1 has no reset-thread API.  The retained legacy target only needs
	// to discard the cancelled coroutine stack; its scheduler reference is
	// removed above and normal collection reclaims the coroutine.
	lua_settop(target, 0);
#endif
	return 0;
}
...
std::string ScriptContext::extractCallStack(lua_State* thread, shared_ptr<BaseScript>& source, int& line)
{
	...
	lua_Debug ar;
#if defined(RBX_LUAU_VM)
	const int depth = std::min(lua_stackdepth(thread), 12);
	for (int level = 0; level < depth; ++level)
	{
		if (!lua_getinfo(thread, level, "sln", &ar))
			continue;
		if (level == 0)
			line = ar.currentline;
		if (ar.currentline >= 0)
		{
			const char* chunk = ar.short_src[0] ? ar.short_src : (ar.source ? ar.source : "?");
			if (ar.name)
				str << chunk << ", line " << ar.currentline << " - "
				    << (ar.what ? ar.what : "") << " " << ar.name << std::endl;
			else
				str << chunk << ", line " << ar.currentline << std::endl;
		}
	}
#else
...
void ScriptContext::printCallStack(lua_State* thread, std::string * output, bool dontPrint)
{
	...
	{
		if (!lua_getinfo(thread, level, "sln", &ar) || ar.currentline < 0)
			continue;
		moreStack = true;
		const char* chunk = ar.short_src[0] ? ar.short_src : (ar.source ? ar.source : "?");
		if (level == 0)
		...
		const std::string messageLine = ar.name
			? RBX::format("Script '%s', Line %d - %s %s", chunk, ar.currentline,
			      ar.what ? ar.what : "", ar.name)
			: RBX::format("Script '%s', Line %d", chunk, ar.currentline);
		if (!dontPrint)
			RBX::StandardOut::singleton()->printf(MESSAGE_INFO, "%s", messageLine.c_str());
		if (output)
			stream << messageLine << std::endl;
	}
	if (moreStack && level < 12)
	{
		if (!dontPrint)
...
ScriptContext::Result ScriptContext::resume(ThreadRef thread, int narg)
{
...
#if defined(RBX_LUAU_VM)
		if (!lua_callbacks(thread)->interrupt && script)
#else
	...
	case LUA_YIELD:
		// Engine yield sites capture their own wake-up source. Uncaptured tasks use
		// the scheduler's next-cycle wake-up, matching the CoreScript task model.
...
void ScriptContext::startScript(ScriptStart scriptStart)
{
	...
	try
	{
		...
		if (profile)
		{
			lua_callbacks(thread)->debugstep = [](lua_State* state, lua_Debug* debug) {
				if (LuaProfiler::instance)
					LuaProfiler::instance->hookCall(state, debug);
			};
			lua_singlestep(thread, 1);
		}
#else
		lua_sethook(thread, ScriptContext::hook,
			(profile ? LUA_MASKCALL | LUA_MASKRET : 0) | LUA_MASKCOUNT, hookCount);
#endif
...
Reflection::Variant ScriptContext::evaluateStudioCommandItem(const char* itemToEvaluate, shared_ptr<LuaSourceContainer> script)
{
	...
	{
#if defined(RBX_LUAU_VM)
		const std::string expression = "return " + std::string(itemToEvaluate);
		const ProtectedString source = ProtectedString::fromTrustedSource(expression);
		if (LuaVM::load(L, source, "=StudioCommand", LuaVM::getModKeyCore()) != 0)
			throw std::runtime_error(safe_lua_tostring(L, -1));
		if (lua_pcall(L, 0, 1, 0) != 0)
			throw std::runtime_error(safe_lua_tostring(L, -1));
		Lua::LuaArguments::get(L, -1, result, false);
		lua_pop(L, 1);
#else
