	...
	case LUA_TUSERDATA:
		{
			if (ObjectBridge::getPtr(L, index, value))
				return true;
			if (ValueObjectBridge::getPtr(L, index, value))
				return true;
			else
				return false;
		}
	...
	case LUA_TUSERDATA:
		{
			...
			if (TweenInfoBridge::getValue(L, luaIndex, value))
				return true;
			if (ContentBridge::getValue(L, luaIndex, value))
				return true;
			if (RaycastParamsBridge::getValue(L, luaIndex, value))
				return true;
			if (OverlapParamsBridge::getValue(L, luaIndex, value))
				return true;
			if (RaycastResultBridge::getValue(L, luaIndex, value))
				return true;
            ...
            if (ColorSequenceKeypointBridge::getValue(L, luaIndex, value))
                return true;
			if (Path2DControlPointBridge::getValue(L, luaIndex, value))
				return true;
...
public:
	...
	int operator()(const TweenInfo& value)
	{
		TweenInfoBridge::pushTweenInfo(L, value);
		return 1;
	}
	...
	int operator()(const shared_ptr<Reflection::DescribedBase>& value)
	{
		ValueObjectBridge::push(L, value);
		return 1;
	}
	...
	int operator()(const Content& value)
	{
		ContentBridge::pushContent(L, value);
		return 1;
	}

	int operator()(const Font& value)
	{
		FontBridge::pushFont(L, value);
		return 1;
	}
