...
namespace LuaDebugExtension
{
	...
	{
		RBX::Profiler::Token token;
		uint64_t timestamp;
	};

	static thread_local std::unordered_map<lua_State*, std::vector<ProfileRegion> > profileRegions;

	static int debug_profilebegin(lua_State* L)
	{
		const char* label = luaL_checkstring(L, 1);
		ProfileRegion region;
		region.token = RBX::Profiler::getToken("Lua", label);
		region.timestamp = RBX::Profiler::enterRegion(region.token);
		profileRegions[L].push_back(region);
		return 0;
	}

	static int debug_profileend(lua_State* L)
	{
		std::unordered_map<lua_State*, std::vector<ProfileRegion> >::iterator it =
			profileRegions.find(L);
		if (it == profileRegions.end() || it->second.empty())
			return 0;
		const ProfileRegion region = it->second.back();
		it->second.pop_back();
		RBX::Profiler::leaveRegion(region.token, region.timestamp);
		if (it->second.empty())
			profileRegions.erase(it);
		return 0;
	}
...
static int debug_traceback(lua_State *L)
{
	...
	{
		const char* message = RBX::Lua::safe_lua_tostring(L, 1);
		if (message && *message)
			callStack = std::string(message) + "\n" + callStack;
	}
...
const luaL_Reg registry[] = 
{
	...
	{"profilebegin", debug_profilebegin},
	{"profileend", debug_profileend},
	{NULL, NULL}
};
