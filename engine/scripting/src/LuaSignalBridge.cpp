...
bool matchesFilteredSignalArgument(const Reflection::Variant& argument, const std::string& filter)
{
	if (argument.isType<std::string>())
		return argument.cast<std::string>() == filter;

	// Instance.Changed is reflected as a PropertyDescriptor pointer. Lua sees
	// that value as the property's name, so GetPropertyChangedSignal must apply
	// its native filter against the descriptor name before dropping the Changed
	// argument from the property-specific callback.
	if (argument.isType<const Reflection::PropertyDescriptor*>())
	{
		const Reflection::PropertyDescriptor* descriptor =
			argument.cast<const Reflection::PropertyDescriptor*>();
		return descriptor && descriptor->name.toString() == filter;
	}
...
int Bridge<EventInstance>::on_index(const EventInstance& object, const char* name, lua_State* L)
{
	...
	if (strcmp(name, "once") == 0 || strcmp(name, "Once") == 0)
	{
		lua_pushcfunction(L, EventBridge::once);
		return 1;
	}
...
public:
    ...
    FunctionScriptSlot(const Reflection::EventDescriptor* descriptor, lua_State* thread, int functionIndex, bool useSubmitTask, const std::string& attributeFilter, bool oneShot)
	...
	{
		Reflection::EventArguments filteredArguments;
		const Reflection::EventArguments* deliveredArguments = &arguments;
		if (!attributeFilter.empty())
		{
			if (arguments.empty() ||
				!matchesFilteredSignalArgument(arguments[0], attributeFilter))
				return;
			filteredArguments.assign(arguments.begin() + 1, arguments.end());
			deliveredArguments = &filteredArguments;
		}
		shared_ptr<WrapperType> oneShotSelf;
		if (oneShot)
		{
			oneShotSelf = weakSelf.lock();
			disconnect();
		}
		if (useSubmitTask)
		{
			...
			{
				if (oneShot)
					dm->submitTask(boost::bind(&FunctionScriptSlot::safeDoEventFireOnce,
						oneShotSelf, *deliveredArguments /*copy arguments*/), DataModelJob::Write);
				else
					dm->submitTask(boost::bind(&FunctionScriptSlot::safeDoEventFire,
						weakSelf, *deliveredArguments /*copy arguments*/), DataModelJob::Write);
			}
		}
		else
		{
			doEventFire(*deliveredArguments);
		}
	}
	...
	static void safeDoEventFireOnce(shared_ptr<WrapperType> self, const Reflection::EventArguments& arguments)
	{
		if (self)
			self->slot.doEventFire(arguments);
	}
	...
	void doEventFire(const Reflection::EventArguments& arguments)
	{
		...
		if (executionDepth>6)
		{
			...
			if (ThreadRef functionThread = function.lock())
			{
				...
				if (lua_getinfo(functionThread, -1, "s", &ar))
#else
		...
		if (ThreadRef functionThread = function.lock())
		{
...

			{
				...
				else
				{
					...
					// Current Luau requires callers to reserve stack space before
					// every API operation that pushes. The old VM grew this slot
					// implicitly, which made error-signal reentrancy trap in
					// lua_newthread once the parent's call frame was full.
					if (!lua_checkstack(functionThread, 1))
						throw std::runtime_error(
							"Unable to reserve the event handler thread stack slot");
	...
	: public BaseScript::Slot
{
	...
	weak_ptr<WrapperType> weakSelf;
public:
	WaitScriptSlot(lua_State* thread, bool useSubmitTask, const std::string& attributeFilter)
		:waitThread(thread)
		,useSubmitTask(useSubmitTask)
		,attributeFilter(attributeFilter)
	{
		...
		if (useSubmitTask)
		{
			if (shared_ptr<DataModel> dm = weakDm.lock())
			{
				dm->submitTask(boost::bind(&WaitScriptSlot::safeDoEventFire,
					weakSelf, *deliveredArguments /*copy arguments*/),
					DataModelJob::Write);
			}
		}
		else
...
static int connectEvent(lua_State* L, bool oneShot)
{
	RobloxExtraSpace* space = RobloxExtraSpace::get(L);
	EventInstance& ei(EventBridge::getObject(L, 1));
	...
	if (source && ei.descriptor->isScriptable() && !space->context()->shouldPreventNewConnections())
	{
		...
		shared_ptr< RBX::Reflection::TGenericSlotWrapper<FunctionScriptSlot> > wrapper(
			rbx::make_shared< RBX::Reflection::TGenericSlotWrapper<FunctionScriptSlot> >(FunctionScriptSlot(ei.descriptor, L, 2, useSubmitTask, ei.attributeFilter, oneShot))
		);
		if (useSubmitTask)
		{
...
{
	return connectEvent(L, false);
}

int EventBridge::once(lua_State* L)
{
	return connectEvent(L, true);
}
...
{
	{
		...
		if (source && ei.descriptor->isScriptable())
		{
			...
			shared_ptr<RBX::Reflection::TGenericSlotWrapper<WaitScriptSlot> > wrapper(
				rbx::make_shared< RBX::Reflection::TGenericSlotWrapper<WaitScriptSlot> >(WaitScriptSlot(L, useSubmitTask, ei.attributeFilter))
			);
			if (useSubmitTask)
			{
				wrapper->slot.setWeakSelf(wrapper);
