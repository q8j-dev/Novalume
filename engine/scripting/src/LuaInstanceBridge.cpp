...
template<>
const char* Bridge< shared_ptr<Instance>, false >::className("Instance");
template<>
const char* Bridge< shared_ptr<Reflection::DescribedBase>, false >::className("ValueObject");

void ValueObjectBridge::registerClass(lua_State* L)
{
    typedef Bridge<shared_ptr<Reflection::DescribedBase>, false> Base;
    luaL_newmetatable(L, Base::className);
    Lua::protect_metatable(L, -1);
    lua_pushliteral(L, "__type");
    lua_pushliteral(L, "Object");
    lua_settable(L, -3);
    lua_pushliteral(L, "__index");
    lua_pushcfunction(L, Base::on_index);
    lua_settable(L, -3);
    lua_pushliteral(L, "__newindex");
    lua_pushcfunction(L, Base::on_newindex);
    lua_settable(L, -3);
    lua_pushliteral(L, "__gc");
    lua_pushcfunction(L, Base::on_gc);
    lua_settable(L, -3);
    lua_pushliteral(L, "__tostring");
    lua_pushcfunction(L, Base::on_tostring);
    lua_settable(L, -3);
...
int Instance::getAttributeChangedSignal(lua_State* state)
{
	if (lua_gettop(state) < 2 || !lua_isstring(state, 2))
		throw RBX::runtime_error("Attribute name must be a string");

	size_t length = 0;
	const char* text = lua_tolstring(state, 2, &length);
	std::string attribute(text, length);
	if (attribute.empty() || attribute.size() > 100)
		throw RBX::runtime_error("Attribute name must be between 1 and 100 characters");

	Lua::EventInstance event;
	event.descriptor = getAttributeChangedEventDescriptor();
	event.source = shared_from(this);
	event.attributeFilter = attribute;
	Lua::EventBridge::pushNewObject(state, event);
	return 1;
}

int Instance::getPropertyChangedSignal(lua_State* state)
{
	if (lua_gettop(state) < 2 || !lua_isstring(state, 2))
		throw RBX::runtime_error("Property name must be a string");

	size_t length = 0;
	const char* text = lua_tolstring(state, 2, &length);
	std::string property(text, length);
	if (property.empty() || !findPropertyDescriptor(property.c_str()))
		throw RBX::runtime_error("%s is not a valid property name", property.c_str());

	Lua::EventInstance event;
	event.descriptor = getPropertyChangedEventDescriptor();
	event.source = shared_from(this);
	// The signal bridge's filtered-event path also removes the Changed event's
	// property-name argument, matching GetPropertyChangedSignal's zero-argument
	// callback contract.
	event.attributeFilter = property;
	Lua::EventBridge::pushNewObject(state, event);
	return 1;
}
...
std::string readInstanceTag(lua_State* state)
{
	if (lua_gettop(state) < 2 || !lua_isstring(state, 2))
		throw RBX::runtime_error("Tag must be a string");
	size_t length = 0;
	const char* text = lua_tolstring(state, 2, &length);
	std::string tag(text, length);
	if (tag.empty() || tag.size() > 100)
		throw RBX::runtime_error("Tag must be between 1 and 100 characters");
	return tag;
}

CollectionService* getScriptCollectionService(lua_State* state)
{
	DataModel* dataModel = DataModel::get(&ScriptContext::getContext(state));
	return dataModel ? dataModel->create<CollectionService>() : NULL;
}

}

int Instance::addTagLua(lua_State* state)
{
	std::string tag = readInstanceTag(state);
	if (CollectionService* service = getScriptCollectionService(state))
		service->addTag(shared_from(this), tag);
	else
		addTagInternal(tag);
	return 0;
}

int Instance::removeTagLua(lua_State* state)
{
	std::string tag = readInstanceTag(state);
	if (CollectionService* service = getScriptCollectionService(state))
		service->removeTag(shared_from(this), tag);
	else
		removeTagInternal(tag);
	return 0;
}

int Instance::hasTagLua(lua_State* state)
{
	lua_pushboolean(state, hasTagInternal(readInstanceTag(state)));
	return 1;
}

int Instance::getTagsLua(lua_State* state)
{
	lua_createtable(state, static_cast<int>(getTagsInternal().size()), 0);
	int index = 1;
	for (const std::string& tag : getTagsInternal())
	{
...
static void pushLuaValue(RBX::Reflection::ConstProperty p, lua_State *L, RBX::Security::Context& securityContext)
{
	...
	// Enum descriptors use integer storage internally, but Luau observes an
	// EnumItem. Resolve that reflected contract before primitive type dispatch.
	if (desc.bIsEnum)
	{
		const EnumPropertyDescriptor* enumDesc =
			static_cast<const EnumPropertyDescriptor*>(&p.getDescriptor());
	...
	if (desc.type==Type::singleton<int>())
	{
		...
		{
			throw RBX::runtime_error("Unable to query integer property %s: %s",
				p.getName().c_str(), error.what());
		}
		return;
	}

	if (desc.type==Type::singleton<long long>())
	{
		lua_pushnumber(L, static_cast<double>(p.getValue<long long>()));
		return;
	}
	...
	if (desc.type==Type::singleton<Content>())
	{
		ContentBridge::pushContent(L, p.getValue<Content>());
		return;
	}

	if (desc.type==Type::singleton<DateTime>())
	{
		DateTimeBridge::pushDateTime(L, p.getValue<DateTime>());
		return;
	}

	if (desc.type==Type::singleton<Font>())
	{
		FontBridge::pushFont(L, p.getValue<Font>());
		return;
	}
	...
	if (desc.type==Type::singleton<Path2DControlPoint>())
	{
		Path2DControlPointBridge::pushNewObject(L, p.getValue<Path2DControlPoint>());
		return;
	}
...
namespace RBX { namespace Lua {
	...
	int Bridge< shared_ptr<Instance>, false >::on_index(const shared_ptr<Instance>& object, const char* name, lua_State *L)
	{
		...
		if (Reflection::YieldFunctionDescriptor* func = object->findYieldFunctionDescriptor(name))
		{
			...
			lua_rawget(L, LUA_REGISTRYINDEX);       // Stack:   closure?
			if (lua_isnil(L, -1)) {
				lua_pop(L, 1);	                    // Stack:   
				...
				lua_settable(L, LUA_REGISTRYINDEX); // Stack:   closure
			}
        ...
        if (name[0] != '\0' && std::islower(static_cast<unsigned char>(name[0])))
        {
            // Preserve the supported lower-camel member aliases by checking
            // the candidate against this instance. The old global Name-table
            // guard incorrectly rejected aliases such as Instance.name as soon
            // as an unrelated script interned the lowercase word "name".
            std::string name2 = name;
            name2[0] = static_cast<char>(
                std::toupper(static_cast<unsigned char>(name[0])));
            const bool memberExists =
                object->findPropertyDescriptor(name2.c_str()) ||
                object->findYieldFunctionDescriptor(name2.c_str()) ||
                object->findFunctionDescriptor(name2.c_str()) ||
                object->findSignalDescriptor(name2.c_str()) ||
                object->findCallbackDescriptor(name2.c_str()) ||
                object->findFirstChildByName(name2.c_str());
            if (memberExists)
            {
...
template<>
int Bridge< shared_ptr<Reflection::DescribedBase>, false >::on_index(
    const shared_ptr<Reflection::DescribedBase>& object, const char* name,
    lua_State* L)
{
    if (!object)
        throw std::runtime_error("The object has been deleted");
    RBX::Security::Context& securityContext = RBX::Security::Context::current();
    if (PropertyDescriptor* property = object->findPropertyDescriptor(name))
    {
        pushLuaValue(Property(*property, object.get()), L, securityContext);
        return 1;
    }
    throw RBX::runtime_error("%s is not a valid member of %s", name,
        object->getDescriptor().name.c_str());
}

template<>
void Bridge< shared_ptr<Reflection::DescribedBase>, false >::on_newindex(
    shared_ptr<Reflection::DescribedBase>& object, const char* name, lua_State*)
{
    if (!object)
        throw std::runtime_error("The object has been deleted");
    throw RBX::runtime_error("%s is read only on %s", name,
        object->getDescriptor().name.c_str());
}

template<>
int Bridge< shared_ptr<Reflection::DescribedBase>, false >::on_tostring(
    const shared_ptr<Reflection::DescribedBase>& object, lua_State* L)
{
    lua_pushstring(L, object ? object->getDescriptor().name.c_str() : "nil");
    return 1;
}
...
static void assignLuaValue(RBX::Reflection::Property p, lua_State *L, int index, RBX::Security::Context& securityContext)
{
	...
	// Enum properties share integer storage with legacy reflection, but their
	// script surface accepts EnumItem (plus the historical numeric/string forms).
	if (const EnumPropertyDescriptor* enumDesc =
		dynamic_cast<const EnumPropertyDescriptor*>(&p.getDescriptor()))
	{
		EnumDescriptorItemPtr item;
		if (EnumItem::getItem(L, index, item))
		{
			if (!enumDesc->setEnumItem(p.getInstance(), *item))
				throw RBX::runtime_error("Invalid type %s for enum %s",
					item->owner.name.c_str(), enumDesc->name.c_str());
		}
		else if (lua_isnumber(L, index))
		{
			if (!enumDesc->setEnumValue(p.getInstance(), lua_tointeger(L, index)))
				throw RBX::runtime_error("Invalid value %d for enum %s",
					static_cast<int>(lua_tointeger(L, index)), enumDesc->name.c_str());
		}
		else if (lua_isstring(L, index))
		{
			if (!enumDesc->setStringValue(p.getInstance(), lua_tostring(L, index)))
				throw RBX::runtime_error("Invalid value \"%s\" for enum %s",
					lua_tostring(L, index), enumDesc->name.c_str());
		}
		else
			throw RBX::runtime_error("Invalid value for enum %s", enumDesc->name.c_str());
		return;
	...
	if (desc.type==Type::singleton<long long>())
	{
		p.setValue<long long>(static_cast<long long>(lua_tonumber(L, index)));
		return;
	}
	...
	if (desc.type==Type::singleton<Path2DControlPoint>())
	{
		p.setValue<Path2DControlPoint>(Path2DControlPointBridge::getObject(L, index));
		return;
	}

	if (desc.type==Type::singleton<Content>())
	{
		p.setValue<Content>(ContentBridge::getObject(L, index));
		return;
	}

	if (desc.type==Type::singleton<Font>())
	{
		p.setValue<Font>(FontBridge::getObject(L, index));
		return;
	}
...
#endif
		const ScriptContext::Result result =
			ScriptContext::getContext(callbackThread.get()).resume(callbackThread, nargs);
#if !defined(RBX_LUAU_VM)
		if (pDebugger)
...
void Bridge< shared_ptr<Instance>, false >::on_newindex(shared_ptr<Instance>& object, const char* name, lua_State *L)
{
	...
	if (PropertyDescriptor* prop = object->findPropertyDescriptor(name))
	{
		...
		else
		{
			...
			{
				throw RBX::runtime_error("Unable to assign %s.%s on %s: %s",
					instance->getFullName().c_str(), prop->name.c_str(),
					instance->getDescriptor().name.c_str(), error.what());
			}
		}
		return;
	}
