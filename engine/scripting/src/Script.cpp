...
{
	template<>
	EnumDesc<Enums::RunContext>::EnumDesc()
		: EnumDescriptor("RunContext")
	{
		addPair(Enums::RunContextLegacy, "Legacy");
		addPair(Enums::RunContextServer, "Server");
		addPair(Enums::RunContextClient, "Client");
		addPair(Enums::RunContextPlugin, "Plugin");
	}
}
...
BaseScript::BaseScript()
	...
	,runContext(Enums::RunContextLegacy)
{
	setName(sBaseScript);
}
...
void BaseScript::setRunContext(Enums::RunContext value)
{
	if (Instance::fastDynamicCast<LocalScript>(this) &&
		value != Enums::RunContextLegacy)
		return;
	if (runContext == value)
		return;
	...
	runContext = value;
	restartScript();
	raisePropertyChanged(prop_RunContext);
}
