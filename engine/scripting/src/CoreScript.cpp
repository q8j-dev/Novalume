...
namespace RBX
{
	namespace
	{
		std::mutex packagedSourcesMutex;
		std::map<std::string, ProtectedString> packagedSources;
	}
    ...
    {
		{
			std::lock_guard<std::mutex> lock(packagedSourcesMutex);
			std::map<std::string, ProtectedString>::const_iterator found =
				packagedSources.find(name);
			if (found != packagedSources.end())
				return found->second;
		}
    ...
    bool CoreScript::hasPackagedSources()
    {
		std::lock_guard<std::mutex> lock(packagedSourcesMutex);
		return !packagedSources.empty();
	}

	void CoreScript::installPackagedSources(
		const std::map<std::string, ProtectedString>& sources)
	{
		std::lock_guard<std::mutex> lock(packagedSourcesMutex);
		packagedSources = sources;
	}
