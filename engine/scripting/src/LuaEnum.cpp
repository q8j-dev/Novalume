...
static int pushEnums(lua_State* L)
{
    lua_createtable(L, static_cast<int>(Reflection::EnumDescriptor::allEnumSize()), 0);
    int position = 1;
    for (std::vector<const Reflection::EnumDescriptor*>::const_iterator it =
             Reflection::EnumDescriptor::enumsBegin();
         it != Reflection::EnumDescriptor::enumsEnd(); ++it)
    {
...
{
	if (strcmp(name, "GetEnums") == 0)
	{
		lua_pushcfunction(L, pushEnums);
		return 1;
	}
...
int Bridge<EnumDescriptorItemPtr, false>::on_index(const EnumDescriptorItemPtr& object, const char* name, lua_State *L)
{
	...
	if (strcmp(name, "EnumType")==0)
	{
		Enum::push(L, &object->owner);
		return 1;
	}
