...
namespace RBX {
	...
	R withVariantValue(const Reflection::Variant& value, F f)
	{
		...
		if (value.isType<Content>())
			return f(value.cast<Content>());
		if (value.isType<Font>())
			return f(value.cast<Font>());
		if (value.isType<RaycastParams>())
			return f(value.cast<RaycastParams>());
		if (value.isType<OverlapParams>())
			return f(value.cast<OverlapParams>());
		if (value.isType<RaycastResult>())
			return f(value.cast<RaycastResult>());
