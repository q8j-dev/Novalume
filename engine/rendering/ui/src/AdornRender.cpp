...
{
public:
    TextureProxy(const TextureRef& texture, const Vector2& requestedSize = Vector2::zero())
        : texture(texture)
    ...
    virtual G3D::Vector2 getOriginalSize()
    {
		...
		return requestedSize != Vector2::zero() ? requestedSize : G3D::Vector2(info.width, info.height);
    }
...
{
    const boost::shared_ptr<Typesetter>& typesetter = visualEngine->getTypesetter(font);
...
TextureProxyBaseRef AdornRender::createTextureProxy(const ContentId& id, bool& waiting, bool bBlocking, const std::string& context)
{
    if (id.isRbxThumb())
    {
        ThumbnailSceneRequest thumbnailRequest;
        boost::shared_ptr<ThumbnailSceneProvider> provider =
            visualEngine->getThumbnailSceneProvider();
        ViewportTextureRequest viewportRequest{};
        if (!parseThumbnailSceneRequest(id, thumbnailRequest) || !provider)
        {
            waiting = false;
            return TextureProxyBaseRef();
        }
        if (!provider->resolveThumbnailScene(thumbnailRequest, viewportRequest))
        {
            waiting = true;
            return TextureProxyBaseRef();
        }
        viewportRequest.cacheKey = thumbnailRequest.cacheKey;
        waiting = false;
        return requestViewportTexture(viewportRequest);
    }
...
TextureProxyBaseRef AdornRender::requestViewportTexture(const ViewportTextureRequest& request)
{
    ViewportTextureRequest renderRequest = request;
    Framebuffer* mainFramebuffer = visualEngine->getDevice()->getMainFramebuffer();
    if (mainFramebuffer && visualEngine->getViewWidth() > 0 && visualEngine->getViewHeight() > 0)
    {
        renderRequest.size.x *= static_cast<float>(mainFramebuffer->getWidth()) / visualEngine->getViewWidth();
        renderRequest.size.y *= static_cast<float>(mainFramebuffer->getHeight()) / visualEngine->getViewHeight();
    }

    TextureRef texture = visualEngine->getViewportRenderer()->request(renderRequest);
    return texture.getStatus() == TextureRef::Status_Null
        ? TextureProxyBaseRef()
        : TextureProxyBaseRef(new TextureProxy(texture, request.size));
}

bool AdornRender::requiresViewportTextureFlipping() const
{
    return visualEngine->getDevice()->getCaps().requiresRenderTargetFlipping;
}

TextureProxyBaseRef AdornRender::requestMediaTexture(const void* owner,
    std::uint64_t generation, unsigned int width, unsigned int height,
    const std::uint8_t* rgba, std::size_t byteCount)
{
    if (!owner || !rgba || width == 0 || height == 0 ||
        byteCount != static_cast<std::size_t>(width) * height * 4)
        return TextureProxyBaseRef();

    MediaTexture& entry = mediaTextures[owner];
    if (!entry.texture || entry.width != width || entry.height != height)
    {
        entry.texture = visualEngine->getDevice()->createTexture(Texture::Type_2D,
            Texture::Format_RGBA8, width, height, 1, 1, Texture::Usage_Dynamic);
        entry.width = width;
        entry.height = height;
        entry.generation = 0;
    }
    ...
    {
        entry.texture->upload(0, 0, TextureRegion(0, 0, width, height), rgba,
            static_cast<unsigned int>(byteCount));
        entry.generation = generation;
    }
    entry.lastFrame = frameNumber;
    return TextureProxyBaseRef(new TextureProxy(TextureRef(entry.texture),
...
void AdornRender::convexPolygon2d(const Vector2* v, const Color4* colors, int countv)
{
    Vector2* vertices = (Vector2*)alloca(sizeof(Vector2) * countv);
    for (int i = 0; i < countv; ++i)
        vertices[i] = Vector2(v[i].x, currentHeight - v[i].y);
    ...
    visualEngine->getVertexStreamer()->triangleList2d(colors, vertices, countv, indices, icount);
}

void AdornRender::texturedPolygon2d(const Vector2* v, const Vector2* uv, const Color4* colors, int countv)
{
    Vector2* vertices = (Vector2*)alloca(sizeof(Vector2) * countv);
    for (int i = 0; i < countv; ++i)
        vertices[i] = Vector2(v[i].x, currentHeight - v[i].y);
    ...
    visualEngine->getVertexStreamer()->texturedTriangleList2d(currentTexture, colors, vertices, uv,
        countv, indices, icount, currentTextureType, getIgnoreTexture());
}
...
void AdornRender::convexPolygon(const Vector3* v, const Color4* colors, int countv)
{
    short* indices = (short*)alloca(sizeof(short) * (countv - 2) * 3);
    int icount = 0;
    for (int i = 1; i < countv - 1; ++i)
    ...
    visualEngine->getVertexStreamer()->triangleList(colors, currentCFrame, v, countv, indices, icount);
}

void AdornRender::texturedPolygon(const Vector3* v, const Vector2* uv, const Color4* colors, int countv)
{
    short* indices = (short*)alloca(sizeof(short) * (countv - 2) * 3);
    int icount = 0;
    for (int i = 1; i < countv - 1; ++i)
    ...
    visualEngine->getVertexStreamer()->texturedTriangleList(currentTexture, colors, currentCFrame, v, uv,
        countv, indices, icount, currentTextureType);
}
...
{
	++frameNumber;
	for (std::map<const void*, MediaTexture>::iterator it = mediaTextures.begin();
		it != mediaTextures.end(); )
	{
		if (frameNumber - it->second.lastFrame > 120)
			it = mediaTextures.erase(it);
		else
			++it;
	}
