...
namespace Graphics
{
    ...
    bool TextureAtlas::getRegion(const std::uint64_t& id, const TextureRegion*& region)
    {
    ...
    void TextureAtlas::upload()
    {
        ...
        lastFailedSize = G3D::Vector2int16(0x7FFF, 0x7FFF);

        if (updateLineY >= UINT_MAX - DFInt::TexAtlasUpdateLineHeight)
            reset();
    }

bool TextureAtlas::insert(const std::uint64_t& id, const G3D::Vector2int16& size, unsigned dataPitch, unsigned char* data, const TextureRegion*& outRegion)
    {
        if (getRegion(id, outRegion))
            return true;

        G3D::Vector2int16 realSize = size + G3D::Vector2int16(2,2);
