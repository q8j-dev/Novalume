...
bool TextureManager::loadAsync(const ContentId& id, const std::string& context)
{
	...
	// File-backed captures are user content, not permanently packaged assets.
	// Keep the same bounded decode policy as other user content so opening the
	// gallery cannot consume a full-resolution framebuffer per thumbnail.
    ...
    try
	{
		...
		else if (loadId.isFile())
		{
			ContentProvider* cp = visualEngine->getContentProvider();
			const std::string path = cp
				? ContentProvider::resolveLocalContent(loadId)
				: std::string();
			if (path.empty())
				throw RBX::runtime_error("Local content is outside the capture gallery sandbox");
			outstandingRequests++;
			loadingPool->schedule(boost::bind(&TextureManager::loadImageLocalContent,
				pendingImages, id, path, maxTextureSize, flags, context));
			return true;
		}
		else if (loadId.isHttp() || loadId.isAssetId() || loadId.isRbxHttp() || loadId.isNamedAsset())
        {
			...
			{
                boost::function<void (boost::shared_ptr<const std::string>)> loadCallback =
					boost::bind(&TextureManager::loadImageHttpCallback, boost::weak_ptr<ThreadPool>(loadingPool), boost::weak_ptr<rbx::safe_queue<LoadedImage> >(pendingImages), _1, id, maxTextureSize, flags, context);
...
{
	std::shared_ptr<Texture> result = visualEngine->getDevice()->createTexture(cube ? Texture::Type_Cube : Texture::Type_2D, Texture::Format_RGBA8, 1, 1, 1, 1, Texture::Usage_Static);
    RBXASSERT(result);
...
{
    std::shared_ptr<Texture> texture = visualEngine->getDevice()->createTexture(
...
void TextureManager::loadImageHttpCallback(const boost::weak_ptr<ThreadPool>& loadingPoolWeak, const boost::weak_ptr<rbx::safe_queue<LoadedImage> >& pendingImagesWeak, const boost::shared_ptr<const std::string>& content, const ContentId& id, unsigned int maxTextureSize, unsigned int flags, const std::string& context)
{
	boost::shared_ptr<ThreadPool> loadingPool = loadingPoolWeak.lock();
	boost::shared_ptr<rbx::safe_queue<LoadedImage> > pendingImages = pendingImagesWeak.lock();
...
void TextureManager::loadImageFile(const boost::shared_ptr<rbx::safe_queue<LoadedImage> >& pendingImages, const ContentId& id, const ContentId& loadId, unsigned int maxTextureSize, unsigned int flags, bool useRetina, const std::string& context)
{
...
void TextureManager::loadImageLocalContent(
	const boost::shared_ptr<rbx::safe_queue<LoadedImage> >& pendingImages,
	const ContentId& id, const std::string& path, unsigned int maxTextureSize,
	unsigned int flags, const std::string& context)
{
	std::ifstream stream(utf8_decode(path).c_str(),
	...
	loadImage(pendingImages, id, stream, maxTextureSize, flags, 1, context);
}

void TextureManager::loadImageHttp(const boost::shared_ptr<rbx::safe_queue<LoadedImage> >& pendingImages, const ContentId& id, const boost::shared_ptr<const std::string>& content, unsigned int maxTextureSize, unsigned int flags, const std::string& context)
{
...
void TextureManager::loadImage(const boost::shared_ptr<rbx::safe_queue<LoadedImage> >& pendingImages, const ContentId& id, std::istream& stream, unsigned int maxTextureSize, unsigned int flags, int scale, const std::string& context)
{
    try
	{
		Timer<Time::Precise> timer;
...
void TextureManager::loadImageError(const boost::shared_ptr<rbx::safe_queue<LoadedImage> >& pendingImages, const ContentId& id, const char* error, const std::string& context)
{
...
bool TextureManager::isFallbackTexture(const std::shared_ptr<Texture>& tex)
{
