...
std::shared_ptr<Texture> TextureRef::emptyTexture;
ImageInfo TextureRef::emptyInfo;

TextureRef TextureRef::future(const std::shared_ptr<Texture>& fallback)
{
    Data* data = new Data();
...
TextureRef::TextureRef(const std::shared_ptr<Texture>& texture, Status status)
    : data(new Data())
{
    if (texture)
	{
...
void TextureRef::updateAllRefs(const std::shared_ptr<Texture>& texture)
{
    RBXASSERT(data);
...
void TextureRef::updateAllRefsToLoaded(const std::shared_ptr<Texture>& texture)
{
    RBXASSERT(texture);
    RBXASSERT(data);
...
void TextureRef::updateAllRefsToLoaded(const std::shared_ptr<Texture>& texture, const ImageInfo& info)
{
    RBXASSERT(texture);
    RBXASSERT(data);
	RBXASSERT(data->status == Status_Waiting);
