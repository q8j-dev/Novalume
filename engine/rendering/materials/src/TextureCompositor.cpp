...
public:
    ...
    const shared_ptr<GeometryBatch>* requestMesh(const MeshId& meshId)
	{
        ...
        boost::shared_ptr<FileMeshData> cachedMesh = boost::static_pointer_cast<FileMeshData>(mcp->requestContent(meshId, ContentProvider::PRIORITY_MESH, false, result));
...
TextureCompositor::JobHandle TextureCompositor::getJob(const std::string& textureid, const std::string& context, unsigned int width, unsigned int height, const Vector2& canvasSize, const std::vector<TextureCompositorLayer>& layers)
{
    ...
    std::shared_ptr<Job>& job = jobs[textureid];
    if (job) return job;
    
    // look for an orphaned job with the same id
    for (size_t i = 0; i < orphanedJobs.size(); ++i)
    {
        const std::shared_ptr<Job>& orphaned = orphanedJobs[i];
...
{
    std::set<std::shared_ptr<Job> > newOrphanedJobs;
    ...
    if (!newOrphanedJobs.empty())
    {
		...
		ExistsInSetPredicate<std::shared_ptr<Job> > isOrphaned(&newOrphanedJobs);
...
void TextureCompositor::garbageCollectOrphanedJobs()
{
    ...
    orphanedJobs.erase(std::remove_if(orphanedJobs.begin(), orphanedJobs.end(), TextureIsNullPredicate<std::shared_ptr<Job> >()), orphanedJobs.end());
}
...
void TextureCompositor::findRebakeTargetAndEnqueue()
{
    ...
    std::vector<std::shared_ptr<Job> > sortedJobs;
    sortedJobs.reserve(jobs.size());
    
    for (JobMap::const_iterator it = jobs.begin(); it != jobs.end(); ++it)
        sortedJobs.push_back(it->second);
        
    std::sort(sortedJobs.begin(), sortedJobs.end(), PriorityComparator<std::shared_ptr<Job> >());
...
void TextureCompositor::update(const Vector3& pointOfInterest)
{
    ...
    if (pendingJobs.size() > 0 && activeJobs.size() < kTextureCompositorActiveJobs)
    {
        ...
        std::nth_element(pendingJobs.begin(), pendingJobs.begin() + count, pendingJobs.end(), PriorityComparator<std::shared_ptr<Job> >());
    ...
    std::sort(activeJobs.begin(), activeJobs.end(), PriorityComparator<std::shared_ptr<Job> >());
...
{
    std::shared_ptr<Job> textureJob(new Job());
