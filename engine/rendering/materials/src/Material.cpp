...
Technique::Technique(const std::shared_ptr<ShaderProgram>& program, unsigned int lodIndex, RenderQueue::Pass pass)
	: lodIndex(lodIndex)
    , pass(pass)
...
void Technique::apply(DeviceContext* context) const
{
    ...
    for (size_t i = 0; i < textures.size(); ++i)
	{
		...
		const std::shared_ptr<Texture>& tex = tu.texture.getTexture();
