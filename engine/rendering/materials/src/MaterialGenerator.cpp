...
MaterialGenerator::Result MaterialGenerator::createMaterialForPart(PartInstance* part, const HumanoidIdentifier* hi, unsigned int flags)
{
    ...
    {
        const TextureId& textureId = meshPart->getTextureId();
        TextureRef texture = textureId.isNull() ? TextureRef() :
            visualEngine->getTextureManager()->load(textureId,
                TextureManager::Fallback_Gray, meshPart->getFullName() + ".TextureID");
