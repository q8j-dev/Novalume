...
public:
	...
	Adorn(): userGuiInset(Vector4::zero()), deviceGuiInset(Vector4::zero()),
		ignoreTexture(false), vr(false), currentMaterial(Material_Default) {}
	...
	void setDeviceGuiInset(const Vector4& value) { deviceGuiInset = value; }
	...
	Rect2D getDeviceGuiRect() const
	{
		Rect2D vp = getViewport();
		return Rect2D::xyxy(vp.x0() + deviceGuiInset.x, vp.y0() + deviceGuiInset.y,
			vp.x1() - deviceGuiInset.z, vp.y1() - deviceGuiInset.w);
	}
	...
	virtual void convexPolygon(
		...
		int countv,
		const Color4&		color) = 0;
	virtual void convexPolygon(const Vector3* v, const Color4* colors, int countv) = 0;
	virtual void texturedPolygon(const Vector3* v, const Vector2* uv, const Color4* colors, int countv) = 0;
	...
	virtual void convexPolygon2d(
		...
		int countv,
		const Color4&		color) = 0;
	virtual void convexPolygon2d(const Vector2* v, const Color4* colors, int countv) = 0;
	virtual void texturedPolygon2d(const Vector2* v, const Vector2* uv, const Color4* colors, int countv) = 0;
