...
{	
	...
	bool GfxBinding::isInRenderRoot(RBX::Instance* part) const
	{
		if (boost::shared_ptr<Instance> root = renderRoot.lock())
			return part == root.get() || part->isDescendantOf(root.get());

		return isInWorkspace(part);
	}
	...
	void GfxBinding::onAncestorChanged(const shared_ptr<Instance>& ancestor)
	{
		...
		if (partInstance && !isInRenderRoot(partInstance.get()))
		{
	...
	void GfxBinding::onPropertyChanged(const Reflection::PropertyDescriptor* descriptor)
	{
		...
		else if (*descriptor==PartInstance::prop_CastShadow) {
			invalidateEntity();
		}
	...
	void GfxBinding::onTexturePropertyChanged(const Reflection::PropertyDescriptor* descriptor)
	{
		...
		else if (*descriptor==DecalTexture::prop_Texture)
		{
			...
			invalidateEntity();
		else if (*descriptor==DecalTexture::prop_OffsetStudsU)
			invalidateEntity();
		else if (*descriptor==DecalTexture::prop_OffsetStudsV)
			invalidateEntity();
