...
void IAdornableCollector::render2dItems(Adorn* adorn)
{
	...
	std::vector<IAdornable*> ordered;
	ordered.reserve(renderable2ds.size());
	for (int i = 0; i < renderable2ds.size(); ++i)
		ordered.push_back(renderable2ds[i]);
	std::stable_sort(ordered.begin(), ordered.end(), [](IAdornable* left, IAdornable* right) {
		return left->render2dOrder() < right->render2dOrder();
	});
	for (IAdornable* item : ordered)
	{
		item->render2d(adorn);
	}
}
