...
void AdornBillboarder::convexPolygon2d(const Vector2* v, const Color4* colors, int countv)
{
#ifdef _WIN32
	Vector3* v3d = (Vector3*)_alloca(sizeof(Vector3) * countv);
#else
...
#endif
	for (int i = 0; i < countv; ++i)
		v3d[i] = Vector3(v[i].x, -v[i].y, 0);
	parent->convexPolygon(v3d, colors, countv);
}

void AdornBillboarder::texturedPolygon2d(const Vector2* v, const Vector2* uv, const Color4* colors, int countv)
{
#ifdef _WIN32
	Vector3* v3d = (Vector3*)_alloca(sizeof(Vector3) * countv);
#else
	...
	for (int i = 0; i < countv; ++i)
		v3d[i] = Vector3(v[i].x, -v[i].y, 0);
	parent->texturedPolygon(v3d, uv, colors, countv);
}
