...
void FrameRateManager::synchronizeQualitySettings()
{
	if (!mSettings)
		return;

	int requested = mSettings->getEnableFRM()
		? static_cast<int>(mSettings->getQualityLevel())
		: static_cast<int>(mSettings->getEditQualityLevel());
	if (requested == CRenderSettings::QualityAuto)
		requested = mSettings->getAutoQualityLevel();
	requested = std::max(1, std::min(requested,
		static_cast<int>(CRenderSettings::QualityLevelMax) - 1));
	if (requested != mCurrentQualityLevel)
	{
		mCurrentQualityLevel = requested;
