...
SceneUpdater::SceneUpdater(boost::shared_ptr<RBX::DataModel> dataModel, VisualEngine* ve,
	const boost::shared_ptr<RBX::Instance>& requestedRenderRoot, SceneManager* requestedSceneManager,
	const void* renderWorld)
    : dataModel(dataModel)
	, renderRoot(requestedRenderRoot ? requestedRenderRoot : shared_from(dataModel->getWorkspace()))
	, sceneManager(requestedSceneManager ? requestedSceneManager : ve->getSceneManager())
	...
	, computeLightingEnabled(!requestedRenderRoot)
{
	...
	renderRoot->visitDescendants(boost::bind(&SceneUpdater::onWorkspaceDescendantAdded, this, _1));
	workspaceDescendantAddedConnection = renderRoot->onDemandWrite()->descendantAddedSignal.connect(boost::bind(&SceneUpdater::onWorkspaceDescendantAdded, this, _1));
...
bool SceneUpdater::contains(const RBX::Instance* instance) const
{
	return instance && (instance == renderRoot.get() || instance->isDescendantOf(renderRoot.get()));
}
...
void SceneUpdater::onWorkspaceDescendantAdded(boost::shared_ptr<RBX::Instance> descendant)
{
...
void SceneUpdater::processPendingAttachments()
{
	...
	}
	{
		...
		{
			boost::shared_ptr<Instance> p = it->second.lock();

			if (p && contains(p.get())) // part could have disappeared before render ever happened.
			{
				addAttachment(p);
			}
		}
...
void SceneUpdater::processPendingMegaClusters()
{
	...
	{
		boost::shared_ptr<RBX::PartInstance> part = it->second.lock();
		if(part)
			addMegaCluster(part);
	}
}
...
void SceneUpdater::processPendingParts(bool priorityParts)
{
	...
	for(PartInstanceSet::iterator it = localCopy.begin(); it != localCopy.end(); ++it)
	{
		boost::shared_ptr<PartInstance> p = it->second.lock();
...
void SceneUpdater::addMegaCluster(const boost::shared_ptr<RBX::PartInstance>& part)
{
	boost::shared_ptr<MegaClusterInstance> terrain = shared_polymorphic_downcast<MegaClusterInstance>(part);
...
void SceneUpdater::addFastPart(const boost::shared_ptr<RBX::PartInstance>& part, bool isFW, bool priorityPart)
{
    if (RBX::Humanoid* humanoid = getHumanoid(part.get()))
    {
        ...
        if (!cluster)
            cluster = new FastCluster(mVisualEngine, this, humanoid, NULL, false);
    ...
    else
    {
        ...
        if (!cluster)
            cluster = new SuperCluster(mVisualEngine, this, mFastGridSC.get(), index, isFW);
...
void SceneUpdater::addAttachment(const boost::shared_ptr<RBX::Instance>& instance)
{
	PartInstance* part = 0;
	...
	{
			if (boost::shared_ptr<RBX::Light> light = Instance::fastSharedDynamicCast<RBX::Light>(instance))
		{
