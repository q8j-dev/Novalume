...
void VertexStreamer::renderInternal(DeviceContext* context, CoordinateSpace coordinateSpace, const RenderCamera& camera, RenderPassStats& stats, int renderIndex)
{
    ...
    context->setRasterizerState(RasterizerState(
        coordinateSpace == CS_ScreenSpace ? RasterizerState::Cull_None
...
void VertexStreamer::triangleList2d(const Color4* colors, const Vector2* v, int vcount, const short* indices, int icount)
{
    if (prepareChunk(shared_ptr<Texture>(), Geometry::Primitive_Triangles, icount, CS_ScreenSpace, BatchTextureType_Color))
    {
        RBXASSERT(icount % 3 == 0);
        for (int i = 0; i < icount; ++i)
        {
            ...
            vertexData.push_back(Vertex(Vector3(v[index], 0), packColor(colors[index], colorOrderBGR), Vector2()));
        }
    }
}

void VertexStreamer::texturedTriangleList2d(const shared_ptr<Texture>& texture, const Color4* colors,
    const Vector2* v, const Vector2* uv, int vcount, const short* indices, int icount,
    BatchTextureType batchTexType, bool ignoreTexture)
{
    if (prepareChunk(texture, Geometry::Primitive_Triangles, icount, CS_ScreenSpace, batchTexType, ignoreTexture))
    {
        RBXASSERT(icount % 3 == 0);
        for (int i = 0; i < icount; ++i)
        {
            const int index = indices[i];
            vertexData.push_back(Vertex(Vector3(v[index], 0), packColor(colors[index], colorOrderBGR), uv[index]));
        }
    }
}
...
void VertexStreamer::triangleList(const Color4* colors, const CoordinateFrame& cframe, const Vector3* v, int vcount, const short* indices, int icount)
{
    if (prepareChunk(shared_ptr<Texture>(), Geometry::Primitive_Triangles, icount, CS_WorldSpace, BatchTextureType_Color))
    {
        RBXASSERT(icount % 3 == 0);
        ...
        {
            const int index = indices[i];
            vertexData.push_back(Vertex(cframe.pointToWorldSpace(v[index]), packColor(colors[index], colorOrderBGR), Vector2()));
        }
    }
}

void VertexStreamer::texturedTriangleList(const shared_ptr<Texture>& texture, const Color4* colors,
    const CoordinateFrame& cframe, const Vector3* v, const Vector2* uv, int vcount,
    const short* indices, int icount, BatchTextureType batchTexType)
{
    if (prepareChunk(texture, Geometry::Primitive_Triangles, icount, CS_WorldSpace, batchTexType))
    {
        RBXASSERT(icount % 3 == 0);
        for (int i = 0; i < icount; ++i)
        {
            const int index = indices[i];
            vertexData.push_back(Vertex(cframe.pointToWorldSpace(v[index]), packColor(colors[index], colorOrderBGR), uv[index]));
        }
    }
}
