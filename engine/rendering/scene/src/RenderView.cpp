...
public:
        ...
        , averageFPS(0)
	{
		...
		static boost::shared_ptr<FrameRateManagerStatsItem> create()
	{
			boost::shared_ptr<FrameRateManagerStatsItem> result = RBX::Creatable<RBX::Instance>::create<FrameRateManagerStatsItem>();
...
void RenderView::bindWorkspace(boost::shared_ptr<RBX::DataModel> dataModel)
{
	...
	if (this->dataModel)
	{
        ...
        visualEngine->bindWorkspace(boost::shared_ptr<DataModel>());
	}
...
void RenderView::renderPerformImpl(double timeJobStart, Framebuffer* mainFramebuffer)
{
    ...
    if (DeviceContext* context = visualEngine->getDevice()->beginFrame())
    {
		...
		visualEngine->getViewportRenderer()->render(context);
...
bool RenderView::saveScreenshotToFile(std::string& /*output*/ filename)
{
	...
	if (framebuffer)
		return saveFramebufferScreenshot(*framebuffer, filename);
#endif
    return false;
}

bool saveFramebufferScreenshot(Framebuffer& framebuffer, std::string& filename)
{
    time_t captureTime;
    time(&captureTime);
    struct tm localCaptureTime;
#if defined(_WIN32)
    localtime_s(&localCaptureTime, &captureTime);
#else
    localtime_r(&captureTime, &localCaptureTime);
#endif

    std::ostringstream name;
    name << "RobloxScreenShot";
    name << std::setw(2) << std::setfill('0') << (localCaptureTime.tm_mon + 1)
         << std::setw(2) << std::setfill('0') << localCaptureTime.tm_mday
         << std::setw(4) << std::setfill('0') << (localCaptureTime.tm_year + 1900)
         << "_" << std::setw(2) << std::setfill('0') << localCaptureTime.tm_hour
         << std::setw(2) << std::setfill('0') << localCaptureTime.tm_min
         << std::setw(2) << std::setfill('0') << localCaptureTime.tm_sec
         << std::setw(3) << std::setfill('0')
         << ((clock() * 1000 / CLOCKS_PER_SEC) % 1000) << ".png";

    const boost::filesystem::path path =
        FileSystem::getUserDirectory(true, RBX::DirPicture) / name.str();
    G3D::GImage image(framebuffer.getWidth(), framebuffer.getHeight(), 4);
    framebuffer.download(image.byte(), image.width() * image.height() * 4);
    image.convertToRGB();

    G3D::BinaryOutput encoded;
    image.encode(G3D::GImage::PNG, encoded);
    std::ofstream output(path.native().c_str(), std::ios::out | std::ios::binary);
    output.write(reinterpret_cast<const char*>(encoded.getCArray()), encoded.length());
    output.close();
    if (!output)
        throw std::runtime_error("could not write rendered screenshot: " + path.string());
