...
RenderEntity* MegaCluster::createSolidGeometry(const SpatialRegion::Id& pos, unsigned int* outQuads)
{
    ...
    *outQuads = quads;
    ...
    return createGeometry(updateChunkNode(pos), vbuf, ibuf, getSolidMaterial(), RenderQueue::Id_OpaqueCasters, false);
}
...
const shared_ptr<Material>& MegaCluster::getSolidMaterial()
{
    ...
    if (shared_ptr<ShaderProgram> program = visualEngine->getShaderManager()->getProgram("MegaClusterShadowVS", "DefaultShadowFS"))
    {
