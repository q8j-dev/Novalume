    ...
    const std::vector<unsigned int>& solidIndices, const std::vector<unsigned int>& waterIndices)
{
	...
	if (!solidIndices.empty())
	{
        ...
        solidEntity = new SmoothClusterRenderEntity(node, GeometryBatch(geometry, Geometry::Primitive_Triangles, ibuf->getElementCount(), vbuf->getElementCount()), material, RenderQueue::Id_OpaqueCasters, &materialConstants, unpackInfo);
	}
...
const shared_ptr<Material>& SmoothClusterBase::getSolidMaterial()
{
    ...
    if (shared_ptr<ShaderProgram> program = visualEngine->getShaderManager()->getProgram("SmoothClusterShadowVS", "DefaultShadowFS"))
    {
...
RenderNode* SmoothClusterChunked::updateChunkNode(const SpatialRegion::Id& pos)
{
    ...
    if (!chunk.node)
    {
        ...
        chunk.node.reset(new RenderNode(visualEngine, RenderNode::CullMode_SpatialHash,
            partInstance->getCastShadow() ? RenderNode::Flags_ShadowCaster : 0));
