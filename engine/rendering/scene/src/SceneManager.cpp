...
{
public:
	ShadowMap(VisualEngine* visualEngine, Texture::Format format,
		Texture::Format depthFormat, int size)
	{
        Device* device = visualEngine->getDevice();
		...
		shared_ptr<Renderbuffer> shadowDepth = device->createRenderbuffer(depthFormat, size, size, 1);
...
, postProcessSettings(PostProcessSettings())
{
	...
	// Allocate multisampling lazily in updateMSAA once the active render size,
	// quality level, and backend sample caps are known.  Eagerly multiplying the
	// native framebuffer dimensions created a persistent 5120x2880 target on a
	// 1280x720 Retina window when the backend reported no MSAA support.
	msaa.reset();
	...
	try
	{
		...
		if (!caps.supportsFramebuffer || !caps.supportsShaders || caps.maxTextureSize < 128)
			throw std::runtime_error("directional shadow maps are unsupported by this graphics device");
		const Texture::Format depthFormat = caps.supportsStencil
			? Texture::Format_D24S8 : Texture::Format_D16;
	#if !defined(RBX_PLATFORM_IOS) && !defined(__ANDROID__)
		shadowMapTexelSize = 0.2f;

		if (caps.maxTextureSize >= 256)
			shadowMaps[0].reset(new ShadowMap(visualEngine, Texture::Format_RGBA8, depthFormat, 256));
		if (caps.maxTextureSize >= 512)
			shadowMaps[1].reset(new ShadowMap(visualEngine, Texture::Format_RGBA8, depthFormat, 512));
		if (caps.maxTextureSize >= 1024)
			shadowMaps[2].reset(new ShadowMap(visualEngine, Texture::Format_RGBA8, depthFormat, 1024));
		if (!shadowMaps[0])
			shadowMaps[0].reset(new ShadowMap(visualEngine, Texture::Format_RGBA8, depthFormat, 128));
	#else
		shadowMapTexelSize = 0.3f;

		shadowMaps[0].reset(new ShadowMap(visualEngine, Texture::Format_RGBA8, depthFormat, 128));
		if (caps.maxTextureSize >= 256)
			shadowMaps[1].reset(new ShadowMap(visualEngine, Texture::Format_RGBA8, depthFormat, 256));
	#endif
	}
...
void SceneManager::renderBegin(DeviceContext* context, const RenderCamera& camera, unsigned int viewWidth, unsigned int viewHeight)
{
	...
	if (shadowMap)
	{
		renderShadowMap(context, shadowMap, camera, frm->GetQualityLevel());
    ...
    else
    {
        ...
        shadowCascadeCount = 0;
        globalShaderData.ShadowCascadeInfo = Vector4(0, 0, 0, 0);
...
void SceneManager::renderView(DeviceContext* context, Framebuffer* mainFramebuffer, const RenderCamera& camera, unsigned int viewWidth, unsigned int viewHeight)
{
    ...
    // UI afterwards so deferred rendering cannot overwrite CoreGui pixels.
    {
		...
		{
			// Render targets use native pixels on Hi-DPI displays, while CoreGui
			// vertices and input use the logical canvas stored by VisualEngine.
			const unsigned int uiWidth = visualEngine->getDevice()->getCaps().retina
				? static_cast<unsigned int>(visualEngine->getViewWidth())
				: viewWidth;
			const unsigned int uiHeight = visualEngine->getDevice()->getCaps().retina
				? static_cast<unsigned int>(visualEngine->getViewHeight())
				: viewHeight;
...
void SceneManager::renderViewport(DeviceContext* context, Framebuffer* framebuffer,
    const RenderCamera& camera, unsigned int viewWidth, unsigned int viewHeight,
    const void* renderWorld, const Color4& background, const Color3& ambient,
    const Color3& lightColor, const Vector3& lightDirection)
{
    RBXPROFILER_SCOPE("GPU", "ViewportFrame");
    RBXPROFILER_SCOPE("Render", "ViewportFrame");
	(void)viewWidth;
	(void)viewHeight;

    GlobalShaderData savedShaderData = globalShaderData;
    const Vector3 savedPointOfInterest = pointOfInterest;

    pointOfInterest = camera.getPosition();
    globalShaderData.setCamera(camera);
    globalShaderData.AmbientColor = Color4(ambient);
    globalShaderData.Lamp0Color = Color4(lightColor);
    globalShaderData.Lamp0Dir = Vector4(
        lightDirection.squaredMagnitude() > 1e-8f ? lightDirection.unit() : Vector3(0, -1, 0), 0);
    globalShaderData.Lamp1Color = Color4::clear();
    globalShaderData.OutlineBrightness_ShadowInfo.w = 0;
    // Shader fog is expressed as (end - clipDepth) * scale.  A zero scale is
    // fully fogged, not disabled, so use a distant end plane for ViewportFrame.
    globalShaderData.FogParams = Vector4(0, 0, 1e9f, 1.f);
    context->updateGlobalConstants(&globalShaderData, sizeof(globalShaderData));

    context->bindFramebuffer(framebuffer);
    context->clearFramebuffer(DeviceContext::Buffer_Color | DeviceContext::Buffer_Depth |
        DeviceContext::Buffer_Stencil, &background.r, 1.f, 0);

    RenderQueue viewportQueue;
    std::vector<CullableSceneNode*> nodes;
    spatialHashedScene->queryFrustumOrdered(nodes, camera, pointOfInterest,
        visualEngine->getFrameRateManager(), renderWorld);
    for (CullableSceneNode* node: nodes)
        node->updateRenderQueue(viewportQueue, camera, RenderQueue::Pass_Default);

    RenderPassStats& stats = visualEngine->getRenderStats()->passScene;
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_Opaque),
        RenderQueueGroup::Sort_Material, stats, "ViewportOpaque");
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_OpaqueDecals),
        RenderQueueGroup::Sort_Material, stats, "ViewportOpaqueDecals");
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_OpaqueCasters),
        RenderQueueGroup::Sort_Material, stats, "ViewportOpaqueCasters");
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_TransparentDecals),
        RenderQueueGroup::Sort_Material, stats, "ViewportTransparentDecals");
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_TransparentUnsorted),
        RenderQueueGroup::Sort_Material, stats, "ViewportTransparentUnsorted");
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_Transparent),
        RenderQueueGroup::Sort_Distance, stats, "ViewportTransparent");
    renderObjects(context, viewportQueue.getGroup(RenderQueue::Id_TransparentCasters),
        RenderQueueGroup::Sort_Distance, stats, "ViewportTransparentCasters");

    globalShaderData = savedShaderData;
    pointOfInterest = savedPointOfInterest;
    context->updateGlobalConstants(&globalShaderData, sizeof(globalShaderData));
}
...
void SceneManager::setShadowMapConfiguration(bool enabled, float softness)
{
...
void SceneManager::renderShadowMap(DeviceContext* context, ShadowMap* shadowMap,
    const RenderCamera& camera, int qualityLevel)
{
    RBXPROFILER_SCOPE("Render", "renderShadowMap");
    RBXPROFILER_SCOPE("GPU", "ShadowMap");

	const int shadowMapSize = shadowMap->getTexture()->getWidth();
	shadowCascadeCount = qualityLevel >= 14 ? 3u : qualityLevel >= 6 ? 2u : 1u;
	const int tileSize = shadowCascadeCount > 1 ? shadowMapSize / 2 : shadowMapSize;
	const float legacyRadius = shadowMapSize * shadowMapTexelSize / 2.f;
	const float cascadeRadii[3] = {
		legacyRadius * 0.35f,
		legacyRadius,
		legacyRadius * 2.5f,
	};
    float clipSign = visualEngine->getDevice()->getCaps().requiresRenderTargetFlipping ? 1 : -1;
	globalShaderData.ShadowCascadeInfo = Vector4(
		cascadeRadii[0] * 0.9f, cascadeRadii[1] * 0.9f,
		static_cast<float>(shadowCascadeCount), clipSign);
	// The atlas stores depth in an 8-bit color channel.  A one-level constant
	// offset plus a bounded slope term avoids self-shadowing without allowing
	// steep faces to detach visibly from their casters.
	globalShaderData.ShadowParams = Vector4(1.f / 255.f, 1.25f, 0.0125f, 0.f);
	...
	Vector4* matrixRows[9] = {
		&globalShaderData.ShadowMatrix0, &globalShaderData.ShadowMatrix1,
		&globalShaderData.ShadowMatrix2, &globalShaderData.ShadowMatrix3,
		&globalShaderData.ShadowMatrix4, &globalShaderData.ShadowMatrix5,
		&globalShaderData.ShadowMatrix6, &globalShaderData.ShadowMatrix7,
		&globalShaderData.ShadowMatrix8,
	};
	bool renderedCasters = false;
	for (unsigned int cascade = 0; cascade < shadowCascadeCount; ++cascade)
	{
		const float radius = cascadeRadii[cascade];
		const float depthRadius = std::max(64.f, radius);
		const Vector3 shadowCenter = camera.getPosition() + camera.getDirection() * (radius * 0.45f);
		RenderCamera shadowCamera = getShadowCamera(shadowCenter, tileSize, radius, depthRadius);

		if (shadowCascadeCount > 1)
		{
			const float tx = cascade == 1 ? 0.5f : -0.5f;
			const float ty = cascade == 2 ? 0.5f : -0.5f;
			const Matrix4 tileClip = Matrix4::translation(tx, ty, 0) * Matrix4::scale(0.5f, 0.5f, 1);
			shadowCamera.setProjectionMatrix(tileClip * shadowCamera.getProjectionMatrix());
		}

		const Matrix4 textureMatrix = clipToUv * shadowCamera.getViewProjectionMatrix();
		*matrixRows[cascade * 3 + 0] = textureMatrix.row(0);
		*matrixRows[cascade * 3 + 1] = textureMatrix.row(1);
		*matrixRows[cascade * 3 + 2] = textureMatrix.row(2);

		renderNodes.clear();
		spatialHashedScene->querySphere(renderNodes, shadowCenter,
			std::max(radius, depthRadius), CullableSceneNode::Flags_ShadowCaster);
		...
		if (group.size() > 0)
		{
			renderObjects(context, group, RenderQueueGroup::Sort_Material,
				visualEngine->getRenderStats()->passShadow, "Id_OpaqueCasters");
			renderedCasters = true;
		}
	}
...
void SceneManager::blurShadowMap(DeviceContext *context, ShadowMap *shadowMap)
{
    ...
    {
        const float filterRadius = 0.5f + shadowMapSoftness * 2.5f;
        float params[] = { filterRadius / shadowMapSize, 0, 0, 0 };
        float atlasParams[] = { static_cast<float>(shadowCascadeCount), 0, 0, 0 };
        context->setConstant(program->getConstantHandle("Params1"), params, 1);
        context->setConstant(program->getConstantHandle("Params2"), atlasParams, 1);
    ...
    {
        const float filterRadius = 0.5f + shadowMapSoftness * 2.5f;
        float params[] = { 0, filterRadius / shadowMapSize, 0, 0 };
        float atlasParams[] = { static_cast<float>(shadowCascadeCount), 0, 0, 0 };
        context->setConstant(program->getConstantHandle("Params1"), params, 1);
        context->setConstant(program->getConstantHandle("Params2"), atlasParams, 1);
