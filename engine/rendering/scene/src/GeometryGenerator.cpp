...
namespace Graphics
{
		...
		const Vector4& edgeDistances = Vector4(NO_OUTLINES, NO_OUTLINES, NO_OUTLINES, NO_OUTLINES))
	{
		...
		r.color = color;
		r.extra = extra;
		r.skinIndices = Color4uint8(extra.r, 0, 0, 0);
		r.skinWeights = Color4uint8(255, 0, 0, 0);
		r.tangent = tangent;
	...
	static Color4uint8 getBaseColor(PartInstance* part, Decal* decal, DataModelMesh* specialShape, const GeometryGenerator::Options& options)
	{
		...
		// Current Decal and Texture instances tint their sampled image with Color3.
		if (decal)
	...
	static Vector3 getMeshScale(PartInstance* part, DataModelMesh* specialShape, const HumanoidIdentifier* humanoidIdentifier)
	{
		...
		{
			const Vector3& initialSize = meshPart->getInitialSize();
			return Vector3(
				initialSize.x != 0.0f ? part->getPartSizeXml().x / initialSize.x : 1.0f,
				initialSize.y != 0.0f ? part->getPartSizeXml().y / initialSize.y : 1.0f,
				initialSize.z != 0.0f ? part->getPartSizeXml().z / initialSize.z : 1.0f);
		}
	...
	inline Vector2 getDecalUVOffset(Decal* decal)
	{
		if (DecalTexture* texture = decal ? decal->fastDynamicCast<DecalTexture>() : NULL)
		{
			const Vector2& offset = texture->getOffsetStuds();
			const Vector2& tile = texture->getStudsPerTile();
			return Vector2(-offset.x / tile.x, offset.y / tile.y);
		}
	...
	{
		size_t sourceVertexBegin = 0;
		size_t vertexCount = data->vnts.size();
		size_t sourceFaceBegin = 0;
		size_t faceCount = data->faces.size();
		if (options.fileMeshSubset >= 0)
		{
			if (!options.fileSkinning ||
				static_cast<std::size_t>(options.fileMeshSubset) >= data->subsets.size())
				throw std::runtime_error("invalid skinned FileMesh subset selection");
			const FileMeshData::Subset& subset =
				data->subsets[static_cast<std::size_t>(options.fileMeshSubset)];
			sourceVertexBegin = subset.vertsBegin;
			vertexCount = subset.vertsLength;
			sourceFaceBegin = subset.facesBegin;
			faceCount = subset.facesLength;
		}
		const bool doubleSided = Instance::fastDynamicCast<MeshPart>(part) &&
			Instance::fastDynamicCast<MeshPart>(part)->getDoubleSided();
		const size_t emittedFaceCount = doubleSided ? faceCount * 2 : faceCount;

		size_t vertexOffset = mVertexCount;
		size_t indexOffset = mIndexCount;
		
		mVertexCount += vertexCount;
		mIndexCount += emittedFaceCount * 3;
		...
		auto finishVertex = [&](Vertex& vertex, std::size_t sourceIndex) {
			if (sourceIndex < data->vertexExtras.size())
			{
				const FileMeshData::VertexExtra& vertexExtra = data->vertexExtras[sourceIndex];
				if ((options.flags & Flag_VertexColorMesh) != 0)
				{
					vertex.color.r = static_cast<unsigned char>(
						(static_cast<unsigned int>(vertex.color.r) * vertexExtra.color[0]) / 255);
					vertex.color.g = static_cast<unsigned char>(
						(static_cast<unsigned int>(vertex.color.g) * vertexExtra.color[1]) / 255);
					vertex.color.b = static_cast<unsigned char>(
						(static_cast<unsigned int>(vertex.color.b) * vertexExtra.color[2]) / 255);
					vertex.color.a = static_cast<unsigned char>(
						(static_cast<unsigned int>(vertex.color.a) * vertexExtra.color[3]) / 255);
				}
				const float tangentScale = 1.0f / 127.0f;
				vertex.tangent = cframe.vectorToWorldSpace(Vector3(
					vertexExtra.tangent[0] * tangentScale,
					vertexExtra.tangent[1] * tangentScale,
					vertexExtra.tangent[2] * tangentScale));
			}

			if (options.fileSkinning)
			{
				if (sourceIndex >= data->skinning.size())
					throw std::runtime_error("skinned FileMesh vertex has no skin record");
				const FileMeshData::Skinning& skin = data->skinning[sourceIndex];
				vertex.skinIndices = Color4uint8(
					options.fileBoneMap[skin.boneIndices[0]],
					options.fileBoneMap[skin.boneIndices[1]],
					options.fileBoneMap[skin.boneIndices[2]],
					options.fileBoneMap[skin.boneIndices[3]]);
				vertex.skinWeights = Color4uint8(skin.boneWeights[0],
					skin.boneWeights[1], skin.boneWeights[2], skin.boneWeights[3]);
				if (options.fileCpuSkinning)
				{
					...
					{
						const float weight = skin.boneWeights[influence] / 255.0f;
						if (weight == 0.0f)
							continue;
						const CoordinateFrame& frame = options.fileSkinTransforms[
							options.fileBoneMap[skin.boneIndices[influence]]];
						position += frame.pointToWorldSpace(vertex.pos) * weight;
						normal += frame.vectorToWorldSpace(vertex.normal) * weight;
						tangent += frame.vectorToWorldSpace(vertex.tangent) * weight;
					}
					vertex.pos = position;
					vertex.normal = normal.direction();
					if (tangent.squaredMagnitude() > 0.0f)
						vertex.tangent = tangent.direction();
					vertex.skinIndices = Color4uint8(0, 0, 0, 0);
					vertex.skinWeights = Color4uint8(255, 0, 0, 0);
				}
			}

			extendBounds(mBboxMin, mBboxMax, vertex.pos);
		};
		...
		if (isHead)
		{
			...
			for (size_t i = 0; i < vertexCount; ++i)
			{
				...
				const std::size_t sourceIndex = sourceVertexBegin + i;
				const FileMeshVertexNormalTexture3d& sv = data->vnts[sourceIndex];
		...
		{
			const FileMeshFace& face = data->faces[sourceFaceBegin + i];
			indices[indexOffset + i * 3 + 0] = vertexOffset + face.a - sourceVertexBegin;
			indices[indexOffset + i * 3 + 1] = vertexOffset + face.b - sourceVertexBegin;
			indices[indexOffset + i * 3 + 2] = vertexOffset + face.c - sourceVertexBegin;
		}
		if (doubleSided)
		{
			for (size_t i = 0; i < faceCount; ++i)
			{
				const size_t output = indexOffset + (faceCount + i) * 3;
				const FileMeshFace& face = data->faces[sourceFaceBegin + i];
				indices[output + 0] = vertexOffset + face.c - sourceVertexBegin;
				indices[output + 1] = vertexOffset + face.b - sourceVertexBegin;
				indices[output + 2] = vertexOffset + face.a - sourceVertexBegin;
			}
		}
	}
	...
	template<bool vertical> void GeometryGenerator::addCylinder(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed, bool ignoreMaterialsStuds)
	{
		...
		Color4uint8 extra = getExtra(part, options);
		const Vector2 decalUVOffset = getDecalUVOffset(decal);
		...
		for (int face = 0; face < 6; ++face)
		{
			...
			if(face % 3 == 0)
			{
				...
				Vector2 mainUV = 0.5f * mainMultiplier + decalUVOffset;
				...
				for(unsigned adjFace = 0; adjFace < 4; adjFace++)
				{
					...
					for(unsigned tess = 0; tess < tesselationFactor+1; tess++) 
					{
						...
						Vector2 mainUV = (verticalRotateDecal<vertical>(normal2d.yx())*capDesc.normal2dToUV +Vector2(1,1))*0.5f * mainMultiplier + decalUVOffset;
				...
				for(unsigned iU = 0; iU < tesselationPerQuad+1; iU++) 
				{
					...
					for(unsigned iV = 0; iV < tesselationFactor+1; iV++) 
					{
						...
						fillVertex(vertices[vertexOffset + vertexCounter], options.cframe.translation + worldOffset + center, worldNormal, 
							(mainCoordsV + decalUVOffset) * surfaceTiling, studsV,
							color, extra, tangent);
						vertexCounter++;
						worlsPosV += worldDirV;
						studsV.x -= studOffsetU;
	...
	template<bool corner> void GeometryGenerator::addWedge(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed, bool ignoreMaterialsStuds)
	{
		...
		if (decal)
		{
			...
			const Vector2 uvOffset = getDecalUVOffset(decal);
	...
	void GeometryGenerator::addBlock(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed, bool ignoreMaterialsStuds)
	{
		...
		if (decal)
		{
			...
			fillVertex(vertices[vertexOffset + 0], corners[faces[face][0]], normals[face], Vector2(uv.x, 0) + uvOffset, Vector2(), color, extra, Vector3());
			fillVertex(vertices[vertexOffset + 1], corners[faces[face][1]], normals[face], Vector2(0, 0) + uvOffset, Vector2(), color, extra, Vector3());
			fillVertex(vertices[vertexOffset + 2], corners[faces[face][2]], normals[face], Vector2(uv.x, uv.y) + uvOffset, Vector2(), color, extra, Vector3());
			fillVertex(vertices[vertexOffset + 3], corners[faces[face][3]], normals[face], Vector2(0, uv.y) + uvOffset, Vector2(), color, extra, Vector3());
	...
	void GeometryGenerator::addTorso(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed)
	{
		...
		if (decal)
		{
			...
			fillVertex(vertices[vertexOffset + 0], cframe.pointToWorldSpace(corners[faces[face][0]]), normal, Vector2(uv.x, 0) + uvOffset, Vector2(), color, extra, Vector3());
			fillVertex(vertices[vertexOffset + 1], cframe.pointToWorldSpace(corners[faces[face][1]]), normal, Vector2(0, 0) + uvOffset, Vector2(), color, extra, Vector3());
			fillVertex(vertices[vertexOffset + 2], cframe.pointToWorldSpace(corners[faces[face][2]]), normal, Vector2(uv.x, uv.y) + uvOffset, Vector2(), color, extra, Vector3());
			fillVertex(vertices[vertexOffset + 3], cframe.pointToWorldSpace(corners[faces[face][3]]), normal, Vector2(0, uv.y) + uvOffset, Vector2(), color, extra, Vector3());
	...
	GeometryGenerator::Resources GeometryGenerator::fetchResources(PartInstance* part, const HumanoidIdentifier* hi, unsigned int flags, AsyncResult* asyncResult)
	{
		if (MeshPart* meshPart = Instance::fastDynamicCast<MeshPart>(part))
		{
			boost::shared_ptr<FileMeshData> fileMeshData =
				fetchMesh<MeshContentProvider, FileMeshData, MeshId>(
					meshPart->getMeshId(), part, asyncResult);
    ...
    void GeometryGenerator::addOperation(PartInstance* part, Decal* decal, const Options& options, const Resources& resources, unsigned int randomSeed)
	{
        ...
        boost::shared_ptr<CSGMesh> csgMesh = resources.csgMeshData;
		...
		for (size_t vi = 0; vi < meshVertices.size(); vi++)
		{
            ...
            if (decal)
            {
                ...
                else
                {
                    ...
                    if (texture)
                    {
                        uv = uv / texture->getStudsPerTile() + getDecalUVOffset(decal);
                    }
                    else
                    {
