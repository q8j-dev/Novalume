...
struct FrustumVisitor: public GfxSpatialHash::SpaceFilter
{
    ...
    bool onPrimitive(CullableSceneNode* n, IntersectResult intersectResult, float distance)
    {
		...
		if (n->getRenderWorld() == renderWorld && n->lastFrustumVisibleFrameNumber < frameNumber)
        {
	...
	const void* renderWorld)
{
    ...
    for (UnhashedNodes::iterator it = unhashedNodes.begin(); it != unhashedNodes.end(); ++it)
    {
        ...
        if (n->getRenderWorld() == renderWorld && camera.isVisible(n->getWorldBounds()))
            nodes.push_back(n);
    }

    FrustumVisitor visitor(nodes, camera, pointOfInterest, frm, getNextFrame(), renderWorld);
