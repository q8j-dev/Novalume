...
{
public:
    FastCluster(VisualEngine* visualEngine, SceneUpdater* sceneUpdater, Humanoid* humanoid, SuperCluster* owner, bool fw);
    virtual ~FastCluster();
	...
	void getAllParts(std::vector<boost::shared_ptr<PartInstance>>& retParts) const;
...
private:
    ...
    {
        PartInstance* root;
		boost::weak_ptr<PartInstance> targetPart;
    ...
    bool dirty;
    bool lightDirty;
	bool cpuSkinningFallback;
	bool rebuildingGeometry;
};
