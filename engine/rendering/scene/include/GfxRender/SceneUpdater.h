...
namespace Graphics
{
	...
	protected:
		...
		boost::shared_ptr<RBX::Instance> renderRoot;
	...
	public:
		...
		const boost::shared_ptr<RBX::Instance>& getRenderRoot() const { return renderRoot; }
		SceneManager* getSceneManager() const { return sceneManager; }
		const void* getRenderWorld() const { return renderWorld; }
		bool contains(const RBX::Instance* instance) const;

	protected:
		rbx::signals::scoped_connection workspaceDescendantAddedConnection;
        rbx::signals::scoped_connection propertyChangedSignal;

		void addMegaCluster(const boost::shared_ptr<RBX::PartInstance>& part);
		void addFastPart(const boost::shared_ptr<RBX::PartInstance>& part, bool isFW, bool isPriorityPart);
		void addAttachment(const boost::shared_ptr<RBX::Instance>& instance);
	...
	public:
		...
		SceneUpdater(boost::shared_ptr<RBX::DataModel> dataModel, VisualEngine* ve,
			const boost::shared_ptr<RBX::Instance>& renderRoot = boost::shared_ptr<RBX::Instance>(),
			SceneManager* sceneManager = NULL, const void* renderWorld = NULL);
		~SceneUpdater();
