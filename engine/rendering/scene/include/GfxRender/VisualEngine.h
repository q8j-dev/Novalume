...
public:
	...
	void bindWorkspace(const boost::shared_ptr<DataModel>& dm);
    ...
    const boost::shared_ptr<Typesetter>& getTypesetter(Text::Font font);
    ...
    ViewportRenderer* getViewportRenderer() const { return viewportRenderer.get(); }
    void setThumbnailSceneProvider(
        const boost::shared_ptr<ThumbnailSceneProvider>& provider)
    {
        thumbnailSceneProvider = provider;
    }
    const boost::shared_ptr<ThumbnailSceneProvider>& getThumbnailSceneProvider() const
    {
        return thumbnailSceneProvider;
    }
...
private:
    ...
    boost::shared_ptr<ThumbnailSceneProvider> thumbnailSceneProvider;
    ...
    boost::shared_ptr<Typesetter> typesetters[Text::FONT_LAST];
    scoped_ptr<TextureAtlas> glyphAtlas;
    scoped_ptr<ViewportRenderer> viewportRenderer;
