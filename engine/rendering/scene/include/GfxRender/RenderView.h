...
// Writes the contents of a rendered framebuffer through the engine's PNG
// encoder and returns the resulting file in the user's pictures directory.
// Player hosts that drive VisualEngine directly use the same path as
// RenderView so screenshot capture does not depend on a particular OS shell.
