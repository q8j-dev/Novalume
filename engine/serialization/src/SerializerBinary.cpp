...
namespace RBX
{
	...
	static ChunkHeader readChunk(std::istream& in, MemoryInputStream& result)
	{
		...
		if (header.size)
		{
			...
			else
			{
				...
				const bool isZstd = compressed.size() >= sizeof(zstdMagic) &&
					memcmp(&compressed[0], zstdMagic, sizeof(zstdMagic)) == 0;

				if (isZstd)
				{
					size_t count = ZSTD_decompress(result.data.get(), result.datasize,
						&compressed[0], compressed.size());

					if (ZSTD_isError(count))
						throw RBX::runtime_error("Malformed Zstandard data: %s", ZSTD_getErrorName(count));
					if (count != result.datasize)
						throw RBX::runtime_error("Malformed Zstandard data (%d != %d)",
							(int)count, (int)result.datasize);
				}
				else
				{
					int count = LZ4_decompress_safe(&compressed[0], result.data.get(),
						compressed.size(), result.datasize);

					if (count != result.datasize)
						throw RBX::runtime_error("Malformed LZ4 data (%d != %d)", count, (int)result.datasize);
				}
			}
		}
	...
	void readPropertyValues(MemoryInputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<Instance*>& instances, const std::vector<shared_ptr<Instance> >& idMap, const std::vector<std::string>& sharedStrings)
    {
		if (desc.type == Reflection::Type::singleton<ProtectedString>())
		{
			char format;
			readRaw(stream, format);
			if (format != bpfString && format != bpfProtectedString)
				throw RBX::runtime_error("Unexpected protected-string format %d", format);

			std::string value;
			for (size_t i = 0; i < instances.size(); ++i)
			{
				readString(stream, value);
				setPropertyValue<ProtectedString>(desc, instances[i],
					format == bpfProtectedString
						? ProtectedString::fromBytecode(value)
						: ProtectedString::fromTrustedSource(value));
			}
		}
		else if (desc.type == Reflection::Type::singleton<std::string>() || desc.type == Reflection::Type::singleton<BinaryString>())
        {
			char format;
			readRaw(stream, format);
			if (format == bpfString)
			...
			else if (format == bpfSharedString)
			{
				std::vector<unsigned int> indices;
				readUIntVector(stream, indices, instances.size());
				for (size_t i = 0; i < instances.size(); ++i)
				{
					if (indices[i] >= sharedStrings.size())
						throw RBX::runtime_error("Shared string index out of bounds: %u", indices[i]);
					desc.setStringValue(instances[i], sharedStrings[indices[i]]);
				}
			}
			else
				throw RBX::runtime_error("Unexpected string format %d", format);
        }
        ...
        else if (desc.type == Reflection::Type::singleton<Content>())
        {
            readFormatExpected(stream, bpfContent);

            std::vector<int> sourceTypes;
            readIntVector(stream, sourceTypes, instances.size());

            unsigned int uriCount = 0;
            readRaw(stream, uriCount);
            std::vector<std::string> uris(uriCount);
            for (std::string& uri : uris)
                readString(stream, uri);

            unsigned int objectCount = 0;
            readRaw(stream, objectCount);
            std::vector<int> objectIds;
            readIdVector(stream, objectIds, objectCount);

            unsigned int externalCount = 0;
            readRaw(stream, externalCount);
            std::vector<int> externalIds;
            readIdVector(stream, externalIds, externalCount);
            ...
            for (size_t i = 0; i < instances.size(); ++i)
            {
                ...
                {
                case CONTENT_SOURCE_NONE:
                    break;
                case CONTENT_SOURCE_URI:
                    if (uriIndex >= uris.size())
                        throw RBX::runtime_error("Content URI array is truncated");
                    value = Content::fromUri(uris[uriIndex++]);
                    break;
                case CONTENT_SOURCE_OBJECT:
                    if (objectIndex >= objectIds.size())
                        throw RBX::runtime_error("Content object array is truncated");
                    if (objectIds[objectIndex] >= 0 &&
                        static_cast<size_t>(objectIds[objectIndex]) < idMap.size())
                        value = Content::fromObject(idMap[objectIds[objectIndex]]);
                    ++objectIndex;
                    break;
                default:
                    throw RBX::runtime_error("Unsupported Content source type %d",
                        sourceTypes[i]);
                }
                setPropertyValue(desc, instances[i], value);
            }
            if (uriIndex != uris.size() || objectIndex != objectIds.size())
                throw RBX::runtime_error("Content side array has unused values");
		}
        ...
        else if (desc.type == Reflection::Type::singleton<Font>())
        {
            readFormatExpected(stream, bpfFont);
            for (size_t i = 0; i < instances.size(); ++i)
            {
                std::string family;
                ...
                readString(stream, family);
                readRaw(stream, weight);
                readRaw(stream, style);
                readString(stream, cachedFaceId);
                if (weight < FONT_WEIGHT_THIN || weight > FONT_WEIGHT_HEAVY)
                    throw RBX::runtime_error("Invalid Font weight %u", weight);
                if (style > FONT_STYLE_ITALIC)
                    throw RBX::runtime_error("Invalid Font style %u", style);
                setPropertyValue(desc, instances[i], Font(family,
                    static_cast<FontWeight>(weight), static_cast<FontStyle>(style)));
            }
        }
        else
        {
	...
	void writePropertyValues(MemoryOutputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<const Instance*>& instances, const std::map<const Instance*, int>& idMap, unsigned int flags)
    {
        if (desc.type == Reflection::Type::singleton<ProtectedString>())
        {
			bool containsBytecode = false;
			for (size_t i = 0; i < instances.size(); ++i)
			{
				const ProtectedString value = getPropertyValue<ProtectedString>(desc, instances[i]);
				if (!value.getBytecode().empty())
				{
					containsBytecode = true;
					break;
				}
			}
			writeRaw(stream, (char)(containsBytecode ? bpfProtectedString : bpfString));
			for (size_t i = 0; i < instances.size(); ++i)
			{
				const ProtectedString value = getPropertyValue<ProtectedString>(desc, instances[i]);
				if (containsBytecode && !value.getSource().empty())
					throw RBX::runtime_error("Cannot serialize mixed source and bytecode ProtectedStrings");
				writeString(stream, containsBytecode ? value.getBytecode() : value.getSource());
			}
        }
        else if (desc.type == Reflection::Type::singleton<std::string>() || desc.type == Reflection::Type::singleton<BinaryString>())
        {
        ...
        {
            writeRaw(stream, (char)bpfContent);

            std::vector<int> sourceTypes;
            std::vector<std::string> uris;
            std::vector<int> objectIds;
            sourceTypes.reserve(instances.size());
            for (const Instance* instance : instances)
            {
                const Content value = getPropertyValue<Content>(desc, instance);
                switch (value.getSourceType())
                {
                case CONTENT_SOURCE_NONE:
                    sourceTypes.push_back(CONTENT_SOURCE_NONE);
                    break;
                case CONTENT_SOURCE_URI:
                    sourceTypes.push_back(CONTENT_SOURCE_URI);
                    uris.push_back(value.getUri());
                    break;
                case CONTENT_SOURCE_OBJECT:
                {
                    sourceTypes.push_back(CONTENT_SOURCE_OBJECT);
                    const Instance* object = dynamic_cast<const Instance*>(
                        value.getObject().get());
                    objectIds.push_back(object ? getInstanceId(idMap, object) : -1);
                    break;
                }
                default:
                    throw RBX::runtime_error(
                        "Opaque Content cannot be serialized in the public RBXL format");
                }
            }
            writeIntVector(stream, sourceTypes);
            writeRaw(stream, static_cast<unsigned int>(uris.size()));
            for (const std::string& uri : uris)
                writeString(stream, uri);
            writeRaw(stream, static_cast<unsigned int>(objectIds.size()));
            writeIdVector(stream, objectIds);
            writeRaw(stream, static_cast<unsigned int>(0));
		}
        else if (desc.type == Reflection::Type::singleton<NumberSequence>())
        {
        ...
        {
            writeRaw(stream, (char)bpfFont);
            for (size_t i = 0; i < instances.size(); ++i)
            {
                const Font value = getPropertyValue<Font>(desc, instances[i]);
                writeString(stream, value.getFamily());
                writeRaw(stream, static_cast<unsigned short>(value.getWeight()));
                writeRaw(stream, static_cast<unsigned char>(value.getStyle()));
                // Current binary files retain a cached face URI after the
                // semantic family/weight/style triplet. The runtime resolves
                // that cache from the family, matching XML serialization.
                writeString(stream, value.getFamily());
            }
        }
        else
	...
	static void deserializeDecodeProperty(MemoryInputStream& stream, const Reflection::ClassDescriptor* typeDesc, const std::vector<shared_ptr<Instance> >& idMap, const std::vector<Instance*>& objects, const std::vector<std::string>& sharedStrings)
	{
		...
		if (propertyDesc && propertyDesc->canXmlRead())
		{
			try
			{
				readPropertyValues(stream, *propertyDesc, objects, idMap, sharedStrings);
			}
			catch (const std::exception& error)
			{
				throw RBX::runtime_error("Failed to deserialize %s.%s: %s",
					typeDesc->name.c_str(), propertyName.c_str(), error.what());
			}
		}
	}
	...
	static void deserializeImpl(std::istream& in, Instance* root, Instances* result)
	{
		...
		while (in.good())
		{
			...
			else if (memcmp(chunk.name, kChunkSharedStrings, sizeof(chunk.name)) == 0)
			{
				...
				readRaw(stream, count);
				if (version != 0 || count > (stream.datasize - stream.offset) / 20)
					throw RBX::runtime_error("Invalid shared string table");
				sharedStrings.clear();
				...
				for (unsigned int index = 0; index < count; ++index)
				{
					...
					std::string value;
					readString(stream, value);
					sharedStrings.push_back(value);
				}
				if (stream.offset != stream.datasize)
					throw RBX::runtime_error("Unexpected shared string table data");
			}
			else if (memcmp(chunk.name, kChunkProperty, sizeof(chunk.name)) == 0)
			{
				...
				if (types[typeIndex])
				{
					deserializeDecodeProperty(stream, types[typeIndex], objects, typedobjects[typeIndex], sharedStrings);
				}
			}
