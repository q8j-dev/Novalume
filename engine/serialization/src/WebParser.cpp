...
namespace RBX
{
	...
	{
		std::string::size_type first = rawWebResponse.find_first_not_of(" \t\r\n");
		const bool scalarRoot = first != std::string::npos &&
			rawWebResponse[first] != '{' && rawWebResponse[first] != '[';
		const std::string parseInput = scalarRoot
			? std::string("[") + rawWebResponse + "]"
			: rawWebResponse;
		rapidjson::Document root;
		root.Parse<0>(parseInput.c_str());
		...
		if (scalarRoot)
		{
			if (!root.IsArray() || root.Size() != 1)
				return false;
			result = populateValueTableFromRapidJson(root[rapidjson::SizeType(0)]);
		}
		else
		...
		rapidjson::Document::AllocatorType& allocator)
	{
		...
		else if(value.isString())
		{
			...
			// Localization tables and current web payloads are UTF-8. Preserve the
			// exact byte length so embedded multibyte text is not rejected or cut.
			node.SetString(strValue.data(),
				static_cast<rapidjson::SizeType>(strValue.size()), allocator);
		}
	...
	bool WebParser::writeJSON(const Reflection::Variant& value, std::string& result, NonJSONBehavior skip)
	{
		...
		const bool scalarRoot = !root.IsObject() && !root.IsArray();
		if (scalarRoot)
		{
			// The historical RapidJSON writer bundled with this source only
			// permits an object or array at the document root. Current
			// HttpService JSONEncode accepts every JSON value, so retain the
			// library's own escaping and numeric formatting by serializing the
			// scalar inside a single-element array.
			rapidjson::Value scalar;
			rapidjson::Value& rootValue = root;
			scalar = rootValue;
			root.SetArray();
			root.PushBack(scalar, root.GetAllocator());
		}
		root.Accept(writer);

		result.assign(buffer.GetString(), buffer.Size());
		if (scalarRoot)
		{
			RBXASSERT(result.size() >= 2 && result.front() == '[' && result.back() == ']');
