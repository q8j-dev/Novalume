...
bool Replicator::JoinDataItem::writeInstance(const Instance* instance, RBX::Network::PacketBuffer& bitStream)
{
...
size_t Replicator::JoinDataItem::writeInstances(RBX::Network::PacketBuffer& bitStream)
{
	// preallocate memory for bitstream using estimated size
	RBX::Network::PacketBuffer dataBitstream(instances.size() * 256);
...
bool Replicator::JoinDataItem::write(RBX::Network::PacketBuffer& bitStream) 
{
...
shared_ptr<DeserializedItem> Replicator::JoinDataItem::read(Replicator& replicator, RBX::Network::PacketBuffer& inBitstream)
{
	...
	for (unsigned int i = 0; i < count; i++)
	{
		try {
			...
			throw RBX::network_stream_exception(RBX::format(
				"join instance %u of %u (%s) at bit %u: %s", i + 1, count,
				deserializedData->instanceInfos[numInstances].classDescriptor
					? deserializedData->instanceInfos[numInstances].classDescriptor->name.c_str()
					: "unknown class",
				static_cast<unsigned int>(bitstream.GetReadOffset()), error.what()));
		}
