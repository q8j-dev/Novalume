...
/*
void logItem(MechanismItem& item, NetworkTime timeStamp)
{
...
void PhysicsReceiver::receiveMechanismCFrames(RBX::Network::PacketBuffer& bitStream, NetworkTime timeStamp, const RBX::RemoteTime& sendTime)
{
...
void PhysicsReceiver::receiveMechanism(RBX::Network::PacketBuffer& bitStream, PartInstance* rootPart, MechanismItem& item, RemoteTime remoteSendTime, int& numNodesInHistory)
{
...
void PhysicsReceiver::readMovementHistory(RBX::Network::PacketBuffer& bitStream, RemoteTime remoteSendTime, PartInstance* rootPart, MechanismItem& mechanismItem, int& numNodesInHistory)
{
    numNodesInHistory = 0;
    ...
    if (hasMovement)
    {
        ...
        //bitStream >> numNode;
        VarInt<>::decode<RBX::Network::PacketBuffer>(bitStream, &numNode);
        ...
        if (numNode > 0)
        {
            ...
            if (rootPart)
            {
                ...
                while (nodeStack.size()>0)
                {
                    ...
                    if (replicator->remoteNetworkTimeToLocalTime(nodeTime) >= rootPart->getLastUpdateTime()) // only process the new nodes
                    {
						if(DFFlag::SimpleHermiteSplineInterpolate)
							rootPart->addInterpolationSample(timedCf.cf,  mechanismItem.getAssemblyItem(mechanismItem.numAssemblies()-1).pv.velocity , nodeTime, replicator->remoteNetworkTimeToLocalTime(remoteSendTime), timedCf.timeToEnd, numNodesAhead);
						else
							rootPart->addInterpolationSample(timedCf.cf,  Velocity() , nodeTime, replicator->remoteNetworkTimeToLocalTime(remoteSendTime), timedCf.timeToEnd, numNodesAhead);
...
void PhysicsReceiver::readMechanismAttributes(RBX::Network::PacketBuffer& bitStream, MechanismItem& historyItem)
{
...
void PhysicsReceiver::readAssembly(RBX::Network::PacketBuffer& bitstream, PartInstance* rootPart, MechanismItem& mechanismItem, bool crossPacketCompression)
{
...
void PhysicsReceiver::deserializeTouches(RBX::Network::PacketBuffer& bitstream, const PeerAddress& from, std::vector<TouchPair>& touchPairs)
{
	while (true)
	{
		TouchPair tp;
...
bool PhysicsReceiver::deserializeTouch(RBX::Network::PacketBuffer& bitstream, const PeerAddress& from, TouchPair& touchPair)
{
	...
	touchPair = TouchPair(part1, part2, touched ? TouchPair::Touch : TouchPair::Untouch, peerAddressToSystemAddress(from));

	return true;
}

void PhysicsReceiver::readTouches(RBX::Network::PacketBuffer& bitstream, const PeerAddress& from)
{
	while (true)
	{
		TouchPair tp;
...
void PhysicsReceiver::readPV(RBX::Network::PacketBuffer& bitStream, AssemblyItem& item, bool crossPacketCompression)
{
...
void PhysicsReceiver::readCoordinateFrame(RBX::Network::PacketBuffer& bitStream, CoordinateFrame& cFrame)
{
...
void PhysicsReceiver::readVelocity(RBX::Network::PacketBuffer& bitStream, Velocity& velocity)
{
...
void PhysicsReceiver::readMotorAngles(RBX::Network::PacketBuffer& bitStream, AssemblyItem& item)
{
...
void PhysicsReceiver::readCompactCFrame(RBX::Network::PacketBuffer& bitStream, CompactCFrame& cFrame)
{
...
void PhysicsReceiver::setPhysics(const MechanismItem& item, const RBX::RemoteTime& remoteSendTime, const NetworkTimeMS lagInMs, int numNodesInHistory)
{
		...
		if (PartInstance* part = assemblyItem.rootPart.get())
		{
			...
			if (hasVelocity)
			{
				if (iAmServer)
				{
                    ...
                    Time localTime = replicator->remoteNetworkTimeToLocalTime(remoteSendTime);
				...
				else
				{
					...
					if (DFFlag::HumanoidFloorPVUpdateSignal)
					{
						float deltaTime =  (replicator->remoteNetworkTimeToLocalTime(remoteSendTime) - lastUpdateTime).seconds();
...
bool PhysicsReceiver::receiveRootPart(shared_ptr<PartInstance>& part, RBX::Network::PacketBuffer& inBitstream)
{
...
bool PhysicsReceiver::receivePart(shared_ptr<PartInstance>& part, RBX::Network::PacketBuffer& inBitstream)
{
