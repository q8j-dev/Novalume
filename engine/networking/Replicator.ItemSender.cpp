...
void Replicator::ItemSender::openPacket()
{
	if (!bitStream)
	{
		bitStream.reset(new RBX::Network::PacketBuffer());
...
void Replicator::ItemSender::closePacket()
{
	if (bitStream)
	{
		...
		if (bytes>0)
		{
			...
			networkPeer->Send(bitStream, packetPriority, DATAMODEL_RELIABILITY, DATA_CHANNEL, replicator.remotePlayerId, false);
...
Replicator::ItemSender::ItemSender(Replicator& replicator, ConcurrentPeer *networkPeer)
	:replicator(replicator),networkPeer(networkPeer)
