...
ServerReplicator::ServerReplicator(PeerAddress systemAddress, Server* server, NetworkSettings* networkSettings)
: Super(systemAddress, server->networkPeer, networkSettings, /*ClusterDebounce*/DFFlag::RCCSupportCloudEdit && server->isCloudEdit())
	, server(server)
...
void ServerReplicator::receiveCluster( RBX::Network::PacketBuffer& inBitstream, Instance* instance, bool usingOneQuarterIterator )
{
...
bool ServerReplicator::checkDistributedReceive(PartInstance* part)
{
	...
	bool fromOwner = (rootMoving->getNetworkOwner() == peerAddressToSystemAddress(remotePlayerId));
...
bool ServerReplicator::checkDistributedSendFast(const PartInstance* part)
{
    ...
    bool toOwner = part->getNetworkOwner() == peerAddressToSystemAddress(remotePlayerId);
...
void ServerReplicator::sendTop(ConcurrentPeer*peer)
{
#ifdef NETWORK_DEBUG
...
CheatHandlingServerReplicator::CheatHandlingServerReplicator(PeerAddress systemAddress, Server* server, NetworkSettings* networkSettings)
...
void ServerReplicator::sendDictionaryFormat()
{
    ...
    networkPeer->rawPeer()->Send(&bitStream, settings().getDataSendPriority(), DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
}
...
PluginReceiveResult CheatHandlingServerReplicator::OnReceive(Packet *packet)
{
	...
	case ID_DATA:
		if (!isAuthenticated)
		{
			FASTLOGS(FLog::Warning, "Player not authenticated %s", peerAddressToString(packet->systemAddress).c_str());
...
void CheatHandlingServerReplicator::processTicket(Packet *packet)
{
    try
    {
	    ...
	    if (!server->protocolVersionMatches(remoteProtocolVersion))
	    {
	    	...
	    	networkPeer->rawPeer()->Send(&bitStream, IMMEDIATE_PRIORITY, DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
	    }
...
void ServerReplicator::readRockyItem(RBX::Network::PacketBuffer& inBitstream)
{
...
#ifdef RBX_RCC_SECURITY
void CheatHandlingServerReplicator::processRockyCallInfoItem(RBX::Network::PacketBuffer& inBitstream)
{
    uint8_t size; 
    inBitstream >> size;
...
void CheatHandlingServerReplicator::decodeHashItem(PmcHashContainer& netHashes, unsigned long long* securityTokens)
{
    ...
    NetworkTime delta = GetTimeMS() - replicatorStats.lastReceivedHashTime;
    if (!reportedHashItemTime && delta >= DFInt::ReportTimeLimit1)
    {
        NetworkTime mccDelta = GetTimeMS() - replicatorStats.lastReceivedMccTime;
        NetworkTime pingDelta = GetTimeMS() - replicatorStats.lastReceivedPingTime;
    ...
    replicatorStats.lastReceivedHashTime = GetTimeMS();
    ++numHashItems;
...
void CheatHandlingServerReplicator::processRockyMccReport(const MccReport& report)
{
    ...
    NetworkTime delta = GetTimeMS() - replicatorStats.lastReceivedMccTime;
    ...
    replicatorStats.lastReceivedMccTime = GetTimeMS();
    ++numMccItems;
...
{
    NetworkTime delta = GetTimeMS() - replicatorStats.lastReceivedPingTime;
...
void ServerReplicator::readItem(RBX::Network::PacketBuffer& inBitstream, RBX::Network::Item::ItemType itemType)
{
	switch (itemType)
	{	
	default:
...
void ServerReplicator::readClientQuotaUpdate(RBX::Network::PacketBuffer& bitStream)
{
	int diff;
...
void ServerReplicator::readRequestCharacter(RBX::Network::PacketBuffer& inBitstream)
{
	...
	if (!DFFlag::DisablePlaceAuthenticationPoll)
	{
		...
		else
		{
			if (placeAuthenticationState == PlaceAuthenticationState_Requesting)
			{
				...
				outBitStream << retry;
				networkPeer->rawPeer()->Send(&outBitStream, settings().getDataSendPriority(), DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
				canSpawnPlayer = false;
			}
	...
	else 
	{
		...
		throw std::runtime_error(format("Couldn't resolve remotePlayer %s from %s", id.readableString().c_str(), peerAddressToString(remotePlayerId).c_str()));
	}
}
...
void ServerReplicator::processRequestCharacter(Instance* instance, RBX::Guid::Data id, unsigned int sendStats, std::string preferedSpawnName)
{
	...
	if (!remotePlayer)
	{
		...
		StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "RequestCharacter - RemotePlayer is null. %s", peerAddressToString(remotePlayerId).c_str());
...
void ServerReplicator::dataOutStep()
{
    if (placeAuthenticationState == PlaceAuthenticationState_Denied)
    {
        ...
        outBitStream << (unsigned char) ID_PLACEID_VERIFICATION;
        outBitStream << retry;
        networkPeer->rawPeer()->Send(&outBitStream, IMMEDIATE_PRIORITY, DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
...
void ServerReplicator::writeChangedProperty(const Instance* instance, const Reflection::PropertyDescriptor& desc, RBX::Network::PacketBuffer& outBitStream)
{
...
void ServerReplicator::readPropAcknowledgement(RBX::Network::PacketBuffer& inBitstream)
{
	int version;
	inBitstream >> version;
...
{
    RBX::Network::PacketBuffer* bitStream = reinterpret_cast<RBX::Network::PacketBuffer*>(context);
    //RBX::StandardOut::singleton()->printf(RBX::MESSAGE_INFO, 
    //    "Sent FFLag: %s: %s", name.c_str(), varValue.c_str());
    bitStream->Write(name);
    bitStream->Write(varValue);
    FLog::SetValueFromServer(name, varValue);
}

void ServerReplicator::serializeSFFlags(RBX::Network::PacketBuffer& outBitStream) const
{
...
{
	networkPeer->rawPeer()->Send(&getApiDictionaryBitStream(), settings().getDataSendPriority(), 
		DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
}

using namespace RBX::Reflection;
void ServerReplicator::writeDescriptorSchema(const ClassDescriptor* classDesc, RBX::Network::PacketBuffer& bitStream) const
{
    ...
    std::string className = classDesc->name.toString();
    ...
    for (; propIter != classDesc->Reflection::MemberDescriptorContainer<Reflection::PropertyDescriptor>::descriptors_end(); propIter++)
    {
        ...
        std::string propName = propDesc->name.toString();
        unsigned int typeId = typeDictionary.getId(&propDesc->type).id;
        std::string propType = propDesc->type.name.toString();
    ...
    for (; eventIter != classDesc->Reflection::MemberDescriptorContainer<Reflection::EventDescriptor>::descriptors_end(); eventIter++)
    {
        ...
        std::string eventName = eventDesc->name.toString();
...
RBX::Network::PacketBuffer ServerReplicator::apiSchemaBitStream;

namespace {
boost::mutex apiDictionaryGenerationMutex;
}

void ServerReplicator::generateSchema(const ServerReplicator* serverRep, bool force)
{
    boost::mutex::scoped_lock generationLock(apiDictionaryGenerationMutex);
    if (force)
    {
        // for unit tests
    ...
    while (enumIter != enumEnd)
    {
        ...
        std::string enumName = e->name.toString();
...
RBX::Network::PacketBuffer ServerReplicator::apiDictionaryBitStream;
...
const RBX::Network::PacketBuffer& ServerReplicator::getSchemaBitStream() const
{
    RBXASSERT(apiSchemaBitStream.GetNumberOfBytesUsed() > 0);
    return apiSchemaBitStream;
}

const RBX::Network::PacketBuffer& ServerReplicator::getApiDictionaryBitStream() const
{
...
void ServerReplicator::teachSchema()
{
    ...
    networkPeer->rawPeer()->Send(&getSchemaBitStream(), settings().getDataSendPriority(), DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
}
...
void ServerReplicator::sendJoinStatsToInflux()
{
	...
	joinAnalytics.addPoint("BytesSent", getPeerStatistics()->runningTotal[ACTUAL_BYTES_SENT]);
