...
void RoundRobinPhysicsSender::sendPhysicsData(RBX::Network::PacketBuffer& bitStream, const Assembly* assembly)
{
	RBXASSERT(assembly);
	...
	if (assembly)
	{
		...
		if (canSend(part, assembly, bitStream))
		{
			...
			if (replicator.isStreamingEnabled())
			{
				...
				{
					RBX::Network::BitCount characterBytes = bitStream.GetNumberOfBytesUsed();
...
class RoundRobinPhysicsSender::JobSender : public boost::noncopyable
{
	...
	void openPacketPhysicsOffset(NetworkTime& newTimeStamp)
	{
		bitStream.reset(new RBX::Network::PacketBuffer(maxStreamSize));
		*bitStream << (unsigned char) ID_TIMESTAMP;
		NetworkTime now = GetTime();
	...
	void closePacket()
	{
		if (bitStream->GetNumberOfBitsUsed()>emptySize)
		{
			...
			networkPeer->Send(bitStream, packetPriority, UNRELIABLE, PHYSICS_CHANNEL, sender.replicator.remotePlayerId, false);
...
public:
	...
	JobSender(RoundRobinPhysicsSender& sender, ConcurrentPeer *peer)
		:sender(sender)
		,networkPeer(peer)
		,firstA(NULL)
		,firstM(NULL)
	...
	bool report(const SimJob& simJob)
	{
		if (SFFlag::getPhysicsPacketSendWorldStepTimestamp())
		{
			...
			NetworkTime passStamp = (NetworkTime)((TaskScheduler::singleton().lastCyclcTimestamp +  Time::Interval(sender.replicator.getRakOffsetTime() - timeOffsetDueToPhysics)).timestampSeconds()* 1000.0);
		...
		ReplicatorStats::PhysicsSenderStats* stats)
{
	...
	JobSender jobSender(*this, replicator.networkPeer.get());
