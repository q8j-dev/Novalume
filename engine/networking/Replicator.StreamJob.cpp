...
namespace
{
PartInstance* findCharacterStreamingRoot(Instance* character)
{
    if (!character)
        return NULL;

    static const char* const rootPartNames[] = { "HumanoidRootPart", "Torso", "UpperTorso" };
    for (size_t i = 0; i < sizeof(rootPartNames) / sizeof(rootPartNames[0]); ++i)
    {
        if (PartInstance* root = Instance::fastDynamicCast<PartInstance>(character->findFirstChildByName(rootPartNames[i])))
            return root;
    }
...
bool Replicator::StreamJob::StreamDataItem::write(RBX::Network::PacketBuffer& bitStream) 
{
...
shared_ptr<DeserializedStreamDataItem> Replicator::StreamJob::StreamDataItem::read(Replicator& replicator, RBX::Network::PacketBuffer& inBitstream)
{
...
void Replicator::StreamJob::readRegionRemoval(RBX::Network::PacketBuffer& bitStream)
{
	...
	RBX::Network::PacketBuffer decompressedIds;
	RBX::Network::PacketBuffer* idList;
...
void Replicator::StreamJob::readInstanceRemoval(RBX::Network::PacketBuffer& bitStream)
{
	Guid::Data id;
...
TaskScheduler::StepResult Replicator::StreamJob::stepDataModelJob(const Stats& stats) 
{
	...
	if(replicator)
	{
		...
		try
		{
            ...
            {
                float bufferHealth = replicator->networkPeer->GetBufferHealth();
                if (bufferHealth < 0.5)
                {
...
int Replicator::StreamJob::sendItems(Container& itemList, int maxInstances, Time syncTime)
{
	...
	ItemSender sender(*replicator.get(), replicator->networkPeer.get());
...
void Replicator::StreamJob::onPlayerCharacterAdd(boost::shared_ptr<Instance> character)
{
    ...
    // R6 characters stream around Torso, while R15 characters stream around
    // HumanoidRootPart.  UpperTorso is retained as a fallback for incomplete rigs.
    PartInstance* streamingRoot = findCharacterStreamingRoot(character.get());
    if (!streamingRoot)
    {
        FASTLOGS(FLog::NetworkStreaming, "Character %s has no streaming root part", character ? character->getName().c_str() : "<null>");
        return;
    }
    playerTorsoPropChangedConnection = streamingRoot->propertyChangedSignal.connect(boost::bind(&StreamJob::onPlayerTorsoChanged, this, _1));
	...
	if (!replicator->players->getShouldAutoSpawnCharacter())
		setStreamCenterAndSentMinRegions(streamingRoot->getCoordinateFrame().translation, false);
}
...
void Replicator::StreamJob::onPlayerTorsoChanged(const Reflection::PropertyDescriptor* desc)
{
    ...
    if (*desc == PartInstance::prop_CFrame)
    {
        ...
        if (PartInstance* streamingRoot = findCharacterStreamingRoot(character.get()))
		{
			FASTLOG3(FLog::NetworkStreaming, "torso changed to (%d,%d,%d)",
				(int)streamingRoot->getCoordinateFrame().translation.x, (int)streamingRoot->getCoordinateFrame().translation.y, (int)streamingRoot->getCoordinateFrame().translation.z);
