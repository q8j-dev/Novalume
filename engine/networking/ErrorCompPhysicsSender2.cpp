			...
			if (!bitStream) 
			{
				...
				*bitStream << (unsigned char) ID_TIMESTAMP;
				...
				if (curBucket->iter->part->computeNetworkOwnerIsSomeoneElse())
				{
					...
					RBXASSERT(timestamp >= curBucket->iter->part->networkTime);
					if (curBucket->iter->part->networkTime)
						timestamp = curBucket->iter->part->networkTime;
				}
...
bool ErrorCompPhysicsSender2::writeNugget(RBX::Network::PacketBuffer& bitStream, Nugget& nugget)
{
...
bool ErrorCompPhysicsSender2::sendPhysicsData(RBX::Network::PacketBuffer& bitStream, const Nugget& nugget)
{
...
void ErrorCompPhysicsSender2::writeAssembly(RBX::Network::PacketBuffer& bitStream, const Assembly* assembly, Compressor::CompressionType compressionType, bool crossPacketCompression)
{
	if (physicsPacketCache)
	{
		...
		if (!physicsPacketCache->fetchIfUpToDate(assembly, (unsigned char)sendDetailed, bitStream))
		{
			...
			RBX::Network::PacketBuffer stream;
