...
namespace RBX {
	namespace Network {
		...
		public:
			...
			bool fetchIfUpToDate(const Assembly* key, unsigned char index, RBX::Network::PacketBuffer& outBitStream);

			// copy data from bitStream starting at its read position to numBits
			bool update(const Assembly* key, unsigned char index, RBX::Network::PacketBuffer& bitStream, unsigned int startReadBitPos, unsigned int numBits);
			...
			, public Service
		{
			...
			public:
				...
				boost::shared_ptr<RBX::Network::PacketBuffer> bitStream[2]; 
		...
		public:
			...
			bool fetchIfUpToDate(const Instance* key, RBX::Network::PacketBuffer& outBitStream, bool isJoinData);

			// copy data from bitStream starting at its read position to numBits
			bool update(const Instance* key, RBX::Network::PacketBuffer& bitStream, unsigned int numBits, bool isJoinData);
