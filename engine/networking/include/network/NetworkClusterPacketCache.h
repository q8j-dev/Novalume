...
namespace RBX {
	...
	namespace Network {
		// Stores the final serialized packet for each cluster chunk.
		...
		public:
			...
			bool fetchIfUpToDate(const Key &regionId, RBX::Network::PacketBuffer& outBitStream);
			...
			bool update(const Key &regionId, RBX::Network::PacketBuffer& bitStream, unsigned int numBits);
