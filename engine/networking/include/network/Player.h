...
namespace Network {
	...
	private:
		...
		// A standalone Player presents a client-only DataModel to CoreScripts but
		// owns its local character lifecycle in-process. This is neither reflected
		// nor replicated and is only enabled after validating that no server exists.
		bool offlineCharacterAutoSpawnEnabled;
		...
		std::string chatAvailabilityStatus;
		bool gameplayPaused;
		...
		shared_ptr<PeerAddress> remoteAddress;
	...
	public:
		...
		bool getGameplayPaused() const { return gameplayPaused; }
		void setGameplayPaused(bool value);
		const std::string& getPlatformName() const { return platformName; }
		void setPlatformName(const std::string& value);
		const std::string& getChatAvailabilityStatus() const { return chatAvailabilityStatus; }
		void setChatAvailabilityStatus(const std::string& value);
		bool getHasVerifiedBadge() const { return hasVerifiedBadge; }
		void setHasVerifiedBadge(bool value);
		...
		// Enables normal death/respawn for an in-process standalone local player.
		// This host contract is deliberately unavailable to Lua.
		void enableOfflineCharacterAutoSpawn();
		void maintainOfflineCharacterCamera();
		...
		const PeerAddress& getRemoteAddress() const;
		const SystemAddress getRemoteAddressAsRbxAddress() const;
		void setRemoteAddress(const PeerAddress& address);
	...
	private:
		...
		bool hasOfflineCharacterAuthority();
		void checkContextReadyToSpawnCharacter(bool allowOfflineAuthority = false);
		void loadCharacterInternal(bool inGame, const std::string& preferedSpawnName,
			bool allowOfflineAuthority);
		void respawnOfflineCharacterAfterDeath();
