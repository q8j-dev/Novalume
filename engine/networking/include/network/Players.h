...
namespace Network {
	...
	public:
		// Player chat handlers use a smaller ownership result set than packet handlers.
	...
	public:
		ReceiveResult OnReceiveChat(Player* sourceValidation, ConcurrentPeer* peer, Packet* packet, unsigned char chatType);
		ReceiveResult OnReceiveReportAbuse(Player* source, ConcurrentPeer* peer, Packet* packet);
		...
		rbx::signal<void(const PeerAddress&, const shared_ptr<RBX::Network::PacketBuffer>&, const shared_ptr<Instance>, const std::string&, const std::string&)> sendFilteredChatMessageSignal;
		...
		void setConnection(ConcurrentPeer* networkPeer);
	...
	private:
		...
		void sendFilteredChatMessageSignalHelper(const PeerAddress& systemAddress,
			const shared_ptr<RBX::Network::PacketBuffer>& baseData, const shared_ptr<Instance> sourceInstance, shared_ptr<ChatMessage> chatEvent, const ChatFilter::Result& response);
