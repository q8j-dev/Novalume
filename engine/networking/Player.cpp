...
static Reflection::PropDescriptor<Player, std::string> prop_displayName("DisplayName", category_Data, &Player::getDisplayName, NULL, Reflection::PropertyDescriptor::SCRIPTING);
static Reflection::PropDescriptor<Player, std::string> prop_chatAvailabilityStatus(
	"ChatAvailabilityStatus", category_Data, &Player::getChatAvailabilityStatus,
	&Player::setChatAvailabilityStatus,
	Reflection::PropertyDescriptor::HIDDEN_SCRIPTING, Security::RobloxScript);
// CorePackages still consumes the historical lower-camel spelling in the
// shared user model.  Keep it script-visible as the legacy descriptor for the
// canonical DisplayName property, matching the engine alias behavior used by
// the shipped package.
static Reflection::PropDescriptor<Player, std::string> prop_displayNameDeprecated(
	"displayName",
	category_Data,
	&Player::getDisplayName,
	NULL,
	Reflection::PropertyDescriptor::Attributes::deprecated(prop_displayName, Reflection::PropertyDescriptor::SCRIPTING));
static Reflection::PropDescriptor<Player, bool> prop_gameplayPaused("GameplayPaused", category_Data, &Player::getGameplayPaused, NULL, Reflection::PropertyDescriptor::SCRIPTING);
static Reflection::PropDescriptor<Player, std::string> prop_platformName("PlatformName", category_Data, &Player::getPlatformName, &Player::setPlatformName, Reflection::PropertyDescriptor::SCRIPTING, Security::RobloxScript);
static Reflection::PropDescriptor<Player, bool> prop_hasVerifiedBadge("HasVerifiedBadge", category_Data, &Player::getHasVerifiedBadge, NULL, Reflection::PropertyDescriptor::SCRIPTING);
...
static Reflection::RefPropDescriptor<Player, Team> prop_team(
	"Team", category_Team, &Player::getTeam, &Player::setTeam,
...
Player::Player(void)
	...
	,offlineCharacterAutoSpawnEnabled(false)
...
void Player::setGameplayPaused(bool value)
{
	if (gameplayPaused == value)
		return;
	gameplayPaused = value;
	raisePropertyChanged(prop_gameplayPaused);
}

void Player::setPlatformName(const std::string& value)
{
	if (platformName == value)
		return;
	platformName = value;
	raisePropertyChanged(prop_platformName);
}

void Player::setChatAvailabilityStatus(const std::string& value)
{
	if (chatAvailabilityStatus == value)
		return;
	chatAvailabilityStatus = value;
	raisePropertyChanged(prop_chatAvailabilityStatus);
}

void Player::setHasVerifiedBadge(bool value)
{
	if (hasVerifiedBadge == value)
		return;
	hasVerifiedBadge = value;
	raisePropertyChanged(prop_hasVerifiedBadge);
}

void Player::setDisplayName(const std::string& value)
{
	const std::string resolved = value.empty() ? getName() : value;
...
void Player::setCharacter(ModelInstance* value)
{
	...
	if (value != this->character.get())
	{
		...
		if (value) {
			...
			if (ownsCharacterLifecycle) {		// Server, solo mode, or the validated standalone host owns this lifecycle.
                RBXASSERT(workspace);
...
{
	const bool backendAuthority = Players::backendProcessing(this);
	const bool offlineAuthority = hasOfflineCharacterAuthority();
	RBXASSERT(backendAuthority || offlineAuthority);
	if (!backendAuthority && !offlineAuthority)
		return;
	...
	if (players == NULL || players->getCharacterAutoSpawnProperty())
	{
		...
		if (TimerService* timer = ServiceProvider::create<TimerService>(this)) {
			if (offlineAuthority)
				timer->delay(boost::bind(
					&Player::respawnOfflineCharacterAfterDeath, shared_from(this)), 5.0);
			else
				timer->delay(boost::bind(
					&Player::loadCharacter, shared_from(this), true, ""), 5.0);
		}
	}
}

bool Player::hasOfflineCharacterAuthority()
{
	if (!offlineCharacterAutoSpawnEnabled ||
		!Players::clientIsPresent(this, false) ||
		Players::serverIsPresent(this, false))
		return false;

	return Players::findLocalPlayer(this) == this;
}

void Player::enableOfflineCharacterAutoSpawn()
{
	if (!Players::clientIsPresent(this, false) ||
		Players::serverIsPresent(this, false))
		throw std::runtime_error(
			"offline character authority requires a client-only DataModel");

	Players* players = ServiceProvider::find<Players>(this);
	if (!players || players->getLocalPlayer() != this)
		throw std::runtime_error(
			"offline character authority requires the local Player");

	offlineCharacterAutoSpawnEnabled = true;
	if (character)
		if (Humanoid* humanoid = Humanoid::modelIsCharacter(character.get()))
			characterDiedConnection = humanoid->diedSignal.connect(
				boost::bind(&Player::onCharacterDied, this));
}

void Player::respawnOfflineCharacterAfterDeath()
{
	if (!hasOfflineCharacterAuthority())
		return;
	loadCharacterInternal(true, "", true);
}

void Player::maintainOfflineCharacterCamera()
{
	if (!hasOfflineCharacterAuthority() || !character)
		return;
	Workspace* workspace = ServiceProvider::find<Workspace>(this);
	Camera* camera = workspace ? workspace->getCamera() : NULL;
	if (!camera || camera->getCameraSubject() != NULL)
		return;
	if (Humanoid* humanoid = Humanoid::modelIsCharacter(character.get()))
		camera->setCameraSubject(humanoid);
}
...
{
	Teams* teams = ServiceProvider::find<Teams>(const_cast<Player*>(this));
	return teams ? teams->getTeamFromPlayer(const_cast<Player*>(this)) : NULL;
}
...
void Player::setTeam(Team* value)
{
	...
	setTeamColor(value->getTeamColor());
	setNeutral(false);
}
			...
			if (idle) 
			{
				...
				else
				{
					if (!players->getLocalPlayer())
						FASTLOG1(FLog::Error, "Player Idle Check no local player, id = %d", this->userId);

					if (!Players::clientIsPresent(this))
						FASTLOG1(FLog::Error, "Player Idle Check client not present, id = %d", this->userId);
				}
			}
		}
...
void Player::checkContextReadyToSpawnCharacter(bool allowOfflineAuthority) {
	Workspace* workspace = ServiceProvider::find<Workspace>(this);
	...
	if (!Players::backendProcessing(this) &&
		!(allowOfflineAuthority && hasOfflineCharacterAuthority())){
...
{
	loadCharacterInternal(inGame, preferedSpawnName, false);
}

void Player::loadCharacterInternal(bool inGame,
	const std::string& preferedSpawnName, bool allowOfflineAuthority)
{
	checkContextReadyToSpawnCharacter(allowOfflineAuthority);
    ...
    bool useR15 = false;
    DataModel::AvatarRigVariant avatarRigVariant = DataModel::AVATAR_RIG_R6;
    if (DFFlag::UseR15Character) 
    {
        if(DataModel* dataModel = DataModel::get(this))
        {
            useR15 = dataModel->getForceR15();
            avatarRigVariant = dataModel->getAvatarRigVariant();
        }
    }
	...
	if (model == NULL)
	{
		...
		if (useR15) 
		{
			if (avatarRigVariant == DataModel::AVATAR_RIG_R15)
			{
				ServiceProvider::create<ContentProvider>(this)->blockingLoadInstances(
					ContentId::fromAssets("avatar/characterR15.rbxm"), instances);
			}
			else if (avatarRigVariant == DataModel::AVATAR_RIG_R15_PLUS)
			{
				ServiceProvider::create<ContentProvider>(this)->blockingLoadInstances(
					ContentId::fromAssets("avatar/rigs/R15-plus.rbxm"), instances);
			}
			else
			{
				ServiceProvider::create<ContentProvider>(this)->blockingLoadInstances(
					ContentId::fromAssets("avatar/rigs/AnthroRigs.rbxm"), instances);
				const char* rigName = avatarRigVariant == DataModel::AVATAR_RIG_RTHRO_SLENDER
					? "AnthroSlender" : "AnthroNormal";
				Instance* rig = instances.size() == 1
					? instances.front()->findFirstChildByName(rigName) : NULL;
				shared_ptr<Instance> rigClone = rig ? rig->clone(EngineCreator) : shared_ptr<Instance>();
				instances.clear();
				if (rigClone)
				{
					// AnthroRigs is RigBuilder source data.  The current
					// character assembly step applies the same R15 collision
					// convention as characterR15: the root, head, and torso
					// collide with the world while articulated limbs do not.
					// Leaving the authored preview defaults on every limb makes
					// the humanoid self-support in Freefall and rejects emotes.
					for (std::size_t index = 0; index < rigClone->numChildren(); ++index)
					{
						PartInstance* part = Instance::fastDynamicCast<PartInstance>(
							rigClone->getChild(index));
						if (!part)
							continue;
						const std::string& partName = part->getName();
						const bool bodyCollider = partName == "HumanoidRootPart" ||
							partName == "Head" || partName == "UpperTorso" ||
							partName == "LowerTorso";
						part->setCanCollide(bodyCollider);
					}
					instances.push_back(rigClone);
				}
			}
		} else {
		...
		if (instances.size() != 1){
			FASTLOG(FLog::Error, "Player:LoadCharacter - Could not load the selected character rig");
			throw std::runtime_error("selected character asset should contain one Character model");
		}

		model = Instance::fastSharedDynamicCast<ModelInstance>(instances.front());
		pModel = model.get();
		if (!pModel){
			throw std::runtime_error("selected character asset should contain a Character model");
		}
	}
	...
	// RigBuilder's current AnthroRigs model intentionally carries the body
	// hierarchy and scaling metadata without relying on a serialized legacy
	// RigType value. Character creation owns the selected rig contract, so
	// establish R15 before Animator setup, joint construction, and humanoid
	// state creation for every non-R6 variant (including a StarterHumanoid).
	if (useR15 && humanoid &&
		humanoid->getRigType() != Humanoid::HUMANOID_RIG_TYPE_R15)
	{
		Security::Impersonator permission(Security::Replicator_);
		humanoid->setRigType(Humanoid::HUMANOID_RIG_TYPE_R15);
	}
	...
	if (DFFlag::UseStarterPlayerCharacterScripts) 
	{
		...
		if (starterPlayerService != NULL) 
		{
			...
			{
				const char* healthScript = useR15
					? "avatar/scripts/humanoidHealthRegenScript.rbxmx"
					: "fonts/humanoidHealthRegenScript.rbxm";
				ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets(healthScript), ContentProvider::PRIORITY_SCRIPT, boost::bind(&CharacterLoadHelper, model, _1, _2), AsyncHttpQueue::AsyncWrite);
			}
			...
			if (!overrideAnimateScript)
			{
				...
				if (useR15)
				{
					ServiceProvider::create<ContentProvider>(this)->blockingLoadInstances(ContentId::fromAssets("avatar/scripts/humanoidAnimateR15CharacterController.rbxm"), extraAnimate);
				} else {
					ServiceProvider::create<ContentProvider>(this)->blockingLoadInstances(ContentId::fromAssets("fonts/humanoidAnimateLocal.rbxm"), extraAnimate);
				}
...
void Player::setName(const std::string& value)
{
	...
	Super::setName(value);
	if (displayName.empty() || displayName == previousName || displayName == sPlayer)
...
const PeerAddress& Player::getRemoteAddress() const {
	if (remoteAddress) {
		return *remoteAddress;
	} else {
		static PeerAddress addr;
		return addr;
	}
}

const SystemAddress Player::getRemoteAddressAsRbxAddress() const {
	return peerAddressToSystemAddress(getRemoteAddress());
}

void Player::setRemoteAddress(const PeerAddress& address) {
	remoteAddress.reset(new PeerAddress(address));
}
...
:EnumDescriptor("MembershipType")
{
	...
	addPair(Player::MEMBERSHIP_PREMIUM,                  "Premium");
}
