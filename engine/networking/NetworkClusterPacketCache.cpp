...
template <class Key>
bool ClusterPacketCacheBase<Key>::fetchIfUpToDate(const Key &regionId, RBX::Network::PacketBuffer& outBitStream)
{
	boost::shared_lock<boost::shared_mutex> lock(sharedMutex);
...
template <class Key>
bool ClusterPacketCacheBase<Key>::update(const Key &regionId, RBX::Network::PacketBuffer& bitStream, unsigned int numBits)
{
	...
	if (!cachedStream.bitStream) {
		cachedStream.bitStream.reset(new RBX::Network::PacketBuffer());
	}
