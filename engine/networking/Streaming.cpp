...
void serializeEnumIndex(const Reflection::EnumDescriptor* enumDesc, const size_t& index, RBX::Network::PacketBuffer &bitStream, size_t enumSizeMSB/*default to 0*/)
{
...
void deserializeEnumIndex(const Reflection::EnumDescriptor* enumDesc, size_t& index, RBX::Network::PacketBuffer &bitStream, size_t enumSizeMSB/*default to 0*/)
{
    if (enumSizeMSB == 0)
    {
        enumSizeMSB = enumDesc->getEnumCountMSB();
...
void serializeEnum(const Reflection::EnumDescriptor* enumDesc , const Reflection::Variant& value, RBX::Network::PacketBuffer &bitStream, size_t enumSizeMSB/*default to 0*/)
{
...
void deserializeEnum(const Reflection::EnumDescriptor* enumDesc, Reflection::Variant& result, RBX::Network::PacketBuffer &bitStream, size_t enumSizeMSB/*default to 0*/)
{
	size_t index = 0;
...
void serializeEnumProperty(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream, size_t enumSizeMSB/*default to 0*/)
{
...
void deserializeEnumProperty(Property& property, RBX::Network::PacketBuffer &bitStream, size_t enumSizeMSB/*default to 0*/)
{
	const EnumPropertyDescriptor& enumDesc = static_cast<const EnumPropertyDescriptor&>(property.getDescriptor());
    size_t index = 0;
...
void serializeStringCompressed(const std::string& value, RBX::Network::PacketBuffer& stream)
{
...
void deserializeStringCompressed(std::string& value, RBX::Network::PacketBuffer& stream)
{
	uint32_t size;
	...
	value.resize(size);
	if (size && !stream.Read(value.data(), size))
		throw RBX::network_stream_exception("Unable to read string payload");
}
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, RBX::Guid::Scope& value)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, std::string& value)
{
	uint32_t size;
	Network::readFastT( stream, size );
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, BinaryString& value)
{
...
RBX::Network::PacketBuffer& operator << (RBX::Network::PacketBuffer& stream, const RBX::ContentId& value)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, RBX::ContentId& value)
{
	std::string text;
	stream >> text;
...
void writeBrickVector(RBX::Network::PacketBuffer& stream, const G3D::Vector3& value)
{
...
void readBrickVector(RBX::Network::PacketBuffer& stream, G3D::Vector3& value)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, G3D::Vector2& value)
{
	stream >> value.x;
	stream >> value.y;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, StreamRegion::Id& value)
{
	Vector3int32 vv;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, G3D::Vector3& value)
{
	stream >> value.x;
	stream >> value.y;
	stream >> value.z;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, G3D::Vector3int16& value)
{
	stream >> value.x;
	stream >> value.y;
	stream >> value.z;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, G3D::Vector2int16& value)
{
	stream >> value.x;
	stream >> value.y;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, RBX::Velocity& value)
{
	stream >> value.linear;
	stream >> value.rotational;
	return stream;
...
RBX::Network::PacketBuffer& operator << (RBX::Network::PacketBuffer& stream, const G3D::CoordinateFrame& cf)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, G3D::CoordinateFrame& value)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, UDim& value)
{
	int offset;
	stream >> value.scale;
	stream >> offset;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, UDim2& value)
{
	stream >> value.x;
	stream >> value.y;
	return stream;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, RbxRay& value)
{
	stream >> value.origin();
	stream >> value.direction();
	return stream;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, Faces& value)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, Axes& value)
{
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, BrickColor& value)
{
	size_t i = 0;
...
template<>
RBX::Network::PacketBuffer& operator >> (RBX::Network::PacketBuffer& stream, G3D::Color3& value)
{
	stream >> value.r;
	stream >> value.g;
	stream >> value.b;
...
template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, NumberSequenceKeypoint& p )
{
    return stream >> p.time >> p.value >> p.envelope;
}

RBX::Network::PacketBuffer& operator<<( RBX::Network::PacketBuffer& stream, const ColorSequenceKeypoint& p )
{
    return stream << p.time << p.value << p.envelope;
}

template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, ColorSequenceKeypoint& p )
{
    return stream >> p.time >> p.value >> p.envelope;
}


RBX::Network::PacketBuffer& operator<<( RBX::Network::PacketBuffer& stream, const NumberSequence& ns )
{
...
template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, NumberSequence& ns )
{
    uint32_t size;
    stream>>size;
...
RBX::Network::PacketBuffer& operator<<( RBX::Network::PacketBuffer& stream, const ColorSequence& ns )
{
...
template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, ColorSequence& ns )
{
    uint32_t size;
    stream>>size;
...
RBX::Network::PacketBuffer& operator<<( RBX::Network::PacketBuffer& stream, const NumberRange& r )
{
    return stream << r.min << r.max;
}

template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, NumberRange& r )
{
    return stream >> r.min >> r.max;
}

RBX::Network::PacketBuffer& operator<<( RBX::Network::PacketBuffer& stream, const Rect2D& r )
{
	return stream << r.x0y0() << r.x1y1();
}

template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, Rect2D& r )
{
	Vector2 x0y0;
	Vector2 x1y1;
	stream >> x0y0;
...
RBX::Network::PacketBuffer& operator<<( RBX::Network::PacketBuffer& stream, const PhysicalProperties& p)
{
...
template<>
RBX::Network::PacketBuffer& operator>>( RBX::Network::PacketBuffer& stream, PhysicalProperties& p)
{
	bool customEnabled;
	float density;
	float friction;
...
template<>
void serialize<RBX::ContentId>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
...
template<>
void serialize<UDim>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
...
template<>
void deserialize<UDim>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	UDim c;
	bitStream >> c;
	if (property.getInstance())
...
template<>
void serialize<UDim2>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
	bitStream << property.getValue<UDim2>();
}

template<>
void deserialize<UDim2>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	UDim2 c;
	bitStream >> c;
	if (property.getInstance())
...
template<>
void serialize<RBX::RbxRay>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
	bitStream << property.getValue<RBX::RbxRay>();
}

template<>
void deserialize<RBX::RbxRay>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	RbxRay c;
	bitStream >> c;
	if (property.getInstance())
...
template<>
void serialize<Faces>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
...
template<>
void deserialize<Faces>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	Faces c;
	bitStream >> c;
	if (property.getInstance())
...
template<>
void serialize<Axes>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
...
template<>
void deserialize<Axes>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	Axes c;
	bitStream >> c;
	if (property.getInstance())
...
template<>
void serialize<BrickColor>(const ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
	bitStream << property.getValue<BrickColor>();
}

template<>
void deserialize<BrickColor>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	BrickColor c;
	bitStream >> c;
	if (property.getInstance())
...
template<>
void deserialize<RBX::ContentId>(Property& property, RBX::Network::PacketBuffer &bitStream)
{
	std::string value;
	bitStream >> value;
	if (property.getInstance())
...
void serializeStringProperty(const Reflection::ConstProperty& property, RBX::Network::PacketBuffer &bitStream)
{
	bitStream << property.getDescriptor().getStringValue(property.getInstance());
}

void deserializeStringProperty(Reflection::Property& property, RBX::Network::PacketBuffer &bitStream)
{
	std::string value;
	bitStream >> value;
...
void serializeGuidScope(RBX::Network::PacketBuffer& stream, const RBX::Guid::Scope& value, bool canDisableCompression)
{
	if (canDisableCompression) {
		std::string scope = value.getName()->toString();
		stream.Write(scope);
	} else {
...
void deserializeGuidScope(RBX::Network::PacketBuffer& stream, RBX::Guid::Scope& value, bool canDisableCompression)
{
	std::string str;
	if (canDisableCompression) {
		std::string scope;
...
bool IdSerializer::trySerializeId(RBX::Network::PacketBuffer& stream, const Instance* instance)
{
	if (instance)
	{
...
void IdSerializer::sendId(RBX::Network::PacketBuffer& stream, const Id& id)
{
	if(id.valid){
...
void IdSerializer::serializeId(RBX::Network::PacketBuffer& stream, const Instance* instance)
{
	if (instance)
	{
		guidRegistry->registerGuid(instance);
...
void IdSerializer::serializeId(RBX::Network::PacketBuffer& stream, const RBX::Guid::Data& id) {
	scopeNames.send(stream, id.scope);
	stream.WriteBits((const unsigned char*) &id.index, 32);
}

void IdSerializer::serializeIdWithoutDictionary(RBX::Network::PacketBuffer& stream, const Instance* instance)
{
	RBX::Guid::Data id;
...
void IdSerializer::serializeIdWithoutDictionary(RBX::Network::PacketBuffer& stream, const RBX::Guid::Data& id)
{
	if (id.scope.isNull())
	{
...
void IdSerializer::deserializeId(RBX::Network::PacketBuffer& stream, RBX::Guid::Data& id)
{
...
void IdSerializer::deserializeIdWithoutDictionary(RBX::Network::PacketBuffer& stream, RBX::Guid::Data& id)
{
	unsigned char code = 0;
...
void IdSerializer::serializeInstanceRef(const Instance* instance, RBX::Network::PacketBuffer& bitStream)
{
...
bool IdSerializer::deserializeInstanceRef(RBX::Network::PacketBuffer& stream, shared_ptr<Instance>& instance, RBX::Guid::Data& id)
{
...
void DescriptorReceiver<ClassDescriptor>::learnName(std::string s, int i, uint32_t checksum)
{
	...
	while (iter!=ClassDescriptor::all_end())
	{
		if ((*iter)->name == n)
		{
			idToDesc.at(i).desc = *iter;
            idToDesc.at(i).outdated = !verifyChecksum((*iter), checksum);
            *((*iter)->isOutdated) = idToDesc.at(i).outdated;
	...
	idToDesc.at(i).desc = NULL;
    idToDesc.at(i).outdated = false;
}
...
void DescriptorReceiver<EventDescriptor>::learnName(std::string s, int i, uint32_t checksum)
{
	...
	if (words.size() != 2)
		throw RBX::network_stream_exception("malformed event descriptor name");
	...
	while (iter!=ClassDescriptor::all_end())
	{
		...
		if (c->name == n)
		{
			...
			{
				idToDesc.at(i).desc = desc;
                idToDesc.at(i).outdated = !verifyChecksum(desc, checksum);
                
				if (idToDesc.at(i).outdated)
                {
                    StandardOut::singleton()->printf(MESSAGE_WARNING, "EventDescriptor %s is out of date, replication will be ignored", s.c_str());
                }

				*(desc->isOutdated) = idToDesc.at(i).outdated;
...
void DescriptorReceiver<PropertyDescriptor>::learnName(std::string s, int i, uint32_t checksum)
{
	...
	boost::split(words, s, boost::is_any_of(":"));
	if (words.size() != 2)
		throw RBX::network_stream_exception("malformed property descriptor name");
...
void DescriptorReceiver<Type>::learnName(std::string s, int i, uint32_t checksum)
{
	...
	for (size_t ti = 0; ti < types.size(); ++ti)
	{
		if (types[ti]->name == n)
		{
			idToDesc.at(i).desc = types[ti];
            idToDesc.at(i).outdated = !verifyChecksum(types[ti], checksum);
			return;
		}
	}
