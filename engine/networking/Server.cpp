...
static shared_ptr<Network::ServerReplicator> createReplicator(PeerAddress a, Network::Server* s, NetworkSettings* networkSettings)
{
...
boost::function<shared_ptr<ServerReplicator>(PeerAddress, Server*, NetworkSettings*)> Server::createReplicator = ::createReplicator;
		...
		for (Instances::const_iterator iter = sharedServer->getChildren()->begin(); iter != end; ++iter)
		{
			if (ServerReplicator* rep = Instance::fastDynamicCast<ServerReplicator>(iter->get()))
			{
				if (Player* player = rep->getRemotePlayer())
				{
					...
					int elapsedTime = (GetTimeUS() - rep->stats().peerStats.transportStats.connectionStartTime) / 1e6f;
					if (elapsedTime > 5 * 60)
					{
...
void Server::start(int port, int threadSleepTime)
{
    ...
    StartupResult res = StartupFailed;
    std::vector<std::string> addresses;
    
#ifdef RBX_STUDIO_BUILD
    addresses = Server::getAllIPv4Addresses();
    shared_ptr<SocketDescriptor> sdArray(new SocketDescriptor[addresses.size()]);
    ...
    res = networkPeer->rawPeer()->Startup(maxClients, sdArray.get(), addresses.size());
#else
	SocketDescriptor d(port, 0);
	res = networkPeer->rawPeer()->Startup(maxClients, &d, 1);
#endif

	if (res != StartupSucceeded)
		throw std::runtime_error(RBX::format(
			"Failed to start network server, id %d: %s", res,
			networkPeer->rawPeer()->GetLastStartupError().empty()
				? "unspecified transport error"
				: networkPeer->rawPeer()->GetLastStartupError().c_str()));
...
#else
	PeerAddress address = networkPeer->rawPeer()->GetMyBoundAddress();
	outgoingPort = address.GetPort();

	StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "Started network server %s", peerAddressToString(address).c_str());
#endif
...
void Server::stop(int blockDuration)
{
	...
	// Do this before shutting down the peer because a replicator may still hold
	// a list of unprocessed packets owned by the networking runtime. The peer
	// clears this pool in shutdown.
	this->visitChildren(boost::bind(&Instance::unlockParent, _1));
	this->removeAllChildren();
	
	if (networkPeer->rawPeer()->IsActive())
...
void Server::configureAsCloudEditServer()
{
	...
	networkPeer->rawPeer()->SetIncomingPassword(Network::versionB.c_str(), Network::versionB.size());
...
void Server::onCreatePeer()
{	
	Super::onCreatePeer();
	networkPeer->rawPeer()->SetMaximumIncomingConnections(maxClients);
...
PluginReceiveResult Server::OnReceive(Packet *packet)
{
	...
	case ID_NEW_INCOMING_CONNECTION:
		{
			...
			try
			{
                ...
                // The legacy plugin dispatcher delivered the incoming-connect
                // notification to the newly attached ServerReplicator during
                // the same receive pass.  The owned transport uses an explicit
                // handler list, so forward this bootstrap packet directly;
                // otherwise the replicator never sends dictionaries, globals,
                // or initial join data.
                proxy->OnReceive(packet);
                incommingConnectionSignal(peerAddressToString(packet->systemAddress), proxy);
			}
