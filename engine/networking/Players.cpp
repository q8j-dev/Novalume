...
namespace {
	shared_ptr<RBX::Network::PacketBuffer> buildBasicChatBitStream(
		...
		shared_ptr<RBX::Network::PacketBuffer> data(new RBX::Network::PacketBuffer());
		*data << chatType;
		if (sender) {
...
void Players::reportAbuse(Player* player, const std::string& comment)
{
	...
	else
	{
		...
		StandardOut::singleton()->print(MESSAGE_INFO, "Sending abuse report to game server");
		...
		*bitStream << comment;
		...
		networkPeer->Send(bitStream, HIGH_PRIORITY, RELIABLE, CHAT_CHANNEL, UnassignedPeerAddress, true);
	}
}
...
void Players::gamechat(const std::string& message)
{
	...
	*bitStream << message;
	...
	networkPeer->Send(bitStream, CHAT_PRIORITY, CHAT_RELIABILITY, CHAT_CHANNEL, UnassignedPeerAddress, true);
}
...
static RBX::Guid::Data readPlayerIdentifier(RBX::Network::PacketBuffer &inBitStream)
{
	RBX::Guid::Data id;
...
static void writePlayerIdentifier(RBX::Network::PacketBuffer& bitStream, const boost::intrusive_ptr<GuidItem<Instance>::Registry>& guidRegistery, shared_ptr<Player> player)
{
	// Write the sender
...
Players::ReceiveResult Players::OnReceiveChat(Player* sourceValidation, ConcurrentPeer*peer, Packet *packet, unsigned char chatType)
{
	...
	if (!sourceInstance)
	{
		if(chatType == ID_CHAT_GAME){
			...
			StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "Chat message from unknown Player %s %s", peerAddressToString(packet->systemAddress).c_str(), message.c_str());
		}
	}
...
void Players::sendFilteredChatMessageSignalHelper(const PeerAddress& systemAddress,
		const shared_ptr<RBX::Network::PacketBuffer>& baseData, const shared_ptr<Instance> sourceInstance, shared_ptr<ChatMessage> chatEvent, const ChatFilter::Result& result) {
...
Players::ReceiveResult Players::OnReceiveReportAbuse(Player* player, ConcurrentPeer*peer, Packet *packet)
{
	// Read all the info
	RBX::Network::PacketBuffer inBitstream(packet->data, packet->length, false);
...
void Players::setConnection(ConcurrentPeer* networkPeer)
{
