...
bool PhysicsPacketCache::fetchIfUpToDate(const Assembly* key, unsigned char index, RBX::Network::PacketBuffer& outBitStream)
{
	...
	{	
		...
		if ((id > 0) && (id == key->getConstAssemblyPrimitive()->getWorld()->getWorldStepId()))
		{
			...
			outBitStream.WriteBits(cachedItem->bitStream->GetData(), cachedItem->dataBitStartOffset, (RBX::Network::BitCount)cachedItem->totalNumBits);
...
bool PhysicsPacketCache::update(const Assembly* key, unsigned char index, RBX::Network::PacketBuffer& bitStream, unsigned int startReadBitPos, unsigned int numBits)
{
	...
	if (iter != streamCache.end())
	{
		...
		if (!cachedItem->bitStream)
			cachedItem->bitStream.reset(new RBX::Network::PacketBuffer(numBytes));
...
bool InstancePacketCache::fetchIfUpToDate(const Instance* key, RBX::Network::PacketBuffer& outBitStream, bool isJoinData)
{
	boost::shared_lock<boost::shared_mutex> lock(sharedMutex);
...
bool InstancePacketCache::update(const Instance* key, RBX::Network::PacketBuffer& bitStream, unsigned int numBits, bool isJoinData)
{
	boost::upgrade_lock<boost::shared_mutex> lock(sharedMutex);
	...
	if (iter != streamCache.end())
	{
		...
		if (!cachedItem->bitStream[(int)isJoinData])
			cachedItem->bitStream[(int)isJoinData].reset(new RBX::Network::PacketBuffer());
