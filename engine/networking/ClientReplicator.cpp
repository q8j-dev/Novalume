...
public:
	...
	/*implement*/ bool write(RBX::Network::PacketBuffer& bitStream) 
	{
...
ClientReplicator::ClientReplicator(PeerAddress systemAddress, Client* client, PeerAddress clientAddress, NetworkSettings* networkSettings)
: Super( systemAddress, client->networkPeer, networkSettings, /*ClusterDebounce*/true)
...
PluginReceiveResult ClientReplicator::OnReceive(Packet *packet)
{
    ...
    case ID_DICTIONARY_FORMAT:
        {
            RBX::Network::PacketBuffer bitStream(packet->data, packet->length, false);
    ...
    case ID_PROTOCAL_MISMATCH:
		{
            std::string mismatchString = RBX::format("Protocol mismatch from %s", peerAddressToString(packet->systemAddress).c_str());
			...
			RBXASSERT(client);
			client->connectionFailedSignal(peerAddressToString(packet->systemAddress), (int) packet->data[0], "Network protocol mismatch. Please upgrade.");
    ...
    case ID_PLACEID_VERIFICATION:
        {
            ...
            else
            {
                ...
                Client *client = getParent()->fastDynamicCast<Client>();
                RBXASSERT(client);
                client->connectionFailedSignal(peerAddressToString(packet->systemAddress), (int) packet->data[0], "Illegal teleport destination.");
...
void ClientReplicator::receiveCluster(RBX::Network::PacketBuffer& inBitstream, Instance* instance, bool usingOneQuarterIterator)
{
	if (streamingEnabled) {
...
shared_ptr<DeserializedItem> ClientReplicator::deserializeItem(RBX::Network::PacketBuffer& inBitstream, RBX::Network::Item::ItemType itemType)
{
...
void ClientReplicator::readItem(RBX::Network::PacketBuffer& inBitstream, RBX::Network::Item::ItemType itemType)
{
	switch (itemType)
	{	
	default:
...
void ClientReplicator::readTag(RBX::Network::PacketBuffer& inBitstream)
{
	int id;
	inBitstream >> id;
...
shared_ptr<Reflection::ValueTable> ClientReplicator::readStats(RBX::Network::PacketBuffer& inBitstream)
{
...
void ClientReplicator::processRockyItem(RBX::Network::PacketBuffer& inBitstream)
{
    uint8_t idx;
...
void ClientReplicator::readStreamData(RBX::Network::PacketBuffer& bitStream)
{
...
{
	bool clientIsOwner = (part->getNetworkOwner() == peerAddressToSystemAddress(clientAddress));
...
void ClientReplicator::requestServerStats(bool request)
{
	...
	networkPeer->rawPeer()->Send(&bitStream, networkSettings->getDataSendPriority(), DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);	
}
...
void ClientReplicator::readChangedProperty(RBX::Network::PacketBuffer& bitStream, Reflection::Property prop)
{
	...
	if (streamingEnabled && prop.getDescriptor() == Instance::propParent)
	{
		...
		RBX::Network::BitCount currentReadOffset = bitStream.GetReadOffset();
...
void ClientReplicator::writePropAcknowledgementIfNeeded(const Instance* instance, const Reflection::PropertyDescriptor& desc, RBX::Network::PacketBuffer& outBitStream)
{
...
bool ClientReplicator::isLimitedByOutgoingBandwidthLimit() const {
	if (const PeerStatistics* transportStats = getPeerStatistics()) {
		return transportStats->isLimitedByOutgoingBandwidthLimit;
	}
	return true;
}

void ClientReplicator::writeChangedProperty(const Instance* instance, const Reflection::PropertyDescriptor& desc, RBX::Network::PacketBuffer& outBitStream)
{
...
void ClientReplicator::deserializeSFFlags(RBX::Network::PacketBuffer& inBitStream)
{
...
void ClientReplicator::learnSchema(RBX::Network::PacketBuffer& inBitStream)
{
#ifdef NETWORK_DEBUG
    ...
    std::string enumName, className, propName, propType, eventName;
    ...
    for (size_t i=0; i<numTotalEnum; i++)
    {
        ...
        const Name& enumNameName = Name::declare(enumName);
    ...
    for (size_t i=0; i<numClass; i++)
    {
        ...
        const Name& classNameName = Name::declare(className);
        ...
        for (size_t j=0; j<numProp; j++)
        {
            ...
            shared_ptr<ReflectionPropertyContainer> propertyContainer(new ReflectionPropertyContainer(propId, Name::declare(propName), typeId, propType, type, canReplicate, enumMSB));
            if (!type)
            {
                if (!isEnum)
                {
                    StandardOut::singleton()->printf(MESSAGE_WARNING, "A new type (%s) is used for property '%s', please make sure this property will not be replicated to or from older clients.", propType.c_str(), propName.c_str());
                }
            ...
            if (propDesc)
            {
                ...
                if (propDesc->canReplicate() != canReplicate
                    || strcmp(propType.c_str(), propDesc->type.name.c_str()) != 0)
                {
        ...
        for (size_t j=0; j<numEvent; j++)
        {
            ...
            bitStream >> numArg;
            shared_ptr<ReflectionEventContainer> eventContainer(new ReflectionEventContainer(eventId, Name::declare(eventName)));
...
void ClientReplicator::writeProperties(const Instance* instance, RBX::Network::PacketBuffer& outBitstream, PropertyCacheType cacheType, bool useDictionary)
{
...
bool ClientReplicator::ProcessOutdatedChangedProperty(RBX::Network::PacketBuffer& inBitstream, const RBX::Guid::Data& id, const Instance* instance, const Reflection::PropertyDescriptor* propertyDescriptor, unsigned int propId)
{
...
bool ClientReplicator::ProcessOutdatedProperties(RBX::Network::PacketBuffer& inBitstream, Instance* instance, PropertyCacheType cacheType, bool useDictionary, bool preventBounceBack, std::vector<PropValuePair>* valueArray)
{
    bool cacheable = (cacheType != PropertyCacheType_NonCacheable);
...
bool ClientReplicator::ProcessOutdatedInstance(RBX::Network::PacketBuffer& inBitstream, bool isJoinData, const RBX::Guid::Data& id, const Reflection::ClassDescriptor* classDescriptor, unsigned int classId)
{
#ifdef NETWORK_DEBUG
    if (FFlag::DebugProtocolSynchronization)
    {
...
bool ClientReplicator::ProcessOutdatedEventInvocation(RBX::Network::PacketBuffer& inBitstream, const RBX::Guid::Data& id, const Instance* instance, const Reflection::EventDescriptor* eventDescriptor, unsigned int eventId)
{
    if (instance && !(*(instance->getDescriptor().isOutdated)))
    {
...
bool ClientReplicator::ProcessOutdatedEnumSerialization(const Reflection::Type& type, const Reflection::Variant& value, RBX::Network::PacketBuffer& outBitStream)
{
...
bool ClientReplicator::ProcessOutdatedEnumDeserialization(RBX::Network::PacketBuffer& inBitStream, const Reflection::Type& type, Reflection::Variant& value)
{
    size_t newMSB = 0;
    if (const Reflection::EnumDescriptor* enumDesc = Reflection::EnumDescriptor::lookupDescriptor(type))
    {
...
bool ClientReplicator::ProcessOutdatedPropertyEnumSerialization(const Reflection::ConstProperty& property, RBX::Network::PacketBuffer& outBitStream)
{
...
bool ClientReplicator::ProcessOutdatedPropertyEnumDeserialization(Reflection::Property& property, RBX::Network::PacketBuffer& inBitStream)
{
    size_t newMSB = 0;
    const EnumPropertyDescriptor& enumDesc = static_cast<const EnumPropertyDescriptor&>(property.getDescriptor());
    if (hasEnumChanged(enumDesc.enumDescriptor, newMSB))
...
void ClientReplicator::skipPropertyValue(RBX::Network::PacketBuffer& inBitStream, const shared_ptr<ClientReplicator::ReflectionPropertyContainer>& prop, bool useDictionary)
{
...
void ClientReplicator::skipPropertiesInternal(const shared_ptr<ClientReplicator::ReflectionPropertyContainer>& prop, RBX::Network::PacketBuffer& inBitstream, bool useDictionary)
{
...
void ClientReplicator::skipChangedProperty(RBX::Network::PacketBuffer& bitStream, const shared_ptr<ClientReplicator::ReflectionPropertyContainer>& prop)
{
    bool versionReset;
    bitStream >> versionReset;
...
void ClientReplicator::skipEventInvocation(RBX::Network::PacketBuffer& bitStream, const shared_ptr<ReflectionEventContainer>& event)
{
    int numArgs;
    bitStream >> numArgs;
