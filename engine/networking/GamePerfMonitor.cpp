	...
	if (statsService)
	{
		if (Instance* networkStats = statsService->findFirstChildByName("Network"))
		{
			if (networkStats->numChildren() > 1)
			{
				...
				if (Stats::Item* networkPing = Instance::fastDynamicCast<Stats::Item>(stats->findFirstChildByName("Ping")))
					Analytics::EphemeralCounter::reportStats("ClientPing", networkPing->getValue());
