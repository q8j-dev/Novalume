...
template<typename T> 
void PhysicsSender::writeTouches(RBX::Network::PacketBuffer& bitStream, unsigned int maxBitStreamSize, T& touchPairList)
{
	RBX::SystemAddress addr = peerAddressToSystemAddress(replicator.remotePlayerId);
...
void PhysicsSender::sendTouches(PacketPriority packetPriority)
{
	...
	{
		boost::shared_ptr<RBX::Network::PacketBuffer> bitStream(new RBX::Network::PacketBuffer(maxStreamSize));
		...
		*bitStream << (unsigned char) ID_PHYSICS_TOUCHES;
		...
		replicator.networkPeer->Send(bitStream, packetPriority, RELIABLE_ORDERED, PHYSICS_CHANNEL, replicator.remotePlayerId, false);
...
void PhysicsSender::sendChildPrimitiveCoordinateFrame(Primitive *prim, RBX::Network::PacketBuffer* bitStream, Replicator *replicator, Compressor::CompressionType compressionType)
{
    ...
    if (PartInstance* part = PartInstance::fromPrimitive(prim))
    {
        if (replicator->isReplicationContainer(part))
        {
            ...
            RBX::Network::PacketBuffer& bitStreamRef = *bitStream;
...
void PhysicsSender::sendMechanismCFrames(RBX::Network::PacketBuffer& bitStream, const PartInstance* part, bool detailed)
{
	RBXASSERT(replicator.isStreamingEnabled());
...
void PhysicsSender::sendMechanism(RBX::Network::PacketBuffer& bitStream, const PartInstance* part, bool detailed, const Time& lastSendTime, CoordinateFrame& lastSendCFrame, float& accumulatedError, const PartInstance* playerHead)
{
    int byteStart = bitStream.GetNumberOfBytesUsed();
...
bool PhysicsSender::writeMovementHistory(RBX::Network::PacketBuffer& bitStream, const MovementHistory& history, const Assembly* assembly, const Time& lastSendTime, CoordinateFrame& lastSendCFrame, bool& hasMovement, Compressor::CompressionType compressionType, float& accumulatedError, const PartInstance* playerHead)
{
    hasMovement = true;
    return false; // client will not use path based part movement
}

void PhysicsSender::writeMechanismAttributes(RBX::Network::PacketBuffer& bitStream, const Assembly* assembly)
{
...
void PhysicsSender::sendChildAssembly(RBX::Network::PacketBuffer* bitStream, const Assembly* assembly, Compressor::CompressionType compressionType)
{
	RBX::Network::PacketBuffer& bitStreamRef = *bitStream;
...
void PhysicsSender::writeAssembly(RBX::Network::PacketBuffer& bitStream, const Assembly* assembly, Compressor::CompressionType compressionType, bool crossPacketCompression)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
void PhysicsSender::writePV(RBX::Network::PacketBuffer& bitStream, const Assembly* a, Compressor::CompressionType compressionType, bool crossPacketCompression)
{
...
void PhysicsSender::writeCoordinateFrame(RBX::Network::PacketBuffer& bitStream, const CoordinateFrame& cFrame, Compressor::CompressionType compressionType)
{
    int byteStart = bitStream.GetNumberOfBytesUsed();
...
void PhysicsSender::writeVelocity(RBX::Network::PacketBuffer& bitStream, const Velocity& velocity)
{
	if (replicator.settings().distributedPhysicsEnabled)
	{
        int byteStart = bitStream.GetNumberOfBytesUsed();
...
void PhysicsSender::writeMotorAngles(RBX::Network::PacketBuffer& bitStream, const Assembly* assembly, Compressor::CompressionType compressionType)
{
...
void PhysicsSender::writeCompactCFrame(RBX::Network::PacketBuffer& bitStream, const CompactCFrame& cFrame, Compressor::CompressionType compressionType)
{
...
bool PhysicsSender::canSend(const PartInstance* part, const Assembly* assembly, RBX::Network::PacketBuffer& bitStream) 
{
...
bool PhysicsSender::sendPhysicsData(RBX::Network::PacketBuffer& bitStream, const PartInstance* part, bool detailed, const Time& lastSendTime, CoordinateFrame& lastSendCFrame, float& accumulatedError, const PartInstance* playerHead)
{
	RBXASSERT(part);
