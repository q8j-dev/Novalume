		...
		ReplicatorStats::PhysicsSenderStats* stats)
{
	...
	boost::shared_ptr<RBX::Network::PacketBuffer> bitStream;
	...
	while (iter != nuggetList.end())
	{
		...
		if (!bitStream) {
			...
			bitStream.reset(new RBX::Network::PacketBuffer(maxStreamSize));

			++packetCount;

			*bitStream << (unsigned char) ID_TIMESTAMP;
			NetworkTime timestamp = GetTime();
			...
			if (!SFFlag::getPhysicsPacketSendWorldStepTimestamp() && DFFlag::PhysicsSenderUseOwnerTimestamp && nugget->part->computeNetworkOwnerIsSomeoneElse())
			{
				...
				RBXASSERT(timestamp >= nugget->part->networkTime);
				if (nugget->part->networkTime)
					timestamp = nugget->part->networkTime;
			}
			...
			*bitStream << (unsigned char) ID_PHYSICS;
			...
			if (SFFlag::getPhysicsPacketSendWorldStepTimestamp())
			{
				...
				{
					if (nugget->part->networkTime)
						interpolationTimeStamp = nugget->part->networkTime;
					else
				...
				else
				{
					...
					interpolationTimeStamp = (NetworkTime)((TaskScheduler::singleton().lastCyclcTimestamp +  Time::Interval(replicator.getRakOffsetTime() - timeOffsetDueToPhysics)).timestampSeconds()* 1000.0);
				}
		...
		if (bitStream->GetNumberOfBytesUsed() > static_cast<int>(maxStreamSize)) 
		{
			...
			// Send ID_PHYSICS
			replicator.networkPeer->Send(bitStream, packetPriority, UNRELIABLE, PHYSICS_CHANNEL, replicator.remotePlayerId, false);
...
bool TopNErrorsPhysicsSender::sendPhysicsData(RBX::Network::PacketBuffer& bitStream, const Nugget& nugget, CoordinateFrame& outLastSendCFrame, float& accumulatedError, const PartInstance* playerHead)
{
...
void TopNErrorsPhysicsSender::writeAssembly(RBX::Network::PacketBuffer& bitStream, const Assembly* assembly, Compressor::CompressionType compressionType, bool crossPacketCompression)
{
...
void TopNErrorsPhysicsSender::writePV(RBX::Network::PacketBuffer& bitStream, const Assembly* a, Compressor::CompressionType compressionType, bool crossPacketCompression)
{
