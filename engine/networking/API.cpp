...
RBX_REGISTER_ENUM(RBX::Network::PacketPriority);
RBX_REGISTER_ENUM(RBX::Network::PacketReliability);
...
static shared_ptr<RBX::Network::ServerReplicator> createSecureReplicator(PeerAddress a, RBX::Network::Server* s, RBX::NetworkSettings* networkSettings)
{
...
#if (defined(_WIN32) && (defined(LOVE_ALL_ACCESS) || defined(_NOOPT) || defined(_DEBUG) || defined(RBX_TEST_BUILD))) || (defined(__APPLE__) && defined(__arm__)) || defined(__ANDROID__) || (defined(__APPLE__) && defined(__aarch64__)) || defined(RBX_PLATFORM_DURANGO) || defined(__linux__) || defined(__EMSCRIPTEN__)
...
void isDebuggedDirectThreadFunc(weak_ptr<RBX::DataModel> weakDataModel) {
	...
	while (true) {
		...
		unsigned int mask = static_cast<unsigned int>(::IsDebuggerPresent() != FALSE) * HATE_DEBUGGER;
