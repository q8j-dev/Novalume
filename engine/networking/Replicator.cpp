...
DYNAMIC_FASTINT(NetworkMaxSplitPacketCount)
...
static Reflection::BoundFuncDesc<Replicator, std::string(int)> func_getNetworkStatsString(&Replicator::getNetworkStatsString, "GetNetworkStatsString", "verbosityLevel", 0, Security::Plugin);
...
RBX::Time Replicator::remoteNetworkTimeToLocalTime(const RemoteTime& time)
{
	return time - Time::Interval(rakTimeOffset);
}

RBX::Time Replicator::networkTimeToLocalTime(const NetworkTime& time)
{
	RBX::RemoteTime t((double)time / 1000.0f);
	return t - Time::Interval(rakTimeOffset);
}

NetworkTime Replicator::localTimeToNetworkTime(const RBX::Time& time)
{
	return (NetworkTime)(time + Time::Interval(rakTimeOffset)).timestampSeconds();
}

void Replicator::writeInstance(shared_ptr<Instance> instance, RBX::Network::PacketBuffer* outBitstream)
{
...
void Replicator::writeProperties(const Instance* instance, RBX::Network::PacketBuffer& outBitstream, PropertyCacheType cacheType, bool useDictionary)
{
...
void Replicator::writePropertiesInternal(const Instance* instance, const Reflection::ConstProperty& property, RBX::Network::PacketBuffer& outBitstream, PropertyCacheType cacheType, bool useDictionary)
{
...
{
	const PeerStatistics* statistics = getPeerStatistics();
	if (!statistics) {
		return 0;
	}
	...
	,deserializePacketsThreadEnabled(false)
{
	if (networkPeer)
	{
		networkPeer->addStats(remotePlayerId,
                          boost::bind(&Replicator::onStatisticsChanged, this, _1));
		networkPeer->rawPeer()->AttachPlugin(this);
	}

    NetworkTime networkNow = GetTime();
    rakTimeOffset = (double)networkNow/1000.0f - Time::nowFastSec();

	FASTLOGS(FLog::Network, "Replicator created for player %s", peerAddressToString(remotePlayerId).c_str());
...
{
	return peerAddressToString(remotePlayerId, false);
}
...
void Replicator::clearIncomingPackets()
{
	...
	{
		networkPeer->DeallocatePacket(packet);
	}
}
...
std::string Replicator::getNetworkStatsString(int verbosityLevel)
{
	const PeerStatistics* statistics = getPeerStatistics();
	if (!statistics)
		return "No active connection";
	return RBX::format(
		"send=%.0f B/s receive=%.0f B/s pending=%u B loss=%.2f%%",
		statistics->valueOverLastSecond[ACTUAL_BYTES_SENT],
		statistics->valueOverLastSecond[ACTUAL_BYTES_RECEIVED],
		statistics->bytesInSendBuffer[MEDIUM_PRIORITY],
		statistics->packetlossLastSecond * 100.0f);
}

const PeerStatistics* Replicator::getPeerStatistics() const
{
	return networkPeer ? &(replicatorStats.peerStats.transportStats) : 0;
}
...
void Replicator::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
	...
	if (!newProvider)
	{
		...
		if (networkPeer)
		{
			...
			boost::shared_ptr<ConcurrentPeer> temp(networkPeer);
			networkPeer.reset();
...
void Replicator::sendFilteredChatMessage(const PeerAddress &systemAddress, const shared_ptr<RBX::Network::PacketBuffer> &data, const shared_ptr<Instance> sourceInstance,
										 const std::string &whitelist, const std::string &blacklist)
{
	...
	shared_ptr<RBX::Network::PacketBuffer> dataCopy(new RBX::Network::PacketBuffer());
	...
	if (networkPeer)
		networkPeer->Send(dataCopy, CHAT_PRIORITY, CHAT_RELIABILITY, CHAT_CHANNEL, remotePlayerId, false);
}
...
void Replicator::updateStatsItem(RBX::Stats::StatsService* stats)
{
	...
	if (stats!=NULL)
	{
		...
		if (network)
		{
			...
			statsItem->setName(peerAddressToString(this->remotePlayerId));
			statsItem->setParent2(network);
		}
	}
}

void Replicator::learnDictionaries(RBX::Network::PacketBuffer& rawBitStream, bool compressed, bool learnSchema)
{
    RBX::Network::PacketBuffer bitStream;
    RBX::Network::PacketBuffer* inBitstream;
    if (compressed)
    {
...
void Replicator::serializeEventInvocation(const Reflection::EventInvocation& eventInvocation, RBX::Network::PacketBuffer& outBitStream)
{
...
bool Replicator::serializeValue(const Reflection::Type& type, const Reflection::Variant& value, RBX::Network::PacketBuffer& outBitStream)
{
...
void Replicator::serializeVariant(const Reflection::Variant& value, RBX::Network::PacketBuffer& outBitStream)
{
...
void Replicator::serializePropertyValue(const Reflection::ConstProperty& property, RBX::Network::PacketBuffer& outBitStream, bool useDictionary)
{
...
void Replicator::deserializeEventInvocation(RBX::Network::PacketBuffer& inBitStream, Reflection::EventInvocation& eventInvocation, bool resolveRefTypes)
{
	int numEntries;
...
bool Replicator::deserializeValue(RBX::Network::PacketBuffer& inBitStream, const Reflection::Type& type,	Reflection::Variant& value)
{
	if (type==Reflection::Type::singleton<bool>())
...
void Replicator::deserializeVariant(RBX::Network::PacketBuffer& inBitStream, Reflection::Variant& value)
{
...
void Replicator::deserializePropertyValue(RBX::Network::PacketBuffer& inBitStream, Reflection::Property property, bool useDictionary, bool preventBounceBack, Reflection::Variant* outValue)
{
    // NOTE:
...
void Replicator::writeChangedProperty(const Instance* instance, const Reflection::PropertyDescriptor& desc, RBX::Network::PacketBuffer& outBitStream)
{
	...
	if (settings().printProperties) {
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_SENSITIVE, 
			...
			desc.name.c_str(),
			peerAddressToString(remotePlayerId).c_str(),
...
void Replicator::onStatisticsChanged(const ConnectionStats& newStats) {
	...
	replicatorStats.peerStats.transportStats = newStats.transportStats;
...
void Replicator::onEventInvocation(Instance* instance, const Reflection::EventDescriptor* descriptor, const Reflection::EventArguments* args, const SystemAddress* target)
{
    ...
    if (target && *target != peerAddressToSystemAddress(remotePlayerId))
        return;
...
void Replicator::logPacketError(Packet* packet, const std::string& type, const std::string& message)
{
...
bool Replicator::processNextIncomingPacket()
{
...

	{
		...
		catch (RBX::physics_receiver_exception& e)
		{
			...
			printf("(ignored) %s (Packet size: %u, packet type: %d)\n", e.what(), packet->length * 8, packet->data[0]); // for unit test
		}
...
bool Replicator::sendItemsPacket()
{
	...
	{
		ItemSender sender(*this, networkPeer.get());
...
template <class Container> void sendClusterContentHelper(MegaClusterInstance* megaClusterInstance, shared_ptr<RBX::Network::PacketBuffer>& bitStream, Container& container, int maxBytesSend)
{
...
void Replicator::sendClusterContent(shared_ptr<RBX::Network::PacketBuffer>& bitStream, ClusterUpdateBuffer& container, unsigned maxBytesSend)
{
    sendClusterContentHelper(megaClusterInstance, bitStream, container, maxBytesSend);
}

void Replicator::sendClusterContent(shared_ptr<RBX::Network::PacketBuffer>& bitStream, ClusterChunksIterator& container, unsigned maxBytesSend)
{
    sendClusterContentHelper(megaClusterInstance, bitStream, container, maxBytesSend);
}

void Replicator::sendClusterContent(shared_ptr<RBX::Network::PacketBuffer>& bitStream, OneQuarterClusterChunkCellIterator &container)
{
...
void Replicator::sendClusterChunk(const StreamRegion::Id &regionId)
{
	...
	if (megaClusterInstance->isSmooth())
	{
        ...
        shared_ptr<RBX::Network::PacketBuffer> bitStream(new RBX::Network::PacketBuffer());
        *bitStream << (unsigned char) ID_CLUSTER;
        *bitStream << true; // using 1/4 iterator (streaming)

        serializeId(*bitStream, megaClusterInstance);

		Voxel2::BitSerializer<RBX::Network::PacketBuffer> serializer;
        ...
        *bitStream << (char)0; // end token
        ...
        networkPeer->Send(bitStream, NetworkSettings::singleton().getDataSendPriority(), DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
	}
	else
	{
...
bool Replicator::sendClusterPacket()
{
	...
	*bitStream << false; // using regular iterator (non-streaming)
    ...
    networkPeer->Send(bitStream, settings().getDataSendPriority(),
...
void Replicator::compressBitStream(const RBX::Network::PacketBuffer& inUncompressedBitStream, RBX::Network::PacketBuffer& outCompressedBitStream, uint8_t compressRatio)
{
#ifdef NETWORK_DEBUG
...
void Replicator::decompressBitStream(RBX::Network::PacketBuffer& inCompressedBitStream, RBX::Network::PacketBuffer& outUncompressedBitStream)
{
    RBXPROFILER_SCOPE("Network", "decompressBitStream");
    
    unsigned int len = 0;
    if (!inCompressedBitStream.Read(len))
        throw std::runtime_error("compressed packet is missing its payload length");
    const std::size_t unreadBytes =
        (inCompressedBitStream.GetNumberOfBitsUsed() - inCompressedBitStream.GetReadOffset()) / 8;
    if (len > unreadBytes || len > RBX::Network::PacketBuffer::MaximumPacketBytes)
        throw std::runtime_error("compressed packet payload length is invalid");

    // read compressed data from bitstream
    boost::scoped_ptr<char> buffer;
    buffer.reset(new char[len]);
    if (!inCompressedBitStream.Read(buffer.get(), len))
        throw std::runtime_error("compressed packet payload is truncated");
...
void Replicator::readProperties(RBX::Network::PacketBuffer& inBitstream, Instance* instance, PropertyCacheType cacheType, bool useDictionary, bool preventBounceBack, std::vector<PropValuePair>* valueArray)
{
            ...
            && (!cacheable || descriptor != Instance::propParent)) // don't read parent if it's cacheable (PropertyCacheType_Cacheable) or potentially cacheable (PropertyCacheType_All)
		{
			...
			if (valueArray)
			{
				...
				try {
					...
					throw RBX::network_stream_exception(RBX::format(
						"property %s.%s at bit %u: %s", instance->getClassName().c_str(),
						descriptor.name.c_str(), static_cast<unsigned int>(inBitstream.GetReadOffset()),
						error.what()));
				}
				if (!value.isVoid())
...
void Replicator::readPropertiesInternal(Reflection::Property& property, RBX::Network::PacketBuffer& inBitstream, bool useDictionary, bool preventBounceBack, Reflection::Variant* outValue)
{
...
// 4) Parent property
//
void Replicator::readInstanceNew(RBX::Network::PacketBuffer& inBitstream, bool isJoinData)
{
...
void Replicator::receiveCluster(RBX::Network::PacketBuffer& inBitstream, Instance* instance, bool usingOneQuarterIterator)
{
	if(instance == NULL)
		return;
...
void Replicator::deserializeData(RBX::Network::PacketBuffer& inBitstream, std::vector<shared_ptr<DeserializedItem> >& items)
{
	RBX::Security::Impersonator impersonate(RBX::Security::Replicator_);
...
void Replicator::receiveData(RBX::Network::PacketBuffer& inBitstream)
{
	RBX::Security::Impersonator impersonate(RBX::Security::Replicator_);
	...
	while (true)
	{
		...
		const unsigned int itemOffset = static_cast<unsigned int>(inBitstream.GetReadOffset());
		try {
			readItem(inBitstream, itemType);
		} catch (const std::exception& error) {
			throw RBX::network_stream_exception(RBX::format(
				"item type %u at bit %u: %s", static_cast<unsigned int>(itemType),
				itemOffset, error.what()));
		}
	}
}

shared_ptr<DeserializedItem> Replicator::deserializeItem(RBX::Network::PacketBuffer& inBitstream, Item::ItemType itemType)
{
...
void Replicator::readItem(RBX::Network::PacketBuffer& inBitstream, Item::ItemType itemType)
{
...
{
	NetworkTime timeStamp = GetTimeMS();
        ...
        (timeStamp - replicatorStats.lastReceivedHashTime) > 120000)
    {
        ...
        FASTLOGS(FLog::Network, "User has stopped sending memory hashes:  %s", peerAddressToString(remotePlayerId).c_str());
        sendDisconnectionSignal(peerAddressToString(remotePlayerId), true);
        ...
        (timeStamp - replicatorStats.lastReceivedMccTime) > 120000)
    {
        ...
        FASTLOGS(FLog::Network, "User has stopped sending Mcc Reports:  %s", peerAddressToString(remotePlayerId).c_str());
        sendDisconnectionSignal(peerAddressToString(remotePlayerId), true);
        requestDisconnect(DisconnectReason_HashTimeOut);
    }
#endif 
    if (canTimeout && ((timeStamp - replicatorStats.lastReceivedPingTime) > 120000) && !DFFlag::DebugDisableTimeoutDisconnect)
	{
		...
		FASTLOGS(FLog::Network, "Lost connection to %s due timeout", peerAddressToString(remotePlayerId).c_str());
		sendDisconnectionSignal(peerAddressToString(remotePlayerId), true);
...
void Replicator::readDataPing(RBX::Network::PacketBuffer& inBitstream)
{
    int start = inBitstream.GetReadOffset();
	bool isPingBack;
...
void Replicator::processDataPing(bool isPingBack, NetworkTime timeStamp)
{
	if (isPingBack) {
		int elapsedTime = (int)(GetTimeMS() - timeStamp);
	...
	replicatorStats.lastReceivedPingTime = GetTimeMS();
}
...
void Replicator::readMarker(RBX::Network::PacketBuffer& inBitstream)
{
	int id;
	inBitstream >> id;
	processMarker(id);
...
void Replicator::readChangedProperty(RBX::Network::PacketBuffer& inBitstream)
{
    int start = inBitstream.GetReadOffset();
	shared_ptr<Instance> instance;
	RBX::Guid::Data id;
...
void Replicator::readChangedProperty(RBX::Network::PacketBuffer& bitStream, Reflection::Property prop)
{
...
void Replicator::readEventInvocationItem(DeserializedEventInvocationItem* item)
{
	...
	if (instance)
	{
		...
		instance->processRemoteEvent(*item->eventDescriptor, item->eventInvocation->args, peerAddressToSystemAddress(remotePlayerId));
...
void Replicator::readEventInvocation(RBX::Network::PacketBuffer& inBitstream)
{
	shared_ptr<Instance> instance;
	RBX::Guid::Data id;
	deserializeInstanceRef(inBitstream, instance, id);
	...
	if (instance)
	{
        instance->processRemoteEvent(*eventDescriptor, eventInvocation.args, peerAddressToSystemAddress(remotePlayerId));
...
unsigned int Replicator::readJoinData(RBX::Network::PacketBuffer& inBitstream)
{
...
{
	if (networkPeer)
		networkPeer->rawPeer()->CloseConnection(remotePlayerId, true);
...
void Replicator::readInstanceDelete(RBX::Network::PacketBuffer& inBitstream)
{
    int start = inBitstream.GetReadOffset();
	RBX::Guid::Data id;
...
{
	RBX::Network::PacketBuffer inBitstream(packet->data, packet->length, false);
	...
	case ID_TEACH_DESCRIPTOR_DICTIONARIES:
		{
            ...
            // Dictionary-format negotiation and the dictionary itself are
            // separate reliable messages.  Legacy RakNet happened to expose
            // the format packet to the plugin before the next message was
            // dequeued; modern transports may deliver the ordered batch to a
            // consumer whose plugin state has not yet observed that side-band
            // packet.  The compressed representation is unambiguous: a
            // network-order byte length followed by a gzip member.  Recognize
            // it at the payload boundary so a valid compressed dictionary can
            // never be interpreted as an enormous descriptor count.
            if (!compressedDictionary && !isServerReplicator() &&
                inBitstream.GetReadOffset() == 8 && packet->length >= 7)
            {
                const unsigned char* bytes = packet->data;
                const std::uint32_t compressedBytes =
                    (static_cast<std::uint32_t>(bytes[1]) << 24) |
                    (static_cast<std::uint32_t>(bytes[2]) << 16) |
                    (static_cast<std::uint32_t>(bytes[3]) << 8) |
                    static_cast<std::uint32_t>(bytes[4]);
                if (compressedBytes == packet->length - 5 &&
                    bytes[5] == 0x1f && bytes[6] == 0x8b)
                {
                    learnSchema = true;
                    compressedDictionary = true;
                }
            }
...
void Replicator::teachDictionaries(const Replicator* rep, RBX::Network::PacketBuffer& bitStream, bool teachSchema, bool toBeCompressed)
{
...
void Replicator::sendDictionaries()
{
    ...
    networkPeer->rawPeer()->Send(&bitStream, settings().getDataSendPriority(),
        DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
}
...
void Replicator::deserializePacketsThreadImpl()
{
	...
	while (deserializePacketsThreadEnabled)
	{
		...
		if (!packetsToDeserailze.empty())
		{
			...
			RBX::Network::PacketBuffer inBitstream(receivedPacket->data, receivedPacket->length, false);
			inBitstream.IgnoreBits(8); // Ignore the packet id
			...
			catch(std::out_of_range& e) // Workaround for iPad, where exceptions can't be caught by ancestor class
			{
				...
				networkPeer->DeallocatePacket(deserializedPacket.rawPacket);
				requestDisconnectWithSignal(DisconnectReason_ReceivePacketError);
				break;
			}
			catch (RBX::network_stream_exception& e)
	...
	if (packet->systemAddress==remotePlayerId)
	{
		try
		{
			...
			case ID_CONNECTION_LOST:
				...
				FASTLOGS(FLog::Network, "Lost connection to %s: ID_CONNECTION_LOST", peerAddressToString(packet->systemAddress).c_str());

				StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "Lost connection to %s\n", peerAddressToString(packet->systemAddress).c_str());
				sendDisconnectionSignal(peerAddressToString(packet->systemAddress), true);

				{
				// We cannot set parent to NULL while processing a network update.
			...
			case ID_DISCONNECTION_NOTIFICATION:
                {
				FASTLOGS(FLog::Network, "Disconnecto from %s: ID_DISCONNECTION_NOTIFICATION", peerAddressToString(packet->systemAddress).c_str());
				StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "Disconnect from %s", peerAddressToString(packet->systemAddress).c_str());
                ...
                if (postDisconnect)
                {
				    sendDisconnectionSignal(peerAddressToString(packet->systemAddress), false);
    				// We cannot set parent to NULL while processing a network update.
	    			DataModel::get(this)->submitTask(boost::bind(&scheduledRemove, shared_from(this)), DataModelJob::Write);
                }
                }
			...
			case ID_CHAT_PLAYER:
				...
				switch (players->OnReceiveChat(getRemotePlayer(), this->networkPeer->rawPeer(), packet, packet->data[0]))
				{
			...
			case ID_REPORT_ABUSE:
				switch (players->OnReceiveReportAbuse(findTargetPlayer(), this->networkPeer->rawPeer(), packet))
				{
				case Players::PLAYERS_STOP_PROCESSING:
					return RR_STOP_PROCESSING;
				default:
...
void Replicator::OnInternalPacket(InternalPacket *internalPacket, unsigned frameNumber, PeerAddress remoteSystemAddress, NetworkTimeMS time, int isSend) 
{
	PacketHandler::OnInternalPacket(internalPacket, frameNumber, remoteSystemAddress, time, isSend);
	if (isSend)
	{
		...
		else
		{
			...
			if (internalPacket->splitPacketCount > (uint32_t)DFInt::NetworkMaxSplitPacketCount)
			{
				RBXASSERT(internalPacket->splitPacketCount < (uint32_t)DFInt::NetworkMaxSplitPacketCount);
	...
	*bitStream << id;
	...
	if (settings().printInstances) {
		RBX::StandardOut::singleton()->printf(RBX::MESSAGE_SENSITIVE,
			...
			id, peerAddressToString(remotePlayerId).c_str());
	}

	incomingMarkers.push(marker);

	// Send ID_REQUEST_MARKER
	networkPeer->Send(bitStream, networkSettings->getDataSendPriority(), RELIABLE, DATA_CHANNEL, remotePlayerId, false);
...
double Replicator::getMetricValue(const std::string& metric) const
{
	...
	{
		FASTLOG1(FLog::NetworkStatsReport, "Reporting ACTUAL_BYTES_RECEIVED: %d", getPeerStatistics()->runningTotal[ACTUAL_BYTES_RECEIVED]);
		return getPeerStatistics()->runningTotal[ACTUAL_BYTES_RECEIVED];	
	}
