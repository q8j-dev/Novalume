...
class TransportStatsItem : public Stats::Item
{
	const PeerStatistics* statistics;
...
public:
	TransportStatsItem(const PeerStatistics* statistics)
		:statistics(statistics)
	{
		...
		addBuffer("messageSendBuffer", statistics->messageInSendBuffer.data());
		addBuffer("bytesInSendBuffer", statistics->bytesInSendBuffer.data());
...
public:
		...
		,instanceBits(0)
	{
		...
		Creatable<Instance>::create<TransportStatsItem>(&stats.peerStats.transportStats)->setParent(this);
	...
	virtual void update() {
		...
		if(shared_ptr<const Replicator> locked = replicator.lock())
		{
            ...
            if (locked->networkPeer)
			{
				bandwidthExceeded->formatPercent(locked->networkPeer->GetBandwidthExceeded(locked->remotePlayerId));
				congestionControlExceeded->formatPercent(locked->networkPeer->GetCongestionControlExceeded(locked->remotePlayerId));
				...
				elapsedTime->formatValue((GetTimeUS() - stats.peerStats.transportStats.connectionStartTime) / 1e6f);
