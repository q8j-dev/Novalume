...
shared_ptr<KeyframeSequence> KeyframeSequenceProvider::privateGetKeyframeSequence(ContentId assetId, bool blocking, bool useCache, const std::string& contextString, const Instance* contextInstance)
{
	...
	if(assetId.isHttp() || assetId.isAsset() || assetId.isAssetId())
	{
		...
		if (DFFlag::AnimationAllowProdUrls &&
			(boost::starts_with(assetId.toString(), "http://www.roblox.com") ||
			 boost::starts_with(assetId.toString(), "https://www.roblox.com")))	{
			// The historical /asset endpoint was retired. This is Roblox's current
			// binary asset delivery endpoint and returns the same KeyframeSequence
			// payload consumed below.
			url.append("https://assetdelivery.roblox.com/v1/asset/?id=");
		}
		else {
		...
		if (boost::starts_with(url, "https://assetdelivery.roblox.com/"))
			url.append(assetIdNumber);
		else {
