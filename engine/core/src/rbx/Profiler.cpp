...
namespace RBX
{
	namespace Profiler
	{
		...
		void enableCapture()
		{
			FFlag::OnScreenProfiler = true;
			MicroProfileSetEnableAllGroups(true);
			if (MicroProfileWebServerPort() == 0)
				g_MicroProfile.bWebServerStart = gWebServerEnabled;
		}

		void dumpToFile(const char* path, unsigned int frames)
		{
			enableCapture();
			MicroProfileDumpFile(path, MicroProfileDumpTypeHtml, frames);
		}
		...
		void setVisible(bool visible)
		{
			...
			else if (FFlag::OnScreenProfiler)
			{
				MicroProfileSetDisplayMode(MP_DRAW_OFF);
			}
		}
		...
		{
			if (enabled == gWebServerEnabled)
				return;

			gWebServerEnabled = enabled;
			if (enabled)
			{
				enableCapture();
				g_MicroProfile.bWebServerStart = true;
			}
			else
			{
				if (MicroProfileWebServerPort() != 0)
					...
					g_MicroProfile.bWebServerStart = false;
			}
		}

		bool isWebServerEnabled()
		{
			return gWebServerEnabled;
		}

		unsigned int getWebServerPort()
		{
			return MicroProfileWebServerPort();
		}

		const char* getWebServerAddress()
		{
...
namespace RBX
{
	namespace Profiler
	{
		...
		void dumpToFile(const char*, unsigned int)
		{
		}
