...
public:
	...
	void join()
	{
		...
		if (thread && thread->joinable() &&
			thread->get_id() != boost::this_thread::get_id())
...
void TaskScheduler::Thread::releaseJob()
{
	...
	shared_ptr<CEvent> completedJoinEvent;

	if (job->isRemoveRequested)
	{
		// A blocking remover is waiting for the worker to release its final
		// reference, not merely for Job::step to return.  Signalling here used
		// to let an owner destroy the job (and its arbiter) while this thread
		// still accessed it below and during job.reset().
		completedJoinEvent = job->joinEvent;
	...
	if (completedJoinEvent)
		completedJoinEvent->Set();
}
