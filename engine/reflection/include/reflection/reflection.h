...
#define NULL_FUNCTION_PTR decltype(NULL)
...
#define RBX_REGISTER_CLASS(Class)		template<> RBX::Reflection::ClassRegistrar<Class> RBX::Reflection::ClassRegistrar<Class>::registrar(0)
		...
		public:
			...
			{
				// Generic style/tween/replication paths preserve the reflected enum
				// token type. Numeric values remain accepted for historical callers.
				if (value.isType<V>())
					setValue(instance, value.cast<V>());
				else if (!setEnumValue(instance, value.get<int>()))
		...
		class ArgHelper
		{
			...
			template <int index, class T>
			static bool try_integral(FunctionDescriptor::Arguments& arguments, long long& arg)
			{
				double value;
				if (!arguments.getDouble(index, value))
					return false;
				arg = static_cast<long long>(value);
				return true;
			}
		...
		public:
			CustomBoundFuncDesc(CustomFunctionPtr function, const char* name,
				const char* arg1Name,
				typename boost::function_traits<Signature>::arg1_type default1,
				Security::Permissions security,
				Descriptor::Attributes attributes = Descriptor::Attributes())
				: BoundFuncDesc<Class, Signature, 1>(NULL, name, arg1Name,
					default1, security, attributes)
				, customFunction(function)
			{
				this->kind = FunctionDescriptor::Kind_Custom;
			}
